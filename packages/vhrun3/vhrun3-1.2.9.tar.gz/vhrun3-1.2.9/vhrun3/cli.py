...
from viper_hrunner.vhrun.report.aggregate import summary_gather
from viper_hrunner.vhrun import __description__, __version__
from viper_hrunner.vhrun.compat import ensure_cli_args
from viper_hrunner.vhrun.ext.har2case import init_har2case_parser, main_har2case
from viper_hrunner.vhrun.make import init_make_parser, main_make
from viper_hrunner.vhrun.scaffold import init_parser_scaffold, main_scaffold
from viper_hrunner.vhrun.utils import init_sentry_sdk
...
def main_run(extra_args) -> tuple:
    ...
    from viper_hrunner.vhrun.report.html.gen_report import gen_html_report
    report_path = ""
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        if sys.argv[1] in ["-V", "--version"]:
            # viper_hrunner.vhrun -V
            print(f"{__version__}")
        elif sys.argv[1] in ["-h", "--help"]:
            # viper_hrunner.vhrun -h
            parser.print_help()
        elif sys.argv[1] == "startproject":
            # viper_hrunner.vhrun startproject
            sub_parser_scaffold.print_help()
        elif sys.argv[1] == "har2case":
            # viper_hrunner.vhrun har2case
            sub_parser_har2case.print_help()
        elif sys.argv[1] == "run":
            # viper_hrunner.vhrun run
            pytest.main(["-h"])
        elif sys.argv[1] == "make":
            # viper_hrunner.vhrun make
            sub_parser_make.print_help()
        sys.exit(0)
    elif (
        len(sys.argv) == 3 and sys.argv[1] == "run" and sys.argv[2] in ["-h", "--help"]
    ):
        # viper_hrunner.vhrun run -h
        pytest.main(["-h"])
        sys.exit(0)
...
def main_hrun_alias():
    """ command alias
        hrun = viper_hrunner.vhrun run
    """
    if len(sys.argv) == 2:
        if sys.argv[1] in ["-V", "--version"]:
            # hrun -V
            sys.argv = ["viper_hrunner.vhrun", "-V"]
        elif sys.argv[1] in ["-h", "--help"]:
            pytest.main(["-h"])
            sys.exit(0)
        else:
...
def main_make_alias():
    """ command alias
        hmake = viper_hrunner.vhrun make
    """
...
def main_har2case_alias():
    """ command alias
        har2case = viper_hrunner.vhrun har2case
    """
