...
class PyS3:
    def __init__(self, path, bucket, accesskey, secretkey, region):
        self.path = path
        self.bucket = bucket
    ...
    def _get_all_op_objects(self):
        ...
        for i in folders:
            ...
            self.op_keys[i.replace("\\", "/").replace(self.path.replace("\\", "/"), "")[1:]] = etag.replace('"', '')
...
if __name__ == "__main__":
    s3 = PyS3(path="xxx", bucket="xxx", accesskey="xxx", secretkey="xxx", region="xxx")
    s3.run()
