...
class NewsItem:
    ...
    def from_rss_entry(cls, entry, max_title_length):
        try:
            ...
            log.error("unable to parse entry " + entry)
...
class NewsProcessor:
    ...
    def parseFeeds(self):
        ...
        if newest_item_timestamp > self.newest_item_timestamp:
            ...
            log.debug(
                "parseFeeds fetched " + str(len(new_newsItems)) + " new news items..."
            )
            self.newest_item_timestamp = newest_item_timestamp
            if len(new_newsItems) > max_notify_items:
                log.debug(
                    "parseFeeds limiting notifications to "
        ...
        if len(newsItems) > max_display_items:
            log.debug(
                "parseFeeds limiting display to "
