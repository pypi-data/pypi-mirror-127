...
class DirectSolver:
	...
	def power_method(self):
		...
		"""
		A, x, tol, N = self.A, self.x, self.tol, self.max_iter
	...
	def steepest_descent(self, b):
		...
		self.errors : tuple
			Collection of yielded norms.
		"""
		A, x0, tol, N = self.A, self.x, self.tol, self.max_iter
		self.b = np.array(b)
	...
	def conjugate_gradient(self, b, C=None):
		...
		k, eigenvectors, errors = 1, [x0], [1]
		while errors[-1] > tol and k <= N:
			# alpha = float((r0.T*r0)/(v0.T*A*v0))
			alpha = float((r0.T*Minv*r0)/(v0.T*A*v0))
			x1 = x0 + alpha*v0
			eigenvectors.append(x1)
			errors.append(norms(x1, x0).l_infinity())
			r1 = r0 - alpha*A*v0
			# s1 = float((r1.T*r1)/(r0t*r0))
			s1 = float((r1.T*Minv*r1)/(r0.T*Minv*r0))
			# v0 = r1 + s1*v0
