...
def deep_setattr(obj: Any, deep_key: str, value: Any):
  if '.' not in deep_key:
    setattr(obj, deep_key, value)
  else:
    key_parts = deep_key.split('.')
    deep_setattr(getattr(obj, key_parts[0]), '.'.join(key_parts[1:]), value)
