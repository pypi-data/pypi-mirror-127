...
"""
Stupidest fucking module on earth.
"""
...
TupValue = Union[str, List[str]]

def partition_tups(tups: List[Tuple[str, TupValue]]):
  """
  This is so far beyond stupid it needs undocumentation
  :param tups:
  :return:
  """
  keys = [t[0] for t in tups]
  repeating_keys = set([k for k in keys if keys.count(k) > 1])
  _equalities = [t for t in tups if t[0] not in repeating_keys]
  inclusions = [t for t in tups if t[0] in repeating_keys]
  _equalities_enum = list(enumerate(_equalities))

  rem_indices = []
  for i, equality in _equalities_enum:
    if isinstance(equality[1], list):
      for match_value in equality[1]:
        synthetic_tuple = (equality[0], match_value)
        inclusions.append(synthetic_tuple)
      rem_indices.append(i)

  equalities = [e for i, e in _equalities_enum if i not in rem_indices]

  return [equalities, inclusions]
