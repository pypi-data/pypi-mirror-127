...
class KatSvc(KatRes):
  ...
  def proxy_post(self, path: str, args: Dict, payload, port=None) -> Optional[str]:
    port = port if port else self.first_tcp_port_num()
    if port:
      try:
        return rest_backend.svc_proxy_post(
          name=f"{self.name}:{port}",
          namespace=self.ns,
          path=path,
          args=(args or {}),
  ...
  # def events(self):
  #   pod_events = [p.events() for p in self.pods()]
  #   return utils.flatten(pod_events)
