...
class ArrayManager(ArrayManagerBase):
    ...
    def atr_no_hl(self, n: int, array: bool = False) -> Union[float, np.ndarray]:
        """
    ...
    def natr_no_hl(self, n: int, array: bool = False) -> Union[float, np.ndarray]:
        """
        NATR.
        """
        result = talib.NATR(self.open, self.close, self.close, n)
        if array:
            return result
        return result[-1]
