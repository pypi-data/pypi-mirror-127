...
class NATRSignal(CtaSignal):
    ...
    def __init__(self, period: int = 30, abs_threshold: float = 1.0,
                 enable_close: bool = False, no_hl=True, interval: Interval = Interval.MINUTE, reverse_bs: int = 0,
                 filter_n_available=1, **kwargs):
        """"""
        self.interval = interval
        self.higher_boundary = abs_threshold
        self.lower_boundary = -abs_threshold
        self.enable_close = enable_close
        self.no_hl = no_hl
        self.reverse_bs = reverse_bs
        size = period * 2 + 1
    ...
    def on_window(self, bar: BarData):
        ...
        if self.no_hl:
            value = self.am.natr_no_hl(self.period)
        else:
            value = self.am.natr(self.period)
        ...
        if value < self.lower_boundary:
            ...
            if pos > 0 and (value > 0 if self.reverse_bs else value < 0):
                # 平 多仓
                self.set_signal_pos(0)
            elif pos < 0 and (value < 0 if self.reverse_bs else value > 0):
                # 平 空仓
                self.set_signal_pos(0)
