...
class StateMachine_data(object):
    ...
    def TidyStateSets(self, ss):
        ...
        if isinstance(ss, StateSet):
            ...
            try:
                iter(ss)
                warn(Warning("[Warning]: %s is an unusual iterable object<type: %s>. It may cause Exception in running. "%(str(ss), str(type(ss)))))
    ...
    def Config(self, start=None, end=None):
        ...
        if start is not None:
            # if not self.raw_ss_auto_fill_flag and start not in self.raw_ss:
            #     raise StateNotInSets(start)
            self.start = start
        if end is not None:
            # if not self.raw_ss_auto_fill_flag and end not in self.raw_ss:
