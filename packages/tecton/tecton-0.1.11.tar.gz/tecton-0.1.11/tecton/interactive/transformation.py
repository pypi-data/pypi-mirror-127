...
class Transformation(Fco):
    ...
    @sdk_public_method
    def run(
        self, *inputs: Union["pd.DataFrame", "pd.Series", "DataFrame"], context: BaseMaterializationContext = None
    ...
    def _sql_run(self, *inputs, context) -> data_frame.DataFrame:
        ...
        args = [create_temp_view(df, i) for i, df in enumerate(inputs)]
    ...
    def _pyspark_run(self, *inputs, context) -> data_frame.DataFrame:
        args = [tecton.interactive.data_frame.DataFrame._create(df).to_spark() for df in inputs]
        if context is not None:
            args.append(context)
