...
class BaseDataSource(Fco):
    ...
    def _get_raw_stream_preview_dataframe(self) -> DataFrame:
        """
        Helper function that allows start_raw_stream_preview() to be unit tested,
        since we can't easily unit test writing to temporary tables.
        """
        if not self.is_streaming:
            raise errors.DS_RAW_STREAM_PREVIEW_ON_NON_STREAM
        spark = TectonContext.get_instance()._spark
        return data_source_helper.start_raw_stream_preview(spark, self.stream_ds)

    @sdk_public_method
    def start_raw_stream_preview(self, table_name: str) -> StreamingQuery:
        """
        Starts a streaming job to write incoming raw, untranslated records
        from this data source's stream to a temporary table.
        ...
        This is a testing method, most commonly used to view the raw,
        untranslated records in a StreamDataSource. Note that the table will
        grow infinitely large, so this is only really useful for debugging in notebooks.

        The method ``start_stream_preview()`` provides similar functionality, but
        streams data that has had raw_stream_translator applied.

        :param table_name: The name of the temporary table that this method will write to.
        """
        df = self._get_raw_stream_preview_dataframe()
    ...
    def _get_stream_preview_dataframe(self) -> DataFrame:
        """
        Helper function that allows start_stream_preview() to be unit tested,
        since we can't easily unit test writing to temporary tables.
        """
        if not self.is_streaming:
    ...
    @sdk_public_method
    def get_raw_dataframe(self) -> TectonDataFrame:
        """
        Returns the raw, untranslated data from this data source as a Tecton DataFrame.

        The method ``get_dataframe()`` provides similar functionality, but
        returns the data after it has been translated by ``raw_batch_translator``.

        :return: Spark DataFrame containing the data source's raw, untranslated data.
        """
        spark = TectonContext.get_instance()._spark
        raw_df = data_source_helper.get_raw_dataframe(spark, self.batch_ds)
        return TectonDataFrame._create(raw_df)
