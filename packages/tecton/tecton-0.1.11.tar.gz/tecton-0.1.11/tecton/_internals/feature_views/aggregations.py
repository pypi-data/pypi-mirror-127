...
def construct_full_tafv_df(
    ...
    :param fd: The BWAFV/SWAFV object
    ...
    if not fd.is_temporal_aggregate:
        raise ValueError(f"Invalid invocation on non BWAFV/SWAFV type for: '{fd.name}'")
    ...
    if not spine_df:
        ...
        partial_aggregations_df = all_partial_aggregations_df.withColumn(
            ...
            + convert_timedelta_for_version(fd.get_tile_interval, fd.get_feature_store_format_version),
        )
    else:
        ...
        spine_df = spine_df.select(
            materialization_params.most_recent_anchor_time(functions.to_timestamp(fd.timestamp_key)).alias(
                TEMPORAL_ANCHOR_COLUMN_NAME
            ),
            *spine_join_keys,
        ).distinct()
