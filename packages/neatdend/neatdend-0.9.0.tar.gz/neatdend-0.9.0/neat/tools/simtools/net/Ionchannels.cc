...
double TestChannel::DfDvNewton(double v){
    ...
    double a01 = 1.0/(1.0 + 1.3498588075760032*exp(-0.01*v_a01));
    return -1. * (5*pow(a00, 3)*pow(a01, 3)*a02 + pow(a10, 2)*pow(a11, 2)*a12 - m_p_open_eq) + (5*pow(a00, 3)*pow(a01, 3) * da02_dv + 2*a10*pow(a11, 2)*a12 * da10_dv + 2*pow(a10, 2)*a11*a12 * da11_dv + 15*pow(a00, 2)*pow(a01, 3)*a02 * da00_dv + pow(a10, 2)*pow(a11, 2) * da12_dv + 15*pow(a00, 3)*pow(a01, 2)*a02 * da01_dv) * (m_e_rev - v);
}
...
double TestChannel2::DfDvNewton(double v){
    ...
    double a01 = 0.5;
    return -1. * (0.90000000000000002*pow(a00, 3)*pow(a01, 2) + 0.10000000000000001*pow(a10, 2)*a11 - m_p_open_eq) + (2.7000000000000002*pow(a00, 2)*pow(a01, 2) * da00_dv + 0.20000000000000001*a10*a11 * da10_dv + 0.10000000000000001*pow(a10, 2) * da11_dv + 1.8*pow(a00, 3)*a01 * da01_dv) * (m_e_rev - v);
}
...
double Na_Ta::DfDvNewton(double v){
    ...
    double m = 0.182*(v_m + 38.0)/((1.0 - 0.0017761035457343791*exp(-0.16666666666666666*v_m))*(-0.124*(v_m + 38.0)/(1.0 - 563.03023683595109*exp(0.16666666666666666*v_m)) + 0.182*(v_m + 38.0)/(1.0 - 0.0017761035457343791*exp(-0.16666666666666666*v_m))));
    return -1. * (h*pow(m, 3) - m_p_open_eq) + (pow(m, 3) * dh_dv + 3*h*pow(m, 2) * dm_dv) * (m_e_rev - v);
}
...
double h::DfDvNewton(double v){
    ...
    double hf = 1.0/(122306.53009058574*exp(0.14285714285714285*v_hf) + 1.0);
    return -1. * (0.80000000000000004*hf + 0.20000000000000001*hs - m_p_open_eq) + (0.20000000000000001 * dhs_dv + 0.80000000000000004 * dhf_dv) * (m_e_rev - v);
}
