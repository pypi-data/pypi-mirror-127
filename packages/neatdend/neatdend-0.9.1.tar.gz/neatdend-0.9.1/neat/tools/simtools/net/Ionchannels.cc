...
double TestChannel::DfDvNewton(double v){
    ...
    double a01 = 1.0/(exp((30.0 - v_a01)/100.0) + 1.0);
    return -1. * (5*pow(a00, 3)*pow(a01, 3)*a02 + pow(a10, 2)*pow(a11, 2)*a12 - m_p_open_eq) + (2*pow(a10, 2)*a11*a12 * da11_dv + 15*pow(a00, 2)*pow(a01, 3)*a02 * da00_dv + 2*a10*pow(a11, 2)*a12 * da10_dv + 5*pow(a00, 3)*pow(a01, 3) * da02_dv + pow(a10, 2)*pow(a11, 2) * da12_dv + 15*pow(a00, 3)*pow(a01, 2)*a02 * da01_dv) * (m_e_rev - v);
}
