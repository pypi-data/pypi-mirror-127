...
class EthicFileViewSet(RevisionMixin, mixins.CreateModelMixin, mixins.DestroyModelMixin,
    ...
    def perform_create(self, serializer):
        ethic_file = serializer.save(ethic=self.ethic, creator=self.request.user)
        set_comment(f'Created ethic file <ID-{ethic_file.id}>')

    def perform_destroy(self, ethic_file):
        ethic_file.delete()
        set_comment(f'Deleted ethic file <ID-{ethic_file.id}>')

    def perform_update(self, serializer):
        ethic_file = serializer.save()
        set_comment(f'Updated ethic file <ID-{ethic_file.id}>')
