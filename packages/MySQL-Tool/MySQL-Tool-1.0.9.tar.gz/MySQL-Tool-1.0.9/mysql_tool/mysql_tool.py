...
class my_mysql():
    ...
    def __init__(self, host, user, database, password, port=3306, unix_socket='', default_character_set="utf-8"):
        self.host = host
        self.user = user
        ...
        try:
            ...
            if self.unix_socket != '':
                return self.driver.connect(host=self.host, port=self.port, user=self.user, passwd=self.password,
                                           db=self.database, charset='utf8', unix_socket=self.unix_socket)
            else:
                return self.driver.connect(host=self.host, port=self.port, user=self.user, passwd=self.password,
