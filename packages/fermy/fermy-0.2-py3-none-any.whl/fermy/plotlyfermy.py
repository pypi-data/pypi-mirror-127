...
v0.7 bug fix for mroe than two bioreactors dataset multiyplot grouped
...
def multiyplot(data: pd.DataFrame, path:str="", yspace:float=0.05, sizep:int=16, groupby:Union[None,str]=None):
...
def ccplot(data: pd.DataFrame, variableofinterest: str, path:str="", dicorename:Dict={}, forcepower:bool=False, addccviol: bool=False) -> pd.DataFrame:
    """
    ...
    dicorename: allow users to custom names of axis with:
    {"old name" : "desired name", "old name2" : "desired name2"}
    
    forcepower: allow users to force scientific notation 10^n
    for y axis and controls limits
    
    addccviol: allow users to add a new column to original data
    with CC rules check-in
