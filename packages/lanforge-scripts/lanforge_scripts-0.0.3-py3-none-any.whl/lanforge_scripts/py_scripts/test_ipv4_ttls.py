...
test_ip_variable_time = importlib.import_module(
    "py_scripts.test_ip_variable_time")
...
class TTLSTest(LFCliBase):
    ...
    def build(self, extra_securities=[]):
        ...
        if self.ieee80211w:
            self.station_profile.set_command_param(
                "add_sta", "ieee80211w", self.ieee80211w)
        if self.enable_pkc:
    ...
    def start(self, sta_list, print_pass, print_fail, wait_time=40):
        ...
        for sec in range(self.timeout):
            for sta_name in sta_list:
                sta_status = self.json_get(
                    "port/1/1/" + sta_name + "?fields=port,alias,ip,ap", debug_=self.debug)
                # print(sta_status)
        ...
        if (len(sta_list) == len(ip_map)) and (len(sta_list) == len(associated_map)):
            ...
            self._fail(
                "FAIL: Not all stations able to associate/get IP", print_fail)
            if self.debug:
    ...
    def stop(self):
        ...
        self.collect_endp_stats(
            self.l3_cx_obj_tcp.cx_profile.created_cx, traffic_type="TCP")
        self.collect_endp_stats(
            self.l3_cx_obj_udp.cx_profile.created_cx, traffic_type="UDP")
    ...
    def compare_vals(self, name, postVal, print_pass=True, print_fail=True):
        ...
        if postVal > 0:
            ...
            self._fail("%s did not report traffic: %s" %
                       (name, postVal), print_fail)
...
''')
    ...
    if optional is not None:
        optional.add_argument(
            '--a_min', help='--a_min bps rate minimum for side_a', default=256000)
        optional.add_argument(
            '--b_min', help='--b_min bps rate minimum for side_b', default=256000)
        optional.add_argument(
            '--test_duration', help='--test_duration sets the duration of the test', default="5m")
        optional.add_argument('--key-mgmt', help="--key-mgt: { %s }" % ", ".join(realm.wpa_ent_list()),
                              default="WPA-EAP")
        optional.add_argument(
            '--wpa_psk', help='wpa-ent pre shared key', default="[BLANK]")
        optional.add_argument(
            '--eap', help='--eap eap method to use', default="TTLS")
        optional.add_argument(
            '--identity', help='--identity eap identity string', default="testuser")
        optional.add_argument(
            '--ttls_passwd', help='--ttls_passwd eap password string', default="testpasswd")
        optional.add_argument('--ttls_realm', help='--ttls_realm 802.11u home realm to use',
                              default="localhost.localdomain")
        optional.add_argument(
            '--domain', help='--domain 802.11 domain to use', default="localhost.localdomain")
        optional.add_argument('--hessid', help='--hessid 802.11u HESSID (MAC addr format/peer for WDS)',
                              default="00:00:00:00:00:01")
        optional.add_argument(
            '--ieee80211w', help='--ieee80211w <disabled(0),optional(1),required(2)', default='1')
        optional.add_argument(
            '--use_hs20', help='use HotSpot 2.0', default=False)
        optional.add_argument(
            '--enable_pkc', help='enable opportunistic PMKSA WPA2 key caching', default=False)
    ...
    station_list = LFUtils.portNameSeries(
        prefix_="sta", start_id_=0, end_id_=num_sta - 1, padding_number_=10000)
