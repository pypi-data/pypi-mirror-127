...
class TestIPV4PS(LFCliBase):
    def __init__(self, ssid, security, password, sta_list, name_prefix, upstream, radio, radio2,
    ...
    def build(self):
        ...
        self.vap_profile.create(resource=1, radio="wiphy1", channel=161, up_=True, debug=False,
                                suppress_related_commands_=True, ip_address=self.ip_address)
...
--debug
    ...
    if optional is not None:
        ...
        optional.add_argument('--radio2', help='second radio used in this test', default='wiphy2')
        optional.add_argument('--vap_ip', help='set the ip address for the VAP which will be created',
    ...
    ipv4ps = TestIPV4PS(host=args.mgr, port=args.mgr_port,
                        number_template="00",
                        ...
                        side_a_min_rate=args.a_min, side_b_min_rate=args.b_min,
                        ip_address=args.vap_ip,
                        _debug_on=args.debug)

    ipv4ps.pre_cleanup()
    ipv4ps.build()
    if not ipv4ps.passes():
        print(ip_var_test.get_fail_message())
        exit(1)
    ipv4ps.start(False, False)
    ipv4ps.stop()
    if not ipv4ps.passes():
        print(ip_var_test.get_fail_message())
        exit(1)
