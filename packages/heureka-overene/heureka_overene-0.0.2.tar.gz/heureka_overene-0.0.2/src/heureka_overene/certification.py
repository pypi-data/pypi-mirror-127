...
class ShopCertification:
	__slots__ = ('_api_key', '_service', '_sended', '_products_ids', '_email', '_order_id', 'response')
	...
	def order_id(self, order_id: int):
		if not str(order_id).isnumeric():
			raise TypeError("Order have to be integer")
		self._order_id = int(order_id)

	def log_order(self) -> bool:
		if self._sended:
			raise RuntimeError('Order can be send just one time')
		self._sended = True
		order_data = {
