...
def score_distribution_plot(dfs:Dict[str, pd.DataFrame], hist_options:Dict[str, Dict], 
                            ...
                            analysis_label_styles:Optional[Dict]=None):
    styles = parse_styles(plot_styles)
    ax = single_frame(styles=styles)
    ...
    if analysis_label_styles is not None:
        draw_atlas_label(ax, dy=0.05, text_options=styles['text'], **analysis_label_styles)
    return ax