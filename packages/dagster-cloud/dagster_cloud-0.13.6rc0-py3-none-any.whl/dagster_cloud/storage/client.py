...
class GqlShimClient(AbstractContextManager):
    ...
    def __init__(
        ...
        try:
            self._session = self._exit_stack.enter_context(
                create_agent_requests_session(self.retries)
            )
    ...
    def execute(self, query: str, variable_values: Dict[str, Any] = None):
        ...
        try:
            response = self._session.request("POST", self.url, **post_args)
            try:
...
@contextmanager
def create_agent_requests_session(retries: int):
...
def create_proxy_client(url: str, config_value: Dict[str, Any]):
