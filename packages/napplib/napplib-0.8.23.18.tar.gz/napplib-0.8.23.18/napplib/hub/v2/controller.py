...
from .models.channel	import SellerChannelUpdateQuantity
...
class HubController:
	...
	@AttemptRequests(success_codes=[200])
	def get_seller_channel_type(self, external_seller_id: str, external_channel_id: str, channel_type: ChannelType) -> requests.Response:
	...
	def put_channel_sku_status(self, channel_sku_id: str, channel_status: ChannelStatus) -> requests.Response:
	...
	@AttemptRequests(success_codes=[200])
	def put_skus(self, skus: List[HubSku], seller_id: int) -> requests.Response:
	...
	@AttemptRequests(success_codes=[200])
	def put_skus_prices(self, seller_id: str, skus_prices: List[HubSkuPrice]) -> requests.Response:
		return requests.put(f'{self.__get_endpoint_base()}/sellers/{seller_id}/skus/prices', headers=self.headers, data=unpack_payload_dict(skus_prices,remove_null=True))
	
	@AttemptRequests(success_codes=[200])
	def put_skus_stocks(self, seller_id: str, skus_stocks: List[HubSkuStock]) -> requests.Response:
		return requests.put(f'{self.__get_endpoint_base()}/sellers/{seller_id}/skus/stocks', headers=self.headers, data=unpack_payload_dict(skus_stocks,remove_null=True))

	@AttemptRequests(success_codes=[200])
	def get_sellers_by_channel_type_and_plataform(self, channel_type: ChannelType, plataform: str) -> requests.Response:
	...
	@AttemptRequests(success_codes=[204])
	def put_seller_by_channel_sync_last_catalog(self, seller_channel_id: str, seller_channel_update_quantity: SellerChannelUpdateQuantity) -> requests.Response:
	...
	@AttemptRequests(success_codes=[204])
	def put_seller_by_channel_sync_last_price(self, seller_channel_id: str, seller_channel_update_quantity: SellerChannelUpdateQuantity) -> requests.Response:
	...
	@AttemptRequests(success_codes=[204])
	def put_seller_by_channel_sync_last_stock(self, seller_channel_id: str, seller_channel_update_quantity: SellerChannelUpdateQuantity) -> requests.Response:
