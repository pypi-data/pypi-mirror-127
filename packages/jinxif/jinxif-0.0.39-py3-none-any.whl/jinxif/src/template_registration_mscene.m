...
m_regex_round_ref = 'peek_s_regex_round_ref';  % dapi ref staining round regexp pattern
m_regex_round_nonref = 'peek_s_regex_round_nonref';  % staining round regexp pattern (capturing rounds other then ref round is crucial. capturing ref round too is ok.)
% input parameter marker
m_regex_marker_ref = 'peek_s_regex_marker_ref';  % ref round stain regex pattern
m_regex_marker_nonref = 'peek_s_regex_marker_nonref';  % stain regex pattern (capturing stains other then ref round is crucial. capturing ref round stains too is ok.)
% input parameter micchannel
m_regex_micchannel_ref = 'peek_s_regex_micchannel_ref';  % ref round microscopy channel regex pattern
m_regex_micchannel_nonref = 'peek_s_regex_micchannel_nonref';  % microscopy channel regex pattern (capturing microscopy channels other then ref round is crucial. capturing ref round microscopy channel too is ok.)
...
m_glob_img_dapinonref = 'peek_s_glob_img_dapinonref';  % slide_mscene specific dapi channel image glob pattern (capturing dapi channels other then ref round is crucial. capturing ref round dapi channel too is ok.)
...
elseif length(m_filedapi_ref) > 1
   sprintf(strcat('Error: more then 1 DAPI reference round file found with glob:', m_src_dir, m_glob_img_dapiref))
   %exit
end
...
for i = 1:length(m_filedapi_nonref)
    ...
    if not(strcmp(m_filedapi_nonref(i).name, m_filedapi_ref.name))
        ...
        m_point_nonref = detectSURFFeatures(m_dapi_nonref);  % detect features of non-reference DAPI (alternative methode detectKAZEFeatures)
        ...
        % for each file form this round, dapi and non-dapi
