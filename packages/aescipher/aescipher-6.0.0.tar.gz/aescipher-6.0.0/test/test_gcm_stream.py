...
def get_cipher(noHASH=False, woIV=False):
    if noHASH:
        if woIV:
            return AESCipherGCMSTREAMnoHASHwoIV(key=key, iv=iv, assoc_data=code)
        else:
            return AESCipherGCMSTREAMnoHASH(key=key, assoc_data=code)
    else:
        if woIV:
            return AESCipherGCMSTREAMwoIV(key=key, iv=iv, assoc_data=code)
        else:
            return AESCipherGCMSTREAM(key=key, assoc_data=code)
