...
def get_cipher(noHASH=False, noIV=False, initial_value=1):
    if noHASH:
        if noIV:
            return AESCipherCTRnoHASHnoIV(key=key)
        else:
            return AESCipherCTRnoHASH(key=key, iv=iv, initial_value=initial_value)
    else:
        if noIV:
            return AESCipherCTRnoIV(key=key)
        else:
            return AESCipherCTR(key=key, iv=iv, initial_value=initial_value)
