...
def get_cipher(noHASH=False, noIV=False, initial_value=1, sequentialize_cv=False):
    if noHASH:
        if noIV:
            return AESCipherCTRnoHASHnoIV(key, sequentialize_cv=sequentialize_cv)
        else:
            return AESCipherCTRnoHASH(key, iv, initial_value, sequentialize_cv=sequentialize_cv)
    else:
        if noIV:
            return AESCipherCTRnoIV(key, sequentialize_cv=sequentialize_cv)
        else:
            return AESCipherCTR(key, iv, initial_value, sequentialize_cv=sequentialize_cv)
...
print(plaintext2 == cipher.decrypt(ciphertext2))
cipher = get_cipher(sequentialize_cv=True)
