...
from disca.util.functional import one_or_many, chunks
from disca.types.user import User
from disca.types.base import SlottedModel, Field, AutoDictField, snowflake, enum, text, cached_property
from disca.types.permissions import Permissions, Permissible, PermissionValue
...
class PermissionOverwrite(ChannelSubType):
    ...
    id : snowflake
        The overwrite ID.
    type : :const:`disca.types.channel.PermissionsOverwriteType`
        The overwrite type.
    allow : :class:`disca.types.permissions.PermissionValue`
        All allowed permissions.
    deny : :class:`disca.types.permissions.PermissionValue`
    ...
    def create_for_channel(cls, channel, entity, allow=0, deny=0):
        from disca.types.guild import Role
...
class Channel(SlottedModel, Permissible):
    ...
    recipients : list(:class:`disca.types.user.User`)
        Members of this channel (if this is a DM channel).
    type : :const:`ChannelType`
        The type of this channel.
    overwrites : dict(snowflake, :class:`disca.types.channel.PermissionOverwrite`)
    ...
    def create_invite(self, *args, **kwargs):
        ...
        from disca.types.invite import Invite
...
class MessageIterator(object):
    ...
    client : :class:`disca.client.Client`
        The disca client instance to use when making requests.
    channel : `Channel`
    ...
    def fill(self):
        """
        Fills the internal buffer up with :class:`disca.types.message.Message` objects from the API.
