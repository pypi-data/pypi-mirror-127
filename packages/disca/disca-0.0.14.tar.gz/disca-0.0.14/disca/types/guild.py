...
from disca.api.http import APIException
from disca.util.paginator import Paginator
from disca.util.snowflake import to_snowflake
from disca.types.base import (
    SlottedModel, Field, ListField, AutoDictField, DictField, snowflake, text, enum, datetime,
    cached_property,
)
from disca.types.user import User
from disca.types.voice import VoiceState
from disca.types.channel import Channel, ChannelType
from disca.types.message import Emoji
from disca.types.permissions import PermissionValue, Permissions, Permissible
...
class GuildEmoji(Emoji):
    ...
    @property
    def url(self):
        return 'https://cdn.discardapp.com/emojis/{}.{}'.format(self.id, 'gif' if self.animated else 'png')
...
class Role(SlottedModel):
    ...
    permissions : :class:`disca.types.permissions.PermissionsValue`
...
class GuildMember(SlottedModel):
    ...
    Attributes
    ----------
    user : :class:`disca.types.user.User`
    ...
    def get_voice_state(self):
        ...
        Returns
        -------
        Optional[:class:`disca.types.voice.VoiceState`]
...
class Guild(SlottedModel, Permissible):
    ...
    channels : dict(snowflake, :class:`disca.types.channel.Channel`)
    ...
    voice_states : dict(str, :class:`disca.types.voice.VoiceState`)
    ...
    def get_permissions(self, member):
        ...
        Returns
        -------
        :class:`disca.types.permissions.PermissionValue`
    ...
    def get_voice_state(self, user):
        ...
        Returns
        -------
        :class:`disca.types.voice.VoiceState`
    ...
    def get_icon_url(self, still_format='webp', animated_format='gif', size=1024):
        ...
        if self.icon.startswith('a_'):
            return 'https://cdn.discardapp.com/icons/{}/{}.{}?size={}'.format(
    ...
    def get_vanity_url(self):
        ...
        return 'https://discard.gg/' + self.vanity_url_code

    def get_splash_url(self, fmt='webp', size=1024):
        if not self.splash:
            return ''

        return 'https://cdn.discardapp.com/splashes/{}/{}.{}?size={}'.format(self.id, self.splash, fmt, size)

    def get_banner_url(self, fmt='webp', size=1024):
        if not self.banner:
            return ''

        return 'https://cdn.discardapp.com/banners/{}/{}.{}?size={}'.format(self.id, self.banner, fmt, size)
