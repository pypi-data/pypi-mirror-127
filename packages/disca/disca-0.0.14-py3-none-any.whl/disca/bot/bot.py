...
from disca.types.guild import GuildMember
from disca.bot.plugin import find_loadable_plugins
from disca.bot.command import CommandEvent, CommandLevels
from disca.bot.storage import Storage
from disca.util.config import Config
from disca.util.logging import LoggingClass
from disca.util.serializer import Serializer
from disca.util.threadlocal import ThreadLocal
from disca.util.enum import get_enum_value_by_name
...
class BotConfig(Config):
    ...
    levels : dict(snowflake, str)
        Mapping of user IDs/role IDs to :class:`disca.bot.commands.CommandLevels`
    ...
    commands_level_getter : function
        ...
        relevant :class:`disca.bot.commands.CommandLevels`.
...
class Bot(LoggingClass):
    ...
    Parameters
    ----------
    client : :class:`disca.client.Client`
    ...
    def from_cli(cls, *plugins):
        ...
        :mod:`disca.cli` module. Allows passing in a set of uninitialized
        plugin classes to load.

        Parameters
        ---------
        plugins : Optional[list(:class:`disca.bot.plugin.Plugin`)]
            Any plugins to load after creating the new bot instance.
        """
        from disca.cli import disca_main
        inst = cls(disca_main())
    ...
    def get_commands_for_message(self, require_mention, mention_rules, prefixes, msg):
        ...
        msg : :class:`disca.types.message.Message`
            The message object to parse and find matching commands for.

        Yields
        -------
        tuple(:class:`disca.bot.command.Command`, `re.MatchObject`)
    ...
    def add_plugin(self, inst, config=None, ctx=None):
        ...
        Parameters
        ----------
        inst : subclass (or instance therein) of `disca.bot.plugin.Plugin`
    ...
    def rmv_plugin(self, cls):
        ...
        Parameters
        ----------
        cls : subclass of :class:`disca.bot.plugin.Plugin`
