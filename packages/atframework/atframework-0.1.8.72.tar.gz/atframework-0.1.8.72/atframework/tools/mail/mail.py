...
class Mail:
    ...
    def send_html_with_attachment(self, emails, mail_content, attachment_path, testing_type, testing_type_details, site_link):
        ssl_context = ssl.create_default_context()
        service = smtplib.SMTP_SSL(self.smtp_server_domain_name, self.port, context=ssl_context)
        service.login(self.sender_mail, self.password)
        ...
        for email in emails:
            ...
            mail['From'] = self.sender_mail
            mail['To'] = email
            mail_content_new = self.html_template.format(testing_name=testing_type,name=email.split("@")[0],
                                                         testing_type_details=testing_type_details, site_link=site_link) + mail_content
            # logger.info("[AtLog] ----- email here ----- " + mail_content_new)
            html_content = MIMEText(mail_content_new, 'html')
            mail.attach(html_content)
