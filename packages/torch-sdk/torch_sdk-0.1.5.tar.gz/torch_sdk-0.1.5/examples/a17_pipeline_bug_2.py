...
def print_execution_score():
    torch_client = create_torch_client()
    execution = torch_client.get_reconciliation_rule_result(584)
    print(execution)
    status = execution.result.status
    ruleExecutionType = execution.result.ruleExecutionType
    description = execution.result.description
    failedRows = execution.result.failedRows
    failureCount = execution.result.failureCount
    qualityScore = execution.result.qualityScore
    row = execution.result.rows
    successCount = execution.result.successCount

    execution = torch_client.get_dq_rule_result(532)
    print(execution)
    status = execution.result.status
    ruleExecutionType = execution.result.ruleExecutionType
    description = execution.result.description
