...
def geojson_to_yaml_obj(gjson: str, name: str = None):
    loaded = json.loads(gjson)
    if not name:
        if 'name' in loaded:
            name = loaded['name']
...
def geojson_to_yaml(gjson: str, name: str = None):
    return yaml.dump(geojson_to_yaml_obj(gjson, name))


def yaml_to_geojsons_obj(yaml_str: str):
