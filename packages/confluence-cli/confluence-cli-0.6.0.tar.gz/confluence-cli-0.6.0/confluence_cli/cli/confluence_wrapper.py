    ...
    def __init__(self, params: Dict[str, Any]):
        ...
        if not self.__dict__ or params:
            ...
            if token and user and password:
                logger.warning("token param was provided along with user/password data. Token takes precedence")
            if token:
                user = password = None
            super().__init__(
                url=params["baseURL"],
        ...
        if cql_result:
            for result in cql_result.get("results"):
                ...
                if content:  # Non pages (blogs, comments) results do not have "content"
                    ...
                    logger.warning(f"Result with no content: firt key {list(result.keys())[0]}")
        return content_list
