...
class Distance(object):
    ...
    def dtw(self, a, b):
        ...
        def single_dtw(a, b):
            return np.array([dtw_ndim.warping_paths(n, a)[0] for n in b])

        return np.array([single_dtw(a,b[:, i, :]) for i in range(b.shape[1])]).T



def main():
    from scipy.spatial import distance
    b = np.array([[1, 2, 43],[2, 2, 42]])
    a = np.array([3, 2, 1, 42])

    d = Distance()
    print('-----------------------------------------------------------------')

    print('My       braycurtis: {}'.format(d.get_distance(a, b, "dtw")))
