...
#                names = ["Feature_"+ f for f in self.som.features_names[0]]
    ...
    def plot_cluster_map(self, data=None, n_clusters=None, anotate=False, onlyzeros=False, labelsize=7, cmap = plt.get_cmap('Pastel1')):
        org_w = self.width
        org_h = self.height
        ...
        if n_clusters is None:
            clusters = self.som.cluster_data(array=data)
        if data is None and n_clusters is not None:
