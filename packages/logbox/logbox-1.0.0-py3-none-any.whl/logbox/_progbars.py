...
class ProgbarKnownLength(ProgbarBase):
    def __init__(self,
                 ...
                 str_info=DEFAULT_STR_INFO, n_decimals=3,
                 leading_line='oneliner'):
        super().__init__(callback_on_update=callback_on_update, str_info=str_info, n_decimals=n_decimals, leading_line=leading_line)
    ...
    def _parse_progress(self, i, is_intermediate=True):
        ...
        str_progress = ' '.join([str_progbar, self.str_info, str_counter])
        return str_progress
...
class ProgbarUnknownLength(ProgbarBase):
    ...
    def _parse_progress(self, i, is_intermediate=True):
        str_counter = self._parse_counter(i)
        str_progress = ' '.join([self.str_info, str_counter])
        return str_progress
