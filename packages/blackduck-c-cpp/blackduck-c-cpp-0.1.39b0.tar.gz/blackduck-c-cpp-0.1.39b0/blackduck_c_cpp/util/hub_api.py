...
class HubAPI:
    ...
    def authenticate(self):
        try:
            self.hub = HubInstance(self.bd_url, api_token=self.api_token, insecure=self.insecure, write_config_flag=False)
        except KeyError:
            util.error_and_exit("Make sure you have the right API key and --insecure flag is set correctly")
        except ConnectionError:
            util.error_and_exit("ConnectionError occurred. Make sure you have the correct url and insecure flag is set correctly")
    ...
    def create_or_verify_project_version_exists(self, project_name, version_name):
        ...
        if not response.ok:
            util.error_and_exit("Error creating project version -- (Response({}): {})".format(response.status_code, response.text))
