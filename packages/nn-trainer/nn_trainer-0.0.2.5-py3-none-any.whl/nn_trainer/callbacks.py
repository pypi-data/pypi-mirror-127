...
class DataPlotterCallback(Callback):
    ...
    def __post_init__(self):
        ...
        self.previous_batch = 0
        for sample_index in range(self.data_set_sample_count):
            if self.shuffle:
                sample_index = random.randint(0, len(self.data_set))
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        inputs_real = self.trainer.to_tensor(inputs_real, dtype=dtype)
        outputs_real = self.trainer.to_tensor(outputs_real, dtype=dtype)
        
        outputs_fake = self.trainer.network(inputs_real)

        inputs_real = self.trainer.to_numpy(inputs_real)
        outputs_real = self.trainer.to_numpy(outputs_real)
        outputs_fake = self.trainer.to_numpy(outputs_fake)
