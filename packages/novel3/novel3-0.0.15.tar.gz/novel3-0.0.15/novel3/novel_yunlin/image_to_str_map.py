...
class ImageToStr:
    ...
    def get_illegal_list(self) -> None:
        with open('%s/../json/illegal.json' % abspath(__file__), 'r', encoding='utf-8') as fp:
            self.illegal_list = json.load(fp)
            fp.close()

    def add_illegal_str(self, illegal_str) -> bool:
        self.get_illegal_list()
        illegal_str not in self.illegal_list and self.illegal_list.append(illegal_str)
        # print(self.illegal_list)
        self.save_illegal_str()

    def save_illegal_str(self):
        with open('%s/../json/illegal.json' % abspath(__file__), 'w', encoding='utf-8') as fp:
            json.dump(self.illegal_list, fp, indent=4, ensure_ascii=False)
            fp.close()
