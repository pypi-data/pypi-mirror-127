...
class Invert:
	...
	def set(self, invert: bool, output=repcap.Output.Default) -> None:
		...
		Turns inversion of the marker signal on and off. \n
			:param invert: 0| 1| OFF| ON 1 | ON Marker is on a falling edge. 0 | OFF Marker is on a rising edge.
	...
	def get(self, output=repcap.Output.Default) -> bool:
		...
		Turns inversion of the marker signal on and off. \n
			:param output: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
			:return: invert: 0| 1| OFF| ON 1 | ON Marker is on a falling edge. 0 | OFF Marker is on a rising edge."""
		output_cmd_val = self._cmd_group.get_repcap_cmd_value(output, repcap.Output)
