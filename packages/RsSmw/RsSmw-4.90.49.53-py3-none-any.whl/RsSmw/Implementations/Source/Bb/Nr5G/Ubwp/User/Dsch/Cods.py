...
class Cods:
	"""Cods commands group definition. 1 total commands, 0 Sub-groups, 1 group commands"""
	...
	def set(self, cot_duration_size: int, userNull=repcap.UserNull.Default) -> None:
		...
		No command help available \n
			:param cot_duration_size: No help available
	...
	def get(self, userNull=repcap.UserNull.Default) -> int:
		...
		No command help available \n
			:param userNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'User')
			:return: cot_duration_size: No help available"""
		userNull_cmd_val = self._cmd_group.get_repcap_cmd_value(userNull, repcap.UserNull)
