...
class Rarb:
	...
	def set(self, state: bool, mobileStation=repcap.MobileStation.Default) -> None:
		...
		Enables/disables repeating the selected PCPCH structure during one ARB sequence. \n
			:param state: 0| 1| OFF| ON ON Within one ARB sequence, the selected PCPCH structure is repeated once. OFF The selected PCPCH structure can be repeated several time, depending on the structure length ([:SOURcehw]:BB:W3GPp:MSTationst:PRACh:TIMing:SPERiod?) and the [:SOURcehw]:BB:W3GPp:MSTationst:PCPCh:RAFTer.
	...
	def get(self, mobileStation=repcap.MobileStation.Default) -> bool:
		...
		Enables/disables repeating the selected PCPCH structure during one ARB sequence. \n
			:param mobileStation: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mstation')
			:return: state: 0| 1| OFF| ON ON Within one ARB sequence, the selected PCPCH structure is repeated once. OFF The selected PCPCH structure can be repeated several time, depending on the structure length ([:SOURcehw]:BB:W3GPp:MSTationst:PRACh:TIMing:SPERiod?) and the [:SOURcehw]:BB:W3GPp:MSTationst:PCPCh:RAFTer."""
		mobileStation_cmd_val = self._cmd_group.get_repcap_cmd_value(mobileStation, repcap.MobileStation)
