...
class Frequency:
	"""Frequency commands group definition. 36 total commands, 6 Sub-groups, 9 group commands"""
	...
	def get_mode(self) -> enums.FreqMode:
		...
		further frequency settings. \n
			:return: mode: CW| FIXed | SWEep| LIST | COMBined CW|FIXed Sets the fixed frequency mode. CW and FIXed are synonyms. The instrument operates at a defined frequency, set with command [:​SOURcehw]:​FREQuency[:​CW|FIXed]. SWEep Sets sweep mode. The instrument processes frequency (and level) settings in defined sweep steps. Set the range and current frequency with the commands: FREQuency:STARt and FREQuency:STOP, FREQuency:CENTer, FREQuency:SPAN, FREQuency:MANual LIST Sets list mode. The instrument processes frequency and level settings by means of values loaded from a list. To configure list mode settings, use the commands of the 'SOURce:LIST Subsystem'. COMBined Sets the combined RF frequency / level sweep mode. The instrument processes frequency and level settings in defined sweep steps. Set the range and current frequency with the commands: COMBined:FREQuency:STARt and COMBined:FREQuency:STOP, COMBined:POWer:STARt and COMBined:POWer:STOP
		"""
