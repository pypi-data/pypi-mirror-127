...
def guess_iParapheur_version(basedir):
    ...
    module_fp = io.BytesIO(str.encode(module_string))
    leMdodule = ConfigParser.RawConfigParser()
    leMdodule.read_file(module_fp)
...
with open(ALF_CONFIG_PATH, 'r') as f:
    config_string = '[Parapheur]\n' + f.read()
config_fp = io.BytesIO(str.encode(config_string))
config = ConfigParser.RawConfigParser()
...
ihmconfig_fp = io.BytesIO(str.encode(ihmconfig_string))
ihmconfig = ConfigParser.RawConfigParser()
ihmconfig.read_file(ihmconfig_fp)
