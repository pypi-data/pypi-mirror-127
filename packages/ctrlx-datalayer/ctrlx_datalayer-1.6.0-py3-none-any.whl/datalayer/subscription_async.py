...
from datalayer.clib import C_DLR_RESULT, userData_c_void_p
from datalayer.clib_client import (C_DLR_CLIENT_NOTIFY_RESPONSE,
...
class SubscriptionAsync(datalayer.subscription.Subscription):
    """
    SubscriptionAsync
    """
    ...
    def __init__(self, client: datalayer.client.Client):
        """
        @param[in]  client    Reference to the client
        """
        self.__ptr_notify: datalayer.client._CallbackPtr
        self.__ptr_resp: datalayer.client._CallbackPtr
        self.__closed = False
    ...
    def __create_sub_notify_callback(self, cb: datalayer.subscription.ResponseNotifyCallback):
        """
        callback management
        """
        cb_ptr = datalayer.client._CallbackPtr()
        ...
        def _cb(status: C_DLR_RESULT, items: ctypes.POINTER(C_NotifyItem), count: ctypes.c_uint32, userdata: ctypes.c_void_p):
            ...
            if r == Result.OK:
                ...
                for x in range(0, count):
                    n = datalayer.subscription.NotifyItem(
    ...
    def _test_notify_callback(self, cb: datalayer.subscription.ResponseNotifyCallback):
        """
            internal use
        """
        return self.__create_sub_notify_callback(cb)

    def __create_response_callback(self, cb: datalayer.client.ResponseCallback):
        """
        callback management
        """
        self.__on_cb = False
    ...
    def _test_response_callback(self, cb: datalayer.client.ResponseCallback):
        """
            internal use
        """
        return self.__create_response_callback(cb)

    def _create(self, prop: Variant, cnb: datalayer.subscription.ResponseNotifyCallback, cb: datalayer.client.ResponseCallback, userdata: userData_c_void_p = None) -> Result:
        """
        ...
        """
        r = Result(datalayer.clib.libcomm_datalayer.DLR_clientCreateSubscriptionAsync(
        ...
        if r == Result.OK:
            self.__id = datalayer.subscription.get_id(prop)
        return r

    def subscribe(self, address: str, cb: datalayer.client.ResponseCallback, userdata: userData_c_void_p = None) -> Result:
        """
        ...
        return Result(datalayer.clib.libcomm_datalayer.DLR_clientSubscribeAsync(
            self.__client().get_handle(),
            b_id,
            b_address,
            self.__create_response_callback(cb),
    ...
    def unsubscribe(self, address: str, cb: datalayer.client.ResponseCallback, userdata: userData_c_void_p = None) -> Result:
        """
        ...
        b_id = self.id().encode('utf-8')
        b_address = address.encode('utf-8')
        return Result(datalayer.clib.libcomm_datalayer.DLR_clientUnsubscribeAsync(
            self.__client().get_handle(),
            b_id,
            b_address,
            self.__create_response_callback(cb),
    ...
    def subscribe_multi(self, address: typing.List[str], cb: datalayer.client.ResponseCallback, userdata: userData_c_void_p = None) -> Result:
        """
        ...
        return Result(datalayer.clib.libcomm_datalayer.DLR_clientSubscribeMultiAsync(
            self.__client().get_handle(),
            b_id,
            b_address,
            len(address),
    ...
    def unsubscribe_multi(self, address: typing.List[str], cb: datalayer.client.ResponseCallback, userdata: userData_c_void_p = None) -> Result:
        """
        ...
        b_address = (ctypes.c_char_p * len(address))(*
                                                     [d.encode('utf-8') for d in address])
        return Result(datalayer.clib.libcomm_datalayer.DLR_clientUnsubscribeMultiAsync(
            self.__client().get_handle(),
            b_id,
            b_address,
            len(address),
    ...
    def unsubscribe_all(self, cb: datalayer.client.ResponseCallback, userdata: userData_c_void_p = None) -> Result:
        """
        ...
        b_id = self.id().encode('utf-8')
        return Result(datalayer.clib.libcomm_datalayer.DLR_clientUnsubscribeAllAsync(
            self.__client().get_handle(),
            b_id,
            self.__create_response_callback(cb),
            userdata))
