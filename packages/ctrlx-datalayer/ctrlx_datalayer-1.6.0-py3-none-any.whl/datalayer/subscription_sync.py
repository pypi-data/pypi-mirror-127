...
from datalayer.clib_client import C_DLR_CLIENT_NOTIFY_RESPONSE, C_NotifyItem
from datalayer.variant import Result, Variant


class SubscriptionSync(datalayer.subscription.Subscription):
    """
    SubscriptionSync
    """
    ...
    def __create_sub_callback(self, cb: datalayer.subscription.ResponseNotifyCallback):
        """
        callback management
        """
        ...
        def _cb(status: datalayer.clib.C_DLR_RESULT, items: ctypes.POINTER(C_NotifyItem),
    ...
    def _create(self, prop: Variant, cnb: datalayer.subscription.ResponseNotifyCallback,
        ...
        """
        r = Result(datalayer.clib.libcomm_datalayer.DLR_clientCreateSubscriptionSync(
    ...
    def subscribe(self, address: str) -> Result:
        ...
        return Result(datalayer.clib.libcomm_datalayer.DLR_clientSubscribeSync(
    ...
    def unsubscribe(self, address: str) -> Result:
        ...
        b_id = self.id().encode('utf-8')
        b_address = address.encode('utf-8')
        return Result(datalayer.clib.libcomm_datalayer.DLR_clientUnsubscribeSync(
            self.__client().get_handle(), b_id, b_address))
    ...
    def subscribe_multi(self, address: typing.List[str]) -> Result:
        ...
        return Result(datalayer.clib.libcomm_datalayer.DLR_clientSubscribeMultiSync(
    ...
    def unsubscribe_multi(self, address: typing.List[str]) -> Result:
        ...
        b_address = (ctypes.c_char_p * len(address))(*
                                                     [d.encode('utf-8') for d in address])
        return Result(datalayer.clib.libcomm_datalayer.DLR_clientUnsubscribeMultiSync(
            self.__client().get_handle(), b_id, b_address, len(address)))
    ...
    def unsubscribe_all(self) -> Result:
        ...
        b_id = self.id().encode('utf-8')
        return Result(datalayer.clib.libcomm_datalayer.DLR_clientUnsubscribeAllSync(
