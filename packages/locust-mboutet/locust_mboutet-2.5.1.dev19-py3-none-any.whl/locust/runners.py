...
class MasterRunner(DistributedRunner):
    ...
    def __init__(self, environment, master_bind_host, master_bind_port):
        ...
        if self.rebalancing_enabled():
            self.greenlet.spawn(self.periodic_rebalance).link_exception(greenlet_exception_handler)
    ...
    def periodic_rebalance(self):
        while True:
            ...
            self.start(user_count=self.target_user_count, spawn_rate=self.target_user_count + 1)
                ...
                if client.heartbeat < 0 and client.state != STATE_MISSING:
                    ...
                    if self._users_dispatcher is not None:
                        ...
                        if self.rebalancing_enabled() and self.state == STATE_RUNNING and self.spawning_completed:
                            self.start(self.target_user_count, self.target_user_count + 1)
