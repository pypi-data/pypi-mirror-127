...
class CommunityClient(AminoHttpClient):
    ...
    async def get_chat_messages(self, chatId: str, size: int = 25, pageToken: str = None) -> GetMessages:
        response = await self.get(f"/x{self.comId}/s/chat/thread/{chatId}/message" /
                f"?v=2&pagingType=t&size={size}" + f"&pageToken={pageToken}" if pageToken else "")
    ...
    async def get_recent_blogs(self, pageToken: str = None, start: int = 0, size: int = 25) -> GetBlogs:
        response = await self.get(f"/x{self.comId}/s/feed/blog-all?start={start}&size={size}" \
            f"&v=2&pagingType=t" + f"&pageToken={pageToken}" if pageToken else "")
