...
def setup_test_environment(stack: ExitStack,
    ...
    if registry_docker_image is None:
        ...
        registry_url = start_microservice_container(registry_docker_image, mongo_, stack, '/components', 8080)
        for image in other_docker_images_:
            start_microservice_container(image.image, mongo_, stack, image.check_path, image.port)
        config_file = f"""
                                        [DEFAULT]
