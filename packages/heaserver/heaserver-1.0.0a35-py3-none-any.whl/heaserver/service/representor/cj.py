...
def to_nvpjson(cj_template: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for d in data:
        ...
        if section is not None and index is not None:
            result.setdefault(section, []).append((index, nm, val))
        elif section is not None:
            result.setdefault(section, {})[nm] = val
        else:
            result[nm] = val
    _extract_list_objects(result)
    return result


def _extract_list_objects(result: Dict[str, Any]) -> None:
    """
    Replaces any list values represented as a three tuple (index, name, value) with a list of nvpjson object dicts
    ordered by index.
    :param result: an nvpobject dict with list values represented as a list of three tuples (index, name, value).
    """
    for section, index_nm_val_list in result.items():
        if isinstance(index_nm_val_list, list):
            dd = defaultdict(list)
            for elt in index_nm_val_list:
                dd[elt[0]].append((elt[1], elt[2]))
            result[section] = [dict(elt[1]) for elt in sorted(dd.items())]
    ...
    for action in actions:
        if _is_in_target('cj-template', action):
            ...
            for d in action['inputs']:
                ...
                if isinstance(value_, list) and len(value_) > 0:
                    if 'section' not in d:
                        data_ = {
                            'name': d['name'],
                            'value': value_,
                        ...
                        for i, v in enumerate(value_):
                            ...
                            rtn['data'].append(data_)
                elif isinstance(value_, dict) and len(value_) > 0:
                    data_ = {
                        'section': d['section'],
                        'name': d['name'],
                        'value': value_[d['name']],
