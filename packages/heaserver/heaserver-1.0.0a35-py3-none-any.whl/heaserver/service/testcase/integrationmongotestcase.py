...
def setup_test_environment(stack: ExitStack,
                           ...
                           registry_docker_image: Optional[str] = None,
                           other_docker_images: Optional[Iterable[str]] = None) -> str:
    ...
    if registry_docker_image is None:
        ...
        registry_url = start_container(registry_docker_image, mongo_, stack)
        for image in other_docker_images_:
            start_container(image, mongo_, stack)
        config_file = f"""
                                        [DEFAULT]
...
def start_container(image: str, mongo_: MongoDbContainer, stack: ExitStack) -> str:
    """
    Starts a Docker container with the provided image, Mongo database container, and exit stack for cleaning up
    resources.

    :param image: the Docker image string (required).
    ...
    base_url = f'http://{microservice.get_container_host_ip()}:{get_exposed_port(microservice, 8080)}'
    wait_for_status_code(str(URL(base_url) / 'components'), 200)
    return base_url
