...
class Arb:
	...
	def udMarker(self):
		...
		if not hasattr(self, '_udMarker'):
			from .UdMarker import UdMarker
			self._udMarker = UdMarker(self._core, self._cmd_group)
	...
	def msegment(self):
		...
		if not hasattr(self, '_msegment'):
			from .Msegment import Msegment
			self._msegment = Msegment(self._core, self._cmd_group)
