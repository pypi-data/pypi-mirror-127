...
class Sstop:
	...
	def set(self, start_index: int, stop_index: int) -> None:
		...
		Snippet: driver.configure.gprf.measurement.power.listPy.sstop.set(start_index = 1, stop_index = 1) \n
	...
	def get(self) -> SstopStruct:
		"""SCPI: CONFigure:GPRF:MEASurement<Instance>:POWer:LIST:SSTop \n
		Snippet: value: SstopStruct = driver.configure.gprf.measurement.power.listPy.sstop.get() \n
		Selects the range of segments to be measured (first and last segment of a sweep) . The total number of segments per sweep,
		including repetitions, must not be higher than 10000. \n
