...
class ParameterSetList:
	"""ParameterSetList commands group definition. 15 total commands, 5 Sub-groups, 1 group commands"""
	...
	def pdefSet(self):
		...
		if not hasattr(self, '_pdefSet'):
			from .PdefSet import PdefSet
			self._pdefSet = PdefSet(self._core, self._cmd_group)
	...
	def clone(self) -> 'ParameterSetList':
		...
		new_group = ParameterSetList(self._core, self._cmd_group.parent)
