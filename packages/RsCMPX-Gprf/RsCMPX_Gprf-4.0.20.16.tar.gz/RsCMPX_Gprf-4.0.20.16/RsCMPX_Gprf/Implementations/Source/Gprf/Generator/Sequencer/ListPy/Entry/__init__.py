...
class Entry:
	...
	def call(self):
		...
		if not hasattr(self, '_call'):
			...
			self._call = Call(self._core, self._cmd_group)
		return self._call
	...
	def mup(self):
		...
		if not hasattr(self, '_mup'):
			from .Mup import Mup
			self._mup = Mup(self._core, self._cmd_group)
		return self._mup
	...
	def mdown(self):
		...
		if not hasattr(self, '_mdown'):
			...
			self._mdown = Mdown(self._core, self._cmd_group)
		return self._mdown
	...
	def clone(self) -> 'Entry':
		...
		new_group = Entry(self._core, self._cmd_group.parent)
