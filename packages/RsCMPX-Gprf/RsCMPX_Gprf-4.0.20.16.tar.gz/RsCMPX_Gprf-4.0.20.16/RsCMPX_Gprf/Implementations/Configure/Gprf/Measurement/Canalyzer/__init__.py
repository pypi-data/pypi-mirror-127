...
class Canalyzer:
	...
	def iqFile(self):
		...
		if not hasattr(self, '_iqFile'):
			from .IqFile import IqFile
			self._iqFile = IqFile(self._core, self._cmd_group)
		return self._iqFile
	...
	def sall(self):
		...
		if not hasattr(self, '_sall'):
			...
			self._sall = Sall(self._core, self._cmd_group)
		return self._sall
	...
	def clone(self) -> 'Canalyzer':
		...
		new_group = Canalyzer(self._core, self._cmd_group.parent)
