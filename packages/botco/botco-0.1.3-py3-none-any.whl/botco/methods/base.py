...
from ..utils.exceptions import BotcoError
...
class TelegramMethod(abc.ABC, BaseModel, Generic[TelegramType]):
    ...
    def build_response(self, data: Dict[str, Any]) -> Response[TelegramType]:
        ...
        if response.ok:
            ...
            e = BotcoError(error_code=response.error_code, description=response.description,
                           parameters=response.parameters, method=response.method)
            raise e
