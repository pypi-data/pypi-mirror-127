...
from ..contrib.mixin import BotObjectMixin
...
class Message(TelegramObject, BotObjectMixin):
    """
    ...
    The channel itself for channel messages. The supergroup itself for 
    messages from anonymous group administrators. The linked channel for 
    ...
    <https://core.telegram.org/bots/api#games>`_"""
    ...
    information about the invoice. `More about payments » <https://core.telegram.org/bots/api#payments>`_"""
    successful_payment: Optional[SuccessfulPayment] = None
    """*Optional*. Message is a service message about a successful payment, information about the 
    payment. `More about payments » <https://core.telegram.org/bots/api#payments>`_"""
    connected_website: Optional[str] = None
    """*Optional*. The domain name of the website on which the user has logged in. `More about 
    Telegram Login » <https://core.telegram.org/widgets/login>`_"""
    ...
    """*Optional*. Inline keyboard attached to the message. :code:`login_url` buttons are 
    represented as ordinary :code:`url` buttons."""

    def __str__(self):
        return self.json(exclude_unset=True, exclude={'bot'})
    ...
    def send_copy(
        ...
        if self.text:
            call = SendMessage(text=text, entities=entities, **kwargs)
        elif self.audio:
            call = SendAudio(
            ...
            call = SendSticker(sticker=self.sticker.file_id, **kwargs)
        elif self.video:
            call = SendVideo(
                video=self.video.file_id, caption=text, caption_entities=entities, **kwargs
            )
        elif self.video_note:
            call = SendVideoNote(video_note=self.video_note.file_id, **kwargs)
        elif self.voice:
            call = SendVoice(voice=self.voice.file_id, **kwargs)
        elif self.contact:
            call = SendContact(
    ...
    def delete_reply_markup(self) -> EditMessageReplyMarkup:
        call = self.edit_reply_markup(reply_markup=None)
        return self.bot(call)
    ...
    def delete(self) -> DeleteMessage:
        ...
        call = DeleteMessage(chat_id=self.chat.id, message_id=self.message_id)
        return self.bot(call)
