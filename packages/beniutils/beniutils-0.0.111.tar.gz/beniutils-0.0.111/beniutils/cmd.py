...
def task_create(
    targetPath: Optional[str] = typer.Argument("", help="指定项目路径，默认在当前系统路径", show_default=False),
):
...
def timestamp(
    value: Optional[float] = typer.Argument(None, help="时间戳，默认使用当前系统的时间戳", show_default=False),
):
