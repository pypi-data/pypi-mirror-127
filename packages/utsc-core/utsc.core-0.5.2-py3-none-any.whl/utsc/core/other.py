...
class Prompt:
    ...
    def string(self, var: str, description: str | None, default_value : str | None = None, **kwargs) -> str:
    ...
    def select(self, var: str, choices: list[str], description: str | None, default_value : str | None = None, **kwargs) -> str:

        validator = self.Validator.from_callable(
            lambda x: x in choices, 
            error_message="Choice must be one of {}".format(", ".join(choices))
        )

        opts = dict(
            completer=self.WordCompleter(choices),
            complete_while_typing=True,
            rprompt=self.HTML("Valid options are: <b>{}</b>".format(", ".join(choices))),
            validator=validator,
            )
        opts.update(kwargs)
    ...
    def bool(self, var: str, description: str | None, default_value: bool | None = None, **kwargs) -> bool:
    ...
    def list(self, var: str, description: str | None, **kwargs) -> list[str]:
        ...
        opts = dict(
            multiline=True,
            rprompt=self.HTML("Press <b>Enter</b> to add a new line, <b>Alt-Enter</b> or <b>Ctrl+D</b> to finish and submit list."),
            key_bindings=kb,
            )
        opts.update(kwargs)
