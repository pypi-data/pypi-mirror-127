...
from ciocore import api_client, common, worker, client_db, loggeria, compat
...
def resolve_args(args):
    ...
    """
    cfg = config.config().config
    args["md5_caching"] = args.get("md5_caching",cfg["md5_caching"] )
    args["database_filepath"] = args.get("database_filepath")
    args["location"] = args.get("location")  
    args["thread_count"] =  args.get("thread_count", cfg["thread_count"])
