...
from packet_client.client import Ui_MainClientWindow
from packet_client.client import AddContactDialog
from packet_client.client import DelContactDialog
...
class ClientMainWindow(QMainWindow):
    ...
    def set_disabled_input(self):
        ...
        self.ui.label_new_message.setText(
            "То choose a recipient make a double click on user icon.")
    ...
    def history_list_update(self):
        ...
        for i in range(start_index, length):
            ...
            if item[1] == 'in':
                ...
                mess = QStandardItem(
                    f'Outcoming from {item[3].replace(microsecond=0)}:\n {item[2]}')
                mess.setEditable(False)
    ...
    def select_active_user(self):
        ...
        # Выбранный пользователем (двойным кликом) находится в выделенном
    ...
    def set_active_user(self):
        ...
        try:
            self.current_chat_key = self.transport.key_request(
                self.current_chat)
            LOGGER.debug(f'Open key is uploaded for {self.current_chat}')
            ...
            self.current_chat_key = None
            self.encryptor = None
            LOGGER.debug(f'Failed to get a key for {self.current_chat}')

        # Если ключа нет, то ошибка, что не удалось начать чат с пользователем
        if not self.current_chat_key:
            self.messages.warning(
                self, 'Error', 'There is no encrypted key for chosen user.')
            return

        # Ставим надпись и активируем кнопки
        self.ui.label_new_message.setText(
            f'Input a message for {self.current_chat}:')
        self.ui.btn_clear.setDisabled(False)
        self.ui.btn_send.setDisabled(False)
        self.ui.text_message.setDisabled(False)

        # Заполняем окно истории сообщений по требуемому пользователю.
        ...
        try:
            ...
            if err.errno:
                self.messages.critical(
                    self, 'Error', 'Connection to server is lost!')
                self.close()
            self.messages.critical(self, 'Error', 'Timeout connection!')
        else:
            self.database.add_contact(new_contact)
            new_contact = QStandardItem(new_contact)
            new_contact.setEditable(False)
            self.contacts_model.appendRow(new_contact)
            LOGGER.info(f'Contact {new_contact} is added successfully')
            self.messages.information(
                self, 'Success', 'Contact is added successfully.')
    ...
    def delete_contact(self, item):
        ...
        try:
            ...
            self.clients_list_update()
            LOGGER.info(f'Contact {selected} is deleted successfully')
            self.messages.information(
                self, 'Success', 'Contact is deleted successfully.')
            item.close()
    ...
    def send_msg(self):
        ...
        try:
            ...
            LOGGER.debug(
                f'message is sent to {self.current_chat}: {message_text}')
            self.history_list_update()
    ...
    def message(self, message):
        ...
        try:
            ...
            self.messages.warning(
                self, 'Error', 'It is impossible to decode teh message.')
            return
        ...
        if sender == self.current_chat:
            ...
            if self.database.check_contact(sender):
                ...
                if self.messages.question(
                        self,
                        'New message',
                        f'New message is received from {sender}, open the chat with him?',
                        QMessageBox.Yes,
                ...
                if self.messages.question(
                        self,
                        'New message',
                        f'New message is received from {sender}.\n This user is out of your contact list.\n '
                        f'Add to contacts and opne the chat with him?',
                        QMessageBox.Yes,
                        QMessageBox.No) == QMessageBox.Yes:
    ...
    def sig_205(self):
        ...
        if self.current_chat and not self.database.check_user(
            ...
            self.messages.warning(
                self,
                'Sorry',
                'Your mate was deleted from server.')
            self.set_disabled_input()
            self.current_chat = None
        self.clients_list_update()
