...
def show_gaps_date(spark=None, path_name=None, hdfs_uri=None):
    ...
    for ind, table in enumerate(table_list):
        ...
        for index, table_supplies_path in enumerate(table_supplies_list):
            ...
            try:
                ...
                df2["year_month"] = "202101"
                df2["max_partition_date"] = pd.to_datetime("2021-01-31")
                df2["count_partition_date"] = np.NaN
                df2["max_month_day"] = "0"
    ...
    df_pivot = pd.pivot_table(data=df_detail2,
                              ...
                              aggfunc=[np.max])
    df_pivot.columns = [f'PARTITION_{str(j).upper()}' for i, j in df_pivot.columns]
