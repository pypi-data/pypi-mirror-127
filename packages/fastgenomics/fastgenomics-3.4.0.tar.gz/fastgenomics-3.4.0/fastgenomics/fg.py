...
import requests
from requests.adapters import Response, Retry
...
class FASTGenomicsAccount:
    ...
    def __init__(self):
        ...
        self.identityserver_url: str = ""
        self.fastgenomics_url: str = ""
        self.fastgenomics_user: str = ""
        self.fastgenomics_passphrase: str = ""
        self.fastgenomics_login_method: str = ""
...
class FASTGenomicsClient:
    ...
    def _get_identityserver_url(self, url: str) -> str:
        """ get the identityserver url from webclient settings """
        try:
            headers = {
            ...
            ids_url = jsonResponse.get("idsUrl")
            if not ids_url.endswith("/"):
                ids_url = ids_url + "/"
            return ids_url

        except HTTPError as http_err:
            raise Exception(
                f"Failed to get settings from '{url}'': {http_err}"
            )
        except Exception as err:
            raise Exception(
                f"Failed to get settings from '{url}'': {err}"
            )
    ...
    def _get_bearer_token(self) -> str:
        ...
        try:
            ...
            if account.fastgenomics_login_method == "pat":
                url = self.get_identityserver_url("api/v1/token/pat")
                ...
                url = self.get_identityserver_url("api/v1/token/resourceowner")
    ...
    def login(self, user: str, passphrase: str, login_method: str, url: str):
        """login to the platform and store credentials in users profile
        ...
        elif login_method.lower() == "nologin":
        ...
        ids_url = self._get_identityserver_url(url)
        account.identityserver_url = ids_url
    ...
    def logout(self):
        """ logout from the platform and remove credentials file """
        filename = FASTGenomicsClient.__get_credentials_filename()
        if path.exists(filename):
            remove(filename)
        self.fastgenomics_account = None
    ...
    def get_platform_url(self, relative_url: str) -> str:
        """ get the absolute platform url """
        if relative_url.startswith("/"):
            relative_url = relative_url[1:]
        return self._get_fastgenomics_account().fastgenomics_url + relative_url

    def get_backend_url(self, relative_url: str) -> str:
        """ get the absolute backend url """
        if relative_url.startswith("/"):
            relative_url = relative_url[1:]
        return self._get_fastgenomics_account().fastgenomics_url + "webclient/" + relative_url

    def get_identityserver_url(self, relative_url: str) -> str:
        """ get the absolute identityserver url """
        if relative_url.startswith("/"):
            relative_url = relative_url[1:]

        url = self._get_fastgenomics_account().identityserver_url
        if url is None or url == "":
            raise Exception(
                "You account information has gone stale! Please logout and login again!")

        return url + relative_url

    def get_dataset_details_url(self, dataset_id: str, tab: str) -> str:
        """ get the url of dataset details tab """
        if tab == "":
            return self.get_platform_url(f"datasets/detail-{dataset_id}")
        return self.get_platform_url(f"datasets/detail-{dataset_id}#{tab}")

    def generic_get(self, url: str) -> Response:
        """ Sends a GET request. Returns :class:`Response` object.

        Parameters:
          url : url (if it is relative, it will is relative to plattform)
    ...
    def generic_put_json(self, url: str, json: str) -> Response:
        """ Sends a JSON PUT request. Returns :class:`Response` object.

        Parameters:
          url : url (if it is relative, it will is relative to plattform)
          json: a json string containing the data
        ...
        response = self.session.put(
            url, headers=headers, data=json.encode("utf-8")
        )
        self._handle_response(response)
        return response

    def generic_post_json(self, url: str, json: str) -> Response:
        """ Sends a JSON POST request. Returns :class:`Response` object.
    ...
    def generic_post_file(self, url: str, name: str, file_name: str, mime_type="application/octet-stream") -> Response:
        """ Sends a file PUT multipart form request. Returns :class:`Response` object.

        Parameters:
          url          : url (if it is relative, it will is relative to plattform)
          name         : the name of the file
          file_name    : the path to file to be uploaded
    ...
    def generic_patch_json(self, url: str, json: str) -> Response:
        """ Sends a JSON PATCH request. Returns :class:`Response` object.
    ...
    def generic_delete(self, url: str) -> Response:
...
class FASTGenomicsLargeFileStorageClient(FASTGenomicsClient):
    ...
    def __create_large_file_storage(
        ...
        try:
            ...
            url = self.get_backend_url("api/lfs")
    ...
    def __create_dataset_and_attach_lfs(
        ...
        url = self.get_backend_url("api/datasets")
        ...
        url = self.get_backend_url(
            f"api/lfs/{sai.storage_id}/dataset/{dataset_id}"
        )
        headers = {
            "Content-type": "application/json",
            "Accept": "application/json",
    ...
    def get_download_url(self, storage_id: str, access_token: str) -> str:
        ...
        url = self.get_backend_url(
            f"api/lfs/{storage_id}/content/{access_token}"
        )
...
class FASTGenomicsDatasetClient(FASTGenomicsClient):
    ...
    def add_file_to_dataset(
        ...
        url = self.get_backend_url("api/fileuploads")
        ...
        url = self.get_backend_url(f"api/datasets/{dataset_id}/files")
        ...
        with open(full_filename, "rb") as f:
            ...
            while chunk:
                url = self.get_backend_url(
                    f"api/fileuploads/{uploadId}/chunk/{chunkId}"
                )
                headers = {
                    "Authorization": f"Bearer {bearer_token}",
                    "User-Agent": user_agent,
        ...
        url = self.get_platform_url(f"/datasets/detail-{dataset_id}#Files")
    ...
    def set_metadata(self, dataset_id: str, metadata: dict):
        ...
        url = self.get_backend_url(f"api/datasets/{dataset_id}/metadata")
    ...
    def delete_dataset(self, dataset_id: str):
        ...
        url = self.get_backend_url(f"api/datasets/{dataset_id}")
    ...
    def get_dataset_types(self) -> requests.Response:
        ...
        url = self.get_backend_url("api/datasettypes")
    ...
    def get_dataset_type_by_id(self, id) -> requests.Response:
        ...
        logging.info(f"getting dataset types")
        url = self.get_backend_url(f"api/datasettypes/{id}")
    ...
    def get_dataset_editable_fields(self, id) -> requests.Response:
        ...
        url = self.get_backend_url(f"api/datasettypes/{id}/editablefields")
