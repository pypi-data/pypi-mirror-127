...
def process(verbose, command, profile, env, stage, stack, app_suffix, build, api_domain, api_stage, app_api_domain, require_approval, tag, bitbucket_username, bitbucket_password, bitbucket_branch, bitbucket_tag_value, aws_home, disable_clone_prompt, version, filter, log_group, starting_hour, disassociate_cname_flag):
    """
    ...
    if command.lower() == 'bootstrap':
        ...
        generate_cdk_json(logger, app_suffix, app_domain, env, stack, stage, brand, app_name, disassociate_cname_flag)
        if stack == 'core':
...
def generate_cdk_json(logger, app_suffix, app_domain, env, stack, stage, brand, app_name, disassociate_cname_flag):
