...
from common.descryptors import Port, Host
...
class MessageProcessor(threading.Thread):
    ...
    def autorize_user(self, message, sock):
        ...
        if message[USER][ACCOUNT_NAME] in self.names.keys():
            ...
            try:
                logger.debug(f'Имя пользователя занято, отправлено {response}')
        ...
        # Проверяем, что пользователь зарегистрирован на сервере.
            ...
            try:
                logger.debug(f'Неизвестное имя пользователя, отправлено {response}')
                send_message(sock, response)
            except OSError:
                pass
            self.clients.remove(sock)
            ...
            logger.debug('Пользователь существует, проверяем пароль')
