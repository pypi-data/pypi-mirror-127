...
def pytest_configure(config):
    ...
    config.addinivalue_line (
        "markers", "external: mark test to run only on --mark-exteranl"
    )
...
def pytest_addoption (parser):
    ...
    parser.addoption (
        "--mark-external", action="store_true", default=False, help="run including exteranl marked tests"
    )
    parser.addoption (
...
def pytest_collection_modifyitems (config, items):
    ...
    skip_external = pytest.mark.skip (reason = "need --mark-external option to run")
    for item in items:
        if not config.getoption ("--mark-slow") and "slow" in item.keywords:
            item.add_marker (skip_slow)
        if not config.getoption ("--mark-external") and "external" in item.keywords:
