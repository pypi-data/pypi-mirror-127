...
class Maze(DOMWidget):
    ...
    @observe('is_inited')
    def _handle_is_inited(self, change):
        print(f"got callback {change.new}")
        if self.is_inited == True:
            print("got inited")
            self.js_call('flush_js_q', [])
    ...
    def js_call(self, method_name, params):
        if self.is_inited == False:
            time_lapsed = datetime.now() - self.init_time
            if time_lapsed.seconds > 4:
                raise RuntimeError("Some Network Error. Please Try Again")
