...
class WorldModel {
    ...
    init_output_window() {
        ...
        jquery_1.default('.output-action-bttn', $currentRun).on('click', function () {
            console.log('clicked button');
            if (jquery_1.default('.output-action-bttn', $currentRun).html() == 'ᐁ') {
                jquery_1.default('.output-action-bttn', $currentRun).html('ᐅ');
            }
            else {
                jquery_1.default('.output-action-bttn', $currentRun).html('ᐁ');
            }
            jquery_1.default('.grid-slider', $currentRun).slideToggle();
        });
        //Draggble $current_run
        if (this.config.floating && !this.draggable) {
            draggable_1.default($currentRun[0]);
            this.draggable = true;
        }
        if (!this.config.floating) {
            $currentRun[0].onmousedown = null;
        }
    }
    init_with_id(ui_id) {
    ...
    skeleton() {
      ...
      <div class="output-header clr-info">
        ...
        <div class="output-actions btn-group">
          <button class="output-action-bttn">&square;</button>
          <button class="output-action-bttn">&boxminus;</button>
          <button class="output-action-close">ᐁ</button>
        </div>
      </div>   
