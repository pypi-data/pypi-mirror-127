...
def rotate_recon(features,recons):
    aligned_crs = np.array([np.roll(recons[t,:],-features[t]) for t in range(len(features))])
    return aligned_crs
...
def IEM(trialbyvoxel,features,stim_max=180,is_circular=True,nfolds=10,
    ...
    aligned_recons = rotate_recon(features,recons)
