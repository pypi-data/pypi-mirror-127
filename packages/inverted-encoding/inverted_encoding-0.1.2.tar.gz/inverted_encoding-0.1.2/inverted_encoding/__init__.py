...
def rotate_recon(trial_features,recons):
    aligned_crs = np.array([np.roll(recons[t,:],-trial_features[t]) for t in range(len(trial_features))])
    return aligned_crs
...
def IEM(trialbyvoxel,features,stim_max=180,is_circular=True,nfolds=10,
    ...
    channel_sd : float, optional
        The standard deviation of each channel of the basis set. Defaults to standard deviation of a cosine raised to nchannels-1.
    ...
    aligned_recons = rotate_recon(trial_features,recons)
