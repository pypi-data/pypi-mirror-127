...
def find_vm_sd(channel_responses,plotting=False):
    ...
    losses = [np.abs(np.sum(yReal - make_gaussian_iter(180,sd,stim_max))) for sd in np.arange(10,180)]

    if plotting:
        plt.plot(yReal)
        plt.plot(make_gaussian_iter(180,np.arange(10,180)[np.argmin(losses)],stim_max))
