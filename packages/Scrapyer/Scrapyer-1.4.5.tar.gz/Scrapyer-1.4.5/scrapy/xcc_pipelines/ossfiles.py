...
class OssFilesPipeline(MediaPipeline):
    ...
    def __init__(self, store_uri, file_pipe_config, download_func=None, settings=None):
        ...
        self.item_header = settings.get('FILE_PIPE_CONFIG').get(self.__class__.__name__).get('ITEM_HEADER',None)
            ...
            def request_add_params(urls):
                for u in urls:
                    ...
                    if self.item_header:
                        ...
                        u.headers = self.item_header
                    if item._cookie and not self.item_header:
