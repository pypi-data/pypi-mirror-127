...
PywbPeriod.prototype.getReadableId = function(hasDayCardinalSuffix) {
    ...
    PywbVue.addComponentConfig('timeline', {
        props: ['period'],
        template: '#timeline',
        data: function() {
            return {
    ...
    PywbVue.addComponentConfig('timeline-summary', {
        props: {
            period: {
                required: true
            }
    ...
    methods: {
      gotoPeriod: function(newPeriod) {
        if (newPeriod.snapshot) {
          this.gotoSnapshot(newPeriod.snapshot.load_url)
        } else {
      ...
      gotoSnapshot(url) {
        ...
        this.$emit('show-snapshot', url);
      },
      init() {
        if (this.config.initialView === 'calendar') {
