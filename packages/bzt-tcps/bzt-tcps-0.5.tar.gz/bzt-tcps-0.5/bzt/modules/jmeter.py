...
class JMeterExecutor(ScenarioExecutor):
    ...
    def prepare(self):
        ...
        self.jvm_args = self.__set_jvm_properties()
    ...
    def __set_jvm_properties(self):
        ...
        if heap_size is not None:
            self.log.debug("bzt-tcps version:0.5")
            ...
            self.env.add_java_param({"HEAP": "-Xms%s -Xmx%s -XX:MaxMetaspaceSize=256m" % (heap_size, heap_size)})
            jvm_args = "-Xms" + heap_size + " -Xmx" + heap_size + " -XX:MaxMetaspaceSize=256m"
            self.log.debug("JVM_ARGS: %s", jvm_args)
            self.env.set({"HEAP": jvm_args})
            return jvm_args
    ...
    def startup(self):
        ...
        """
        cmdline = ["JVM_ARGS="+self.jvm_args+" " +self.tool.tool_path, "-t", self.modified_jmx, "-j", self.jmeter_log, "-q", self.properties_file]
