    ...
    for metric in metrics:
        if metric == SUPPORT:
            if "X and Y" in expression_metrics.keys():
                results[metric] = expression_metrics.get("X and Y")
            elif "X" in expression_metrics.keys():
                results[metric] = expression_metrics["X"]
            else:
                continue
        elif metric == RELATIVE_SUPPORT:
            if (
                "X and Y" in expression_metrics.keys()
                and "X" in expression_metrics.keys()
            ):
                results[metric] = (
                    expression_metrics["X and Y"] / expression_metrics["X"]
                )
            elif "X" in expression_metrics.keys() and "~X" in expression_metrics.keys():
                results[metric] = expression_metrics["X"] / (
                    expression_metrics["X"] + expression_metrics["~X"]
                )
            else:
                continue
        elif metric == EXCEPTIONS:
            if "X and ~Y" in expression_metrics.keys():
                results[metric] = expression_metrics["X and ~Y"]
            elif "~X" in expression_metrics.keys():
                results[metric] = expression_metrics["~X"]
            else:
                continue
        elif metric == RELATIVE_EXCEPTIONS:
            if (
                "X and ~Y" in expression_metrics.keys()
                and "X" in expression_metrics.keys()
            ):
            ...
            if (
                "X and Y" in expression_metrics.keys()
                and "X and ~Y" in expression_metrics.keys()
            ):
                results[metric] = np.round(
                    expression_metrics["X and Y"]
                    / (
                        expression_metrics["X and Y"]
                        + expression_metrics["X and ~Y"]
                        + 1e-11
                ...
                results[metric] = np.round(
                    expression_metrics["X"]
                    / (expression_metrics["X"] + expression_metrics["~X"] + 1e-11),
                    4,
                )
            else:
                continue
            ...
            if "X and Y" in expression_metrics.keys():
                ...
                results[metric] -= (
                    expression_metrics["X and Y"] / expression_metrics["Y"]
                )
            else:
                continue
