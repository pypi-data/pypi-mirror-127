...
class PatternMiner:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.df_patterns is not None and self.df_data is not None:
    ...
    def __process_parameters(self, *args, **kwargs):
        ...
        self.metapatterns = self.__process_key(
            "metapatterns", dict, self.metapatterns, *args, **kwargs
        )
        self.metapatterns = self.__process_key(
            "metapatterns", list, self.metapatterns, *args, **kwargs
        )
        self.df_patterns = self.__process_key(
            "df_patterns", None, self.df_patterns, *args, **kwargs
        )
        self.df_data = self.__process_key(
            "dataframe", pd.DataFrame, self.df_data, *args, **kwargs
        )
        self.clean_quotation = self.__process_key(
            "clean_quotation", bool, self.clean_quotation, *args, **kwargs
        )
        ...
        if (
            self.clean_quotation
        ):  
            # clean data of quotation marks. Needed with data that contains names with quotations
            self.df_data.loc[:, self.df_data.dtypes == object] = self.df_data.loc[
                :, self.df_data.dtypes == object
            ].astype(str)
            self.df_data.loc[:, self.df_data.dtypes == object] = self.df_data.loc[
                :, self.df_data.dtypes == object
    ...
    def find(self, *args, **kwargs):
        ...
        new_df_patterns = self.derive_patterns()
    ...
    def correct_data(self, *args, **kwargs):
        ...
        df_results["correct_value"] = df_results["pattern_def"].apply(
            lambda x: get_value(x, num=2)
        )
        ...
        df_data.loc[df_data.index.isin(df_results.index), colq] = df_results[
            "correct_value"
        ]
    ...
    def analyze(self, *args, **kwargs):
        ...
        self.df_results = derive_results(
            **kwargs,
            df_patterns=self.df_patterns,
            dataframe=self.df_data,
            metapatterns=self.metapatterns
        )

        return self.df_results


    def update_statistics(self, df_data = None):
        ...
        for level in range(len(self.df_data.index.names)):
            self.df_data[self.df_data.index.names[level]] = self.df_data.index.get_level_values(
                level=level
            )

        for idx in self.df_patterns.index:
            # make sure to only evaluate rules that have the columns of the dataframe
            key_in = True
            keys = re.findall(r"{(.*?)}", self.df_patterns.loc[idx, PATTERN_DEF])
            for key in keys:
                if key[1:-1] not in self.df_data.columns:
                    key_in = False
                    break
            if key_in == False:
                self.df_patterns = self.df_patterns.drop(index=idx)
                continue

            # Calculate pattern statistics (from evaluating pandas expressions)
            pandas = ast.literal_eval(str(self.df_patterns.loc[idx, PANDAS]))

            # in case of cluster we need to alter the data
            if self.df_patterns.loc[idx, CLUSTER] != 0:
                new_dataframe = self.df_data[
                    self.df_data[self.metapatterns[0]["cluster"]]
                    == self.df_patterns.loc[idx, CLUSTER]
                ]
            else:
                new_dataframe = self.df_data
            ...
            try:
                ...
                )  # strange, suppress slice error
                self.df_patterns.loc[idx, SUPPORT] = metrics[SUPPORT]
                self.df_patterns.loc[idx, EXCEPTIONS] = metrics[EXCEPTIONS]
                self.df_patterns.loc[idx, CONFIDENCE] = metrics[CONFIDENCE]
                pd.set_option("mode.chained_assignment", "warn")
            except TypeError as e:
                self.df_patterns.loc[idx, SUPPORT] = None
                self.df_patterns.loc[idx, EXCEPTIONS] = None
                self.df_patterns.loc[idx, CONFIDENCE] = None
                self.df_patterns.loc[idx, ERROR] = e
            except:
                self.df_patterns.loc[idx, SUPPORT] = None
                self.df_patterns.loc[idx, EXCEPTIONS] = None
                self.df_patterns.loc[idx, CONFIDENCE] = None
                self.df_patterns.loc[idx, ERROR] = "ERROR unknown"

        for level in range(len(self.df_data.index.names)):
            del self.df_data[self.df_data.index.names[level]]
    ...
    def convert_columns_to_time(self, name_col, year, extra=[], tqd=False):
        ...
        for name in tqdm(
            iterable=names, total=names.size, disable=tqd, position=0, leave=True
        ):
            count += 1
    ...
    def convert_to_time(self, to_index, year, to_del=[], set_year=True):
        ...
        for i in range(len(years) - 1):
            ...
            df_new = pd.merge(
                df1, df2, on=to_index, suffixes=(" (t-1)", " (t)")
            )  # merge with suffices
            df_new[year] = (
                df_new[year + " (t-1)"].astype(str)
                + " - "
                + df_new[year + " (t)"].astype(str)
            )  # make new year index
    ...
    def derive_patterns(self):
        ...
        for metapattern in self.metapatterns:
            if (
                "parameters" in metapattern.keys()
                and "metrics" in metapattern["parameters"]
            ):
                for metric in metapattern["parameters"].get("metrics"):
                    if (metric not in metrics) and (metric in ALL_METRICS):
                        metrics.append(metric)
        df_patterns = pd.DataFrame(
            columns=[PATTERN_ID, CLUSTER, PATTERN_DEF]
            + metrics
            + [PATTERN_STATUS, ENCODINGS, PANDAS, ERROR]
        )

        dataframe = self.df_data.reset_index()
        ...
        for metapattern in self.metapatterns:
            ...
            if cluster != 0:  # Clusters
                ...
                for i in clusters:
                    ...
                    metapattern[
                        "cluster_group"
                    ] = i  # set cluster to name of cluster instead of column name
                    if "expression" in metapattern.keys():
                        patterns = derive_patterns_from_template_expression(
                            metapattern=metapattern, dataframe=new_data  # expression
                        )
                    else:
                        patterns = derive_patterns_from_code(
                            metapattern=metapattern, dataframe=new_data  # code pattern
                        )
                    logger.info("Total rows in patterns: " + str(patterns.shape[0]))
                    if "min_confidence" in parameters:
                        if (
                            parameters["min_confidence"] == "highest"
                        ):  # For when you want the highest confidence in a pattern
                            patterns = get_highest_conf(patterns)
                            logger.info(
                                "Reduction of rows in patterns to: "
                                + str(patterns.shape[0])
                            )
            ...
            # if no cluster
                ...
                if "expression" in metapattern.keys():
                    patterns = derive_patterns_from_template_expression(
                        metapattern=metapattern, dataframe=dataframe  # expression
                    )
                else:
                    patterns = derive_patterns_from_code(
                        metapattern=metapattern, dataframe=dataframe  # code pattern
                    )
                logger.info("Total rows in patterns: " + str(patterns.shape[0]))
                if "min_confidence" in parameters:
                    if (
                        ...
                        patterns = get_highest_conf(patterns)
                        logger.info(
                            "Reduction of rows in patterns to: " + str(patterns.shape[0])
                        )
                df_patterns = df_patterns.append(patterns, ignore_index=True, sort=False)

        if cluster == 0:
            df_patterns[CLUSTER] = df_patterns[CLUSTER].astype(np.int64)

        if SUPPORT in df_patterns.columns:
            df_patterns[SUPPORT].fillna(0, inplace=True)
            df_patterns[SUPPORT] = df_patterns[SUPPORT].astype(np.int64)
        if EXCEPTIONS in df_patterns.columns:
            df_patterns[EXCEPTIONS].fillna(0, inplace=True)
...
def get_highest_conf(df_patterns):
    ...
    """
    df_patterns["P_val"] = df_patterns["pattern_def"].apply(
        lambda x: get_value(x, num=1)
    )
    df_patterns = df_patterns.sort_values(SUPPORT, ascending=False)  # Sort values
    _, idx = np.unique(
        df_patterns["P_val"].astype(str).values, return_index=True
    )  # Drop duplicate rows

    df_patterns = df_patterns.iloc[
        idx
    ].sort_index()  # Only get dataframe rows from these indices
    df_patterns = df_patterns.drop(["P_val"], 1)
    return df_patterns


def derive_patterns_from_template_expression(metapattern=None, dataframe=None):
    """
    Takes the expression and splits it into a conditional or quantitative expression
    """
    ...
    if re.search(r"IF(.*)THEN(.*)", expression) or expres:  # conditional statement
        patterns = derive_conditional_patterns_from_expression(
            expression, metapattern, dataframe
        )
    else:
        patterns = derive_quantitative_pattern_from_expression(
            expression, metapattern, dataframe
        )
    df_patterns = to_dataframe(
        patterns=patterns, parameters=parameters, encodings=encodings
    )
    return df_patterns


def derive_quantitative_pattern_from_expression(expression, metapattern, dataframe):
    """
    ...
    logger.info("Calling function derive_quantitative_pattern_from_expression")
    ...
    patterns = list()
    logging.info(
        'Make sure the pattern is a standard form of sum, column comparison or value comparison. If you have a special arithmetic, use the parameter "expres":True'
    )
    ...
    if len(columns) > 2:  # Sum pattern
        ...
        # Right format
        Q_columns = [
            dataframe.columns.get_loc(c) for c in Q_columns if c in numerical_columns
        ]
        P_columns = [
            dataframe.columns.get_loc(c) for c in P_columns if c in numerical_columns
        ]
        Q_columns.sort()
        P_columns.sort()

        # Use numpy now
        sums = patterns_sums_column(
            dataframe=dataframe,
            pattern_name=name,
            P_columns=P_columns,
            Q_columns=Q_columns,
        ...
        if (
            item3[0] == "="
        ):  # Stupid bug that does not capture >= or <=, so fixed it like this
            pattern += "="
            item3 = item3[1:]
        ...
        if len(columns) == 2:  # column comparing pattern
            ...
            # right format
            P_columns = get_possible_columns(
                columns[0].count(".*"), columns[0], dataframe, True
            )
            Q_columns = get_possible_columns(
                columns[1].count(".*"), columns[1], dataframe, True
            )
            Q_columns = [
                dataframe.columns.get_loc(c)
                for c in Q_columns
            ...
            compares = patterns_column_column(
                dataframe=dataframe,
                pattern=pattern,
                pattern_name=name,
                P_columns=P_columns,
            ...
            columns = get_possible_columns(
                columns[0].count(".*"), columns[0], dataframe, True
            )
            value = float(item3)
            columns = [
                dataframe.columns.get_loc(c) for c in columns if c in numerical_columns
            ]
            columns.sort()
            values = patterns_column_value(
                dataframe=dataframe,
                value=value,
                pattern=pattern,
                pattern_name=name,
...
def get_possible_columns(amount, expression, dataframe, quant=False):
    ...
    for datapoint in re.findall(
        r"{.*?}", expression
    ):  # See which columns we are looking for per left open column
        ...
        expression = expression.replace(
            datapoint, "{.*}", 1
        )  # Replace it so that it goes well later
    ...
    if (
        amount > 1
    ):  # Combine the lists into combinations where we do not have duplicates
        if re.search("AND", expression):
            possibilities = [
                p
                for p in itertools.product(*all_columns)
                if len(set(p)) == int(len(p) / 2)
            ]
        else:
            possibilities = [
                p for p in itertools.product(*all_columns) if len(set(p)) == len(p)
            ]
    ...
    for columns in possibilities:
        ...
        for column in columns:  # replace with the possible column value
            possible_expression = possible_expression.replace(
                "{.*}", '{"' + column + '"}', 1
            )  # replace with column
...
def get_possible_values(amount, possible_expressions, dataframe):
    ...
    """
    Get the possible values for the conditional expression
    ...
    if amount < 1:  # no values to be found
        ...
        for possible_expression in possible_expressions:
            ...
            for item in re.findall(
                r".*?@", possible_expression
            ):  # See which columns we are looking for per left open column
                value_col = re.findall("{.*?}", item)[
                    -1
                ]  # Get the column that matches the value indicator *@
                value_col = value_col[2:-2]  # strip { and }
                all_columns.append(value_col)

            all_columns_v = (
                dataframe[all_columns].drop_duplicates().values
            )  # get possibilities
            ...
            for i in range(len(items)):
                if len(items[i]) > 1:  # only when we have a string
                    ...
                    for j in range(
                        len(all_columns_v)
                    ):  # check if each column can be used
                        if re.search(item, str(all_columns_v[j][i])) is not None:
                            if re.search(item, str(all_columns_v[j][i]))[0] != str(
                                all_columns_v[j][i]
                            ):
                                del_rows.append(j)
                        else:
            ...
            for columns_v in all_columns_v:
                ...
                for column_v in columns_v:
                    if isinstance(column_v, str):
                        possible_expression_v = possible_expression_v.replace(
                            "[@]", '"' + column_v + '"', 1
                        )  # replace and add ""
                    else:
                        possible_expression_v = possible_expression_v.replace(
...
def derive_conditional_patterns_from_expression(
    expression="", metapattern=None, dataframe=None
):
    """
    ...
    logger.info("Calling function derive_conditional_patterns_from_expression")
    parameters = metapattern.get("parameters", {})
    ...
    if metapattern.get("expression", None):
        ...
        if encode != {}:
            for c in df_features.columns:
                if c in encode.keys():
                    df_features[c] = eval(
                        str(encode[c]) + "(s)", encodings, {"s": df_features[c]}
                    )
    ...
    possible_expressions = get_possible_values(
        amount_v, possible_expressions, dataframe2
    )
    ...
    if len(possible_expressions) > 40:
        logger.warning(
            " Amount of possibilities is high! Namely, "
            + str(len(possible_expressions))
        )
    else:
        logger.info(" Amount of possibilities: " + str(len(possible_expressions)))
    for possible_expression in tqdm(
        iterable=possible_expressions,
        total=len(possible_expressions),
        disable=disable,
        position=0,
        leave=True,
    ):
        pandas = to_pandas_expressions(
            possible_expression, encode, parameters, dataframe
        )
        metrics = evaluate_metrics(
            pandas, encodings, eval_dict, metapattern=metapattern, df_patterns=None
        )
        try:  # Some give error so we use try
            if (metrics[CONFIDENCE] >= confidence) and (metrics[SUPPORT] >= support):
                patterns.extend(
                    [
                        [[name, cluster], possible_expression, list(metrics.values())]
                        + [pandas]
                        + [""]
                    ]
                )
            logger.info(
                "Pattern " + name + " correctly parsed (metrics: " + str(metrics) + ")"
            )
        except TypeError as e:
            logger.error(
                "Error in trying to process pandas expression: "
                + str(e)
                + " : "
                + str(pandas)
            )
            ...
            logger.error(
                "Error in trying to process pandas expression: UNKNOWN ERROR : "
                + str(pandas)
            )
            if solvency:
                patterns.extend(
...
def evaluate_metrics(
    expressions, encodings, eval_dict, metapattern=None, df_patterns=None
):

    if metapattern is not None:
        metrics = DEFAULT_METRICS.copy()
        if (
            "parameters" in metapattern.keys()
            and "metrics" in metapattern["parameters"]
            ...
            for metric in metapattern["parameters"].get("metrics"):
                if (metric not in metrics) and (metric in ALL_METRICS):
                    metrics.append(metric)
    elif df_patterns is not None:
        metrics = [metric for metric in ALL_METRICS if metric in df_patterns.columns]
    else:
        metrics = DEFAULT_METRICS.copy()

    expression_metrics = {
        key: len(eval(expressions[key], encodings, eval_dict).index)
        for key in expressions.keys()
    }

    supp_x = expression_metrics["X"] / (expression_metrics["X"] + expression_metrics["~X"])
    supp_nx = expression_metrics["~X"] / (expression_metrics["X"] + expression_metrics["~X"])
    
    if "Y" in expression_metrics.keys():
        supp_y = expression_metrics["Y"] / (expression_metrics["Y"] + expression_metrics["~Y"])
        supp_ny = expression_metrics["~Y"] / (expression_metrics["Y"] + expression_metrics["~Y"])
    else:
        supp_y = 0
        supp_ny = 0

    results = OrderedDict()
    if "X and Y" in expression_metrics.keys():

        x_and_y = expression_metrics["X and Y"]
        x_and_ny = expression_metrics["X and ~Y"]
        supp_x_and_ny = x_and_ny / (expression_metrics["X"]+expression_metrics["~X"])
        supp_x_and_y = x_and_y / (expression_metrics["X"]+expression_metrics["~X"])

        nx_and_ny = expression_metrics["~X and ~Y"]
        nx_and_y = expression_metrics["~X and Y"]
        supp_nx_and_ny = nx_and_ny / (x_and_y + nx_and_y + x_and_ny + nx_and_ny)
        supp_nx_and_ny = nx_and_ny / (expression_metrics["X"]+expression_metrics["~X"])

        for metric in metrics:

            if metric == SUPPORT:
                results[metric] = x_and_y
            elif metric == EXCEPTIONS:
                results[metric] = x_and_ny

            elif metric == CONFIDENCE:
                if supp_x == 0:
                    results[metric] = np.inf
                else:
                    results[metric] = supp_x_and_y / supp_x

            elif metric == RELATIVE_SUPPORT:
                results[metric] = supp_x_and_y

            elif metric == CONVICTION:
                if supp_x == 0:
                    conf = np.inf
                else:
                    conf = supp_x_and_y / supp_x
                if conf == 1:
                    results[metric] = np.inf
                else:
                    results[metric] = (1-supp_y) / (1-conf)

            elif metric == LIFT:
                if supp_x == 0 or supp_y == 0:
                    results[metric] = np.inf
                else:
                    results[metric] = (supp_x_and_y / supp_x) / supp_y

            elif metric == ADDED_VALUE:
                results[metric] = supp_x_and_y - supp_y

            elif metric == CASUAL_CONFIDENCE:
                if supp_x == 0 or supp_nx == 0:
                    results[metric] = np.inf
                else:
                    results[metric] = 0.5 * supp_x_and_y / supp_x
                    results[metric] += 0.5 * supp_nx_and_ny / supp_nx

            elif metric == CASUAL_SUPPORT:
                results[metric] = supp_x_and_y + supp_nx_and_ny

            elif metric == CERTAINTY_FACTOR:
                continue

            elif metric == COSINE:
                continue

            elif metric == CONFIRMED_CONFIDENCE:
                continue

            else:
                continue

            results[metric] = np.round(results[metric], 4)
        ...
        for metric in metrics:
            if metric == SUPPORT:
                results[metric] = expression_metrics["X"]
            elif metric == EXCEPTIONS:
                results[metric] = expression_metrics["~X"]
            elif metric == RELATIVE_SUPPORT:
                results[metric] = supp_x
            elif metric == CONFIDENCE:
                results[metric] = supp_x
            elif metric == ADDED_VALUE:
                continue
...
def derive_patterns_from_code(metapattern=None, dataframe=None):
    ...
    # conditional pattern
    if pattern == "-->":
        possible_expressions = derive_conditional_pattern(
            metapattern=metapattern, dataframe=dataframe
        )
        for expression in possible_expressions:
            patterns.extend(
                derive_conditional_patterns_from_expression(
                    expression, metapattern, dataframe
                )
            )
...
def derive_conditional_pattern(dataframe=None, metapattern=None):
    """
    ...
    P_values = metapattern.get(
        "P_values", ["[@]"] * len(P_columns)
    )  # in case we do not have values
    ...
    if encode != {}:
        ...
        for idx in range(len(df_features.index)):
            ...
            expression = generate_conditional_expression(
                P_columns, P_values, Q_columns, Q_values, parameters
            )
            expression = expression.replace('"[@]"', "[@]")
...
def derive_quantitative_pattern(
    metapattern=None,
    dataframe=None,
    pattern=None,
    ...
    if (P_dataframe is not None) and (Q_dataframe is not None):
        try:
            ...
            logger.error(
                "Join of P_dataframe and Q_dataframe failed, overlapping columns?"
            )
            return []
...
def patterns_column_value(
    ...
    for c in columns:
        co = reduce(
            operators[pattern], [data_array[c, :], value]
        )  # Quick operations to check
        co_sum, ex_sum, conf = derive_pattern_statistics(co)
        # If it fulfills the conditions
        if (conf >= confidence) and (co_sum >= support):
            possible_expression = generate_single_expression(
                [dataframe.columns[c]], value, pattern
            )
            pandas = to_pandas_expressions(
                possible_expression, {}, parameters, dataframe
            )
            pattern_data = [
                [[pattern_name, cluster], possible_expression, [co_sum, ex_sum, conf]]
                + [pandas]
                + [""]
            ]
            yield pattern_data
...
def patterns_percentile(
    ...
    for c in columns:
        ...
        if (conf >= confidence) and (co_sum >= support):
            # Make expression
            possible_expression = generate_single_expression(
                [dataframe.columns[c]], [lower, upper], pattern
            )
            pandas = to_pandas_expressions(
                possible_expression, {}, parameters, dataframe
            )
    ...
    for c0 in tqdm(
        ...
        for c1 in Q_columns:
            ...
            if c0 != c1:
                # applying the filter
                data_filter = reduce(
                    preprocess_operator, [nonzero[c] for c in [c0, c1]]
                )
                if data_filter.any():
        ...
        for rhs_column in tqdm(
            ...
            for lhs_columns in itertools.combinations(lhs_column_list, lhs_elements):
                ...
                if data_filter.any():
                    ...
                    co = abs(np.sum(data_array[all_columns, :], axis=0)) < 1.5 * 10 ** (
                        -decimal
                    )
                    co_sum, ex_sum, conf = derive_pattern_statistics(co)
                    ...
                    if (conf >= confidence) and (co_sum >= support):
                        possible_expression = generate_single_expression(
                            ...
                            "sum",
                            [neg_col[c] for c in all_columns],
                        )
                        pandas = to_pandas_expressions(
                            possible_expression, {}, parameters, dataframe
                        )
...
def derive_ratio_pattern(
    dataframe=None, pattern_name=None, P_columns=None, Q_columns=None, parameters={}
):
    ...
    for c0 in P_columns:
        for c1 in Q_columns:
            if c0 != c1:
                ...
                data_array = map(
                    lambda e: Fraction(e).limit_denominator(limit_denominator),
                    dataframe.values[data_filter, c0]
                    / dataframe.values[data_filter, c1],
                )
...
def to_pandas_expressions(pattern, encode, parameters, dataframe):
    ...
    results = expression2pandas(res, nonzero_col, parameters)
    logger.debug(" Pandas out: " + str(results))
    return results
...
def to_dataframe(patterns=None, parameters={}, encodings={}):
    ...
    metrics = DEFAULT_METRICS.copy()
    for metric in parameters.get("metrics", []):
        if (metric not in metrics) and (metric in ALL_METRICS):
            metrics.append(metric)
...
def derive_results(
    ...
    if (dataframe is not None) and (df_patterns is not None):
        ...
        for idx in tqdm(
            iterable=df_patterns.index,
            total=df_patterns.shape[0],
            disable=disable,
            position=0,
            leave=True,
        ):
            pandas = df_patterns.loc[idx, PANDAS]
            encode = df_patterns.loc[idx, ENCODINGS]
            cluster = df_patterns.loc[idx, CLUSTER]
            metrics = [
                metric for metric in ALL_METRICS if metric in df_patterns.columns
            ]

            # in case we have clusters, we need to alter the data
            if df_patterns.loc[idx, CLUSTER] != 0:
                df = df_tot[
                    df_tot[metapatterns[0]["cluster"]] == df_patterns.loc[idx, CLUSTER]
                ]
            else:
                df = df_tot
            ...
            try:
                results_co = eval(
                    pandas["X and Y"] if "X and Y" in pandas.keys() else pandas["X"],
                    encodings,
                    {
                        "df": df,
                        "MAX": np.maximum,
                ...
                # Get it in the rich format, list if necessary
                ...
                for i in results_ex:
                    ...
                    if colp != None:
                        ...
                        if isinstance(
                            values_p, pd.Series
                        ):  # If we have a pandas series we might have duplicate indices
                            if len(values_p) > 1:
                    ...
                    results.append(
                        ...
                        + [df_patterns.loc[idx, metric] for metric in metrics]
                        + [
                ...
                logger.error("Error in analyze: UNKNOWN")
...
# def read_excel(filename=None, dataframe=None, sheet_name="Patterns"):
...
#         pandas = ast.literal_eval(df.loc[row, PANDAS])
#         error = df.loc[row, ERROR]
#         patterns.append(
#             [[df.loc[row, PATTERN_ID], 0], pattern_def, [0, 0, 0], pandas, error]
#         )
        ...
        if len(p_columns) > 2:  # only
            ...
            for subset in p_subsets:
                ...
                for i, row2 in enumerate(df_patterns.index):
                    ...
                    if (set(q_columns2) == set(q_columns)) and (
                        len(p_columns2) == len(P_dict.keys())
                    ):
                        equal = True
                        for key in P_dict.keys():
                            if key not in p_columns2:
                                ...
                                if P_dict[key] not in p_item2:
                                    ...
                                    if (
                                        P_dict[key]
                                        != p_item2[p_item2.index(P_dict[key])]
                                    ):
                                        equal = False
                        if equal:
                            if (
                                df_patterns.loc[row, CONFIDENCE]
                                <= df_patterns.loc[row2, CONFIDENCE]
                            ) and (
                                df_patterns.loc[row, SUPPORT]
                                <= df_patterns.loc[row2, SUPPORT]
                            ):
                                df_patterns.loc[
                                    row, "pattern status"
                                ] = "redundant with pattern " + str(row2)
    return df_patterns
