...
def expression2pandas(g, nonzero_col, parameters):
    ...
    if item is not None:
        if both_ways:  # this is casual confidence?
            # we have an if-then rule with and and-construction
            ...
            # we should delete this option (both_ways)
            # as this is a way to calculate casual confidence
            # temporary workaround:
            results["X"] = "df[(" + var_X + ") | ~(" + var_X + ")]"
            results["~X"] = "df[(" + var_X + ") | ~(" + var_X + ")]"
            # results["X"] = "df[(" + var_X + ")"
            # results["~X"] = "df[~(" + var_X + ")"
            results["Y"] = "df[(" + var_Y + ")]"
