...
def superhuman_calibration(df_calibrate, df_test, deployment_name='Integration'):
    ...
    if uat_model is not None and uat_calibration_data is not None:
        ...
        if uat_model is not None:
            logger.warning("Current UAT model doesn't have calibration data, so may being replaced. If this is not desired republish the UAT model in the platform.")
