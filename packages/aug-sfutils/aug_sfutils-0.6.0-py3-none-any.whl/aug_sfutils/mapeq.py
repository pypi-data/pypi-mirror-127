...
# Trivial case
    ...
    psi_sign = -eqm.orientation*cocos.sigma['bp'][jcoco]
...
#        fout = interp1d(PFL[i], label_out[i], fill_value="extrapolate")
#        (sep_out, mag_out) = fout([PSIX[i], PSI0[i]])
        mag_out, sep_out = np.interp([PSI0[i], PSIX[i]],  PFL[i], label_out[i])
        if lbl_in != lbl_out:
#            fin  = interp1d(PFL[i], label_in[i] , fill_value="extrapolate")
#            (sep_in , mag_in ) = fin( [PSIX[i], PSI0[i]])
...
# COCOS, Eq. 19, 24
    ...
    sigma_ip = sign_ip*cocos.sigma['rphiz'][jcoco]
    sigma_b0 = sign_b0*cocos.sigma['rphiz'][jcoco]
    phi_sign = sigma_b0
    psi_sign = sigma_ip*cocos.sigma['bp'][jcoco]
...
# n_line = 201 <=> 5 mm, but then there's interpolation!
    ...
    if coord_in == 'Psi':
        ...
        rho_line *= -eqm.orientation*cocos.sigma['bp'][jcoco]
        rho      *= -eqm.orientation*cocos.sigma['bp'][jcoco]
    else:
