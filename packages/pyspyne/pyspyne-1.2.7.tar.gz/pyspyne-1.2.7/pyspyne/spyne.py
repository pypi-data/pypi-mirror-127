...
class Spyne:
    ...
    class Math:
        ...
        class Polynomial:
            def __init__(self, degree: int, coefs: list, space=False):
                degree_list = [x for x in range(0, degree+1)]
                degree_list.reverse()
                self.string = ""
                arr_prev = [f"{coefs[v]}x{degree_list[v]}" for v in range(0, len(degree_list)) if coefs[v] != 0]
                for _ in arr_prev:
                    if _[0] == "1":
                        arr_prev[arr_prev.index(_)] = "x" + _[1:]
                    if _[-1] == "1":
                        arr_prev[arr_prev.index(_)] = _.split("x")[0] + "x"
                    elif _[-1] == "0":
                        arr_prev[arr_prev.index(_)] = _.split("x")[0]
                if space:
                    self.string = " + ".join(arr_prev)
                else:
                    self.string = "+".join(arr_prev)
