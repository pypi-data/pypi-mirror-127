        ...
        for q_text in cluster_dic:
            ...
            for h_text in cluster_dic[q_text]:
                ...
                for el in cluster_dic[q_text][h_text]['cluster_elements']:
                    ...
                    #user_node = self.db.create_or_update_node('User', uid=el[5] if el[5] else 'Unknown', client_id=client_id, anonymous=el[6])
                    user_node = self.db.find_node('User', uid=el[5] if el[5] else 'Unknown')
                    if user_node is None:
                        user_node = self.db.add_node('User', uid=el[5] if el[5] else 'Unknown', client_id=client_id, anonymous=el[6])
                    else:
                        user_node.client_id = client_id
                        user_node.anonymous = el[6]
                        user_node.process_datetime = datetime.now()
                        user_node.save()
