...
class Photo:
    ...
    def compare_images_from_image(self, master=None, received=None, mask_coordinates=None):
        if type(master) == bytes and len(master) != 0 and type(received) == bytes and len(received) != 0:
            try:
                self.image_path_master = BytesIO(master)
                self.image_path_grab = BytesIO(received)
                if mask_coordinates not in (None, 'None'):
                    mask_name, mask_coordinates = self.create_mask(None, ast.literal_eval(mask_coordinates))
                    mask_coordinates.load()
                else:
                    mask_coordinates = None
                self.images_read(convert=False)
                return self.compare_image(self.image_data_master, self.image_data_grab, mask_coordinates)
    ...
    def image_crop(self, image, crop_coordinate=None):
        ...
        try:
            if crop_coordinate is not None and type(crop_coordinate) == list and len(crop_coordinate) == 4:
    ...
    def convert_image_to_txt_with_ocr(self, data, crop_coordinate=None, lang='eng'):
        try:
            ...
            if crop_coordinate is not None and type(crop_coordinate) == list:
            ...
            print(f'OCR read operation successful for {data if type(data) == str else None} ---> String:###{txt}###')
            return txt
        except Exception as error:
            print_error(error, locals())
            return None
    ...
    def images_check(self, data=None, blank_image_color=None, convert=False):
        try:
            ...
            elif type(data) == bytes and len(data) != 0:
                data = Image.open(BytesIO(data))
            elif type(data) == str and os.path.exists(path=data):
                data = self.image_read(data)
            elif (
                    type(data) == Image.Image or type(data) == ImageFile or
                    type(data) == PngImagePlugin.PngImageFile or type(data) == BmpImagePlugin.BmpImageFile
            ):
                pass
            else:
    ...
    def image_save(self, data, path=None, message=None, transpose=None, blank_image_color=None, convert=True):
        try:
            byte_io = None
    ...
    def create_mask(self, mask_name=None, coordinates=None):
        ...
        try:
            ...
            # print(f'-------------> Invert={invert}, Invert Type={invert_type}, Number of Coordinate: {xy_value}')
            ...
            for i in range(xy_value):
                try:
                    ...
                    print(f'-------------> CreateMask Error Name:{mask_name} --- Coordinates:{coordinates} Errors:{e}')
            img_base.load()
    ...
    def create_image_with_text(self, text='Failed to grab image from TV.'):
        try:
            ...
            draw.text(((self.width / 2) - 66 - 10, self.height / 2 - ((len(text) / 33) * 2) - 10), text_new, 'white',
                      font)
            image.load()
            del draw
        except Exception as error:
    ...
    def __difference(self, grabbed_image, master_image):
        ...
        if not images_match:
            ...
            for i in range(len(grabbed_data)):
                grab_r, grab_g, grab_b = grabbed_data[i]
                master_r, master_g, master_b = master_data[i]
                is_different = not ((grab_r == master_r) and (grab_g == master_g) and (grab_b == master_b))
                if is_different:
                    diff = (255, 0, 0)
                else:
                    diff = (255, 255, 255)
    ...
    def compare_image(self, master, grabbed, mask):
        ...
        if not result:
            self.image_save(difference, self.image_path_diff, f'Diff Picture Created:{self.image_path_diff}')
    ...
    def screen_matches(
            self, specific, sub_name, mask, does_not_match, path_result, path_master, path_master_common,
            path_master_public
    ):
        try:
            if specific is None:
            ...
            elif type(specific) == list and len(specific) == 2:
                ...
                if specific[-9:] != '__ref.png':
                    self.image_path_master = specific[:-4] + '__ref.png'
                if not os.path.exists(self.image_path_master):
                    try:
                        ...
                        print(f':::::> COPY PROBLEM!!!: {specific} ---> {self.image_path_master} :::> {error}')
            elif specific == 'Video':
                self.image_path_master = os.path.join(path_result, 'Video_REF.png')
                self.image_save(self.images_check(blank_image_color='black'), self.image_path_master, convert=True)
            else:
                self.image_path_master = os.path.join(
            ...
            if mask is None:
                ...
                self.image_path_mask, img_base = self.create_mask(f'{self.image_path_grab[:-4]}_MASK.png', mask)
            self._result_boolean = None
            if not os.path.exists(self.image_path_master) or os.path.getsize(self.image_path_master) == 0:
                print(f'-------------> ####### Master Picture NONE! ####### ---> Master Path:{self.image_path_master}')
            elif not os.path.exists(self.image_path_grab) or os.path.getsize(self.image_path_grab) == 0:
                ...
                self._result_boolean, difference = self.compare_image(master, grabbed, mask)
                ...
                if mask is not None:
                    mask.close()
                if does_not_match:
                    print(f'-------------> Picture Compare Does Not Match Before:{sub_name} :::>{self._result_boolean}')
                    self._result_boolean = False if self._result_boolean else True
            print(f'-------------> Picture Compare: Master Path:{self.image_path_master}')
            print(f'-------------> Picture Compare: Result Path:{self.image_path_grab}')
            print(f'-------------> Picture Compare: {sub_name} :::> : {self._result_boolean}')
        except Exception as error:
            print_error(error, locals())
        return [
            sub_name, self._result_boolean, self.image_path_master, self.image_path_grab,
            self.image_path_mask, self.image_path_diff, self.image_path_master, self.image_path_grab
        ]
