...
class RemoteClient:
    ...
    def __get_commit_change_list(self, data):
        files = []
        if type(self.__ignore_files) == str:
            self.__ignore_files = [self.__ignore_files]
        if type(self.__ignore_dirs) == str:
            self.__ignore_dirs = [self.__ignore_dirs]
        for (status, file) in data:
            _file = file[1:].split('/')
            if type(self.__ignore_dirs) == list and any(i in _file for i in self.__ignore_dirs):
    ...
    def get_commits(
        ...
        if self.__revision_from:
            assert type(self.__revision_from) == int, 'Value must be integer'
        if self.__revision_to:
            assert type(self.__revision_to) == int, 'Value must be integer'
