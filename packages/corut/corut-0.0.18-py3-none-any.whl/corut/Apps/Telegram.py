...
async def message_sender(to, text, attachments):
    try:
        ...
        if attachments is not None:
            for file_path in (attachments if type(attachments) == list else [attachments]):
        ...
        file_rw(
            ...
            data=f'TEXT = """\n{message}\n"""' +
                 str(SENDER_ON_THREAD).format(
                     ...
                     self.api_id,
                     self.api_hash,
                     (to_list if type(to_list) == list else [to_list]),
                     (
                         attachments if type(attachments) == list else [attachments]
