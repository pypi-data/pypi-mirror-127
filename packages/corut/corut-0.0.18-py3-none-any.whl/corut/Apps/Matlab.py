...
def create_chart_pie(data, out_file=None, *args, **kwargs):
    ...
    """
    try:
        assert (type(data) == dict), "All values must be entered as dictionary."
        ax = {}
        fig = pyplot.figure()
...
def create_chart_bar(
    ...
    """
    try:
        assert (type(y_data) == list), "Must be [1, 2, 3, 4, ...]"
        assert (
                type(bar_label) == str and type(bar_color) == str and type(bar_value_color) == str
...
def create_chart_line(
    ...
    try:
        assert (type(data) == list), "Must have list value. For example [[x, y, label], ...]"
        assert (
                type(set_plot_text) == list
        ), "Must have list value. For example [[x, y, value], ...]"
        assert (type(max_y) == float), "Must have float value"
        assert (
                type(line_extra_y) == list
        ), "Must have list value. For example [[15, 'green'], [30, 'orange']]"
        assert (
                type(label_x) == str and type(label_y) == str and type(title) == str
...
class Matlab:
    create_chart_pie = create_chart_pie
    create_chart_bar = create_chart_bar
    create_chart_line = create_chart_line
