...
class Client:
    ...
    def launch_job(
        ...
        response = cast(
            ...
            self._launch_job(
                LaunchJobMode.BLOCKING, qualified_name, uuid.uuid4(), args, kwargs
            ),
        )
    ...
    async def launch_async_job(
        ...
        response = cast(
            ...
            self._launch_job(
                LaunchJobMode.ASYNC, qualified_name, uuid.uuid4(), args, kwargs
            ),
        )
    ...
    def launch_fire_and_forget_job(
        ...
        task_id = uuid.uuid4()
        self._launch_job(
            LaunchJobMode.FIRE_AND_FORGET, qualified_name, task_id, args, kwargs
        )
    ...
    def _build_request(
        ...
        if mode_is_fire_and_forget(mode):
            request = remote_pb2.CreateTaskRequest(job=job, sdk_version=__version__)
    ...
    def _launch_job(
        ...
        request = self._build_request(mode, qualified_name, task_id, args, kwargs)
