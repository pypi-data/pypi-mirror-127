...
class Pipeline(Component, Visible, WorkspaceTelemetryMixin):
    ...
    def _validate_parameters(self, parameters):
        ...
        Raise exception if there are unused parameters which have been optimized out, but pipeline pass them.
        And raise exception if pipeline don't pass used and required parameters.
        ...
        :rtype: dict
        """
        result = super()._validate_parameters(parameters)
        if isinstance(self._definition, PipelineComponentDefinition):
            # Raise error if there are unused parameters which have been optimized out,
            # but pipeline outside pass them.
            for _k, _v in parameters.items():
                if _k not in self._definition._pipeline_parameters:
                    raise UserErrorException(f"Pipeline definition {self._definition.name} optimized out parameter "
                                             f"{_k} which can't be passed by pipeline {self.name}")

            # Raise error if don't pass used and required parameters.
            for _k, _v in self._definition._parameters.items():
                if not _v.optional and _v.default is None and _k not in parameters:
                    raise UserErrorException(f"Required parameter {_k} not provided in pipeline {self.name}")
        return result
    ...
    def set_inputs(self, *args, **kwargs) -> 'Component':
        ...
        kwargs = super()._validate_parameters(kwargs)
    ...
    def _expand_default_pipeline_parameters(self, pipeline_parameters):
        ...
        if pipeline_parameters is None:
            pipeline_parameters = {}
        super()._validate_parameters(pipeline_parameters)
