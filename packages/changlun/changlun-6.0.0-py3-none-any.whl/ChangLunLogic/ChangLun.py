...
class ChangLun:
    ...
    def isPenBuyPoint(self):
        ...
        if lastPen['type'] == PenType.down and lastPen['endOrginIndex'] == len(self.histData) - 2:
            return True
        return False
    ...
    def isLastPartingDown(self):
        if self.partingList is None or len(self.partingList) == 0:
            return False
        lastParting = self.partingList[-1]
        if lastParting['type'] == PartingType.down and lastParting['simplifyIndex'] == len(self.simplifyData) -2:
            return True
        return False
    ...
    def genCentreFeature(self,freq):
        if not self.centreDic.__contains__(freq):
            return {}
    ...
    def genFeature(self):
        ...
        feature['buy_date'] = self.penList[-1]['endDate']
        feature['buy_value'] = self.penList[-1]['endValue']
