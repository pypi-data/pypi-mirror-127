...
def _prepare_model_to_fold_BN(model, map_neural_layer_to_bn,
    ...
    for layer_bn in map_neural_layer_to_bn.values():
        model._self_tracked_trackables.remove(layer_bn)  # pylint: disable=protected-access
...
def load_partial_weights(dest_model, src_model):
    ...
    Args:
        dest_model(:obj:`keras.Model`): the destination Model
        src_model(:obj:`keras.Model`): the source Model
...
def create_trainable_quantizer_model(quantized_model):
    ...
    Args:
        quantized_model(str, :obj:`keras.Model`): a keras Model or a
