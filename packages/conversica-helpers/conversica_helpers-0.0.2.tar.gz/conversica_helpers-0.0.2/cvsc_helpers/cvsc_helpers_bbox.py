...
import re, time, csv, pysftp, os, subprocess, glob, json, shutil, requests, chardet, asyncio, aiohttp
...
def new_field_loot(row, loot, lookup_field, new_field, mode=None, cache=None, **kwargs):
    ...
    try:
      ...
      print_log(f"Look up table does not contain data for {row[lookup_field]} to be used in new field {new_field}.", level=2, **kwargs)
      print_log(f"{mode} {new_data}")
...
def upload_backup(config, debug_mode=True):
  ...
  if os.path.isfile(config.csv_output):
    ...
    rtv = upload_shell(config, filename=csv_stored)
  # tmp update
...
def write_csv(file, data,  encoding, fieldnames, delimiter, rename_headers={}, limit=None, **kwargs):
...
def upload_api(api_data, row, debug=False):
  ...
  try:
    ...
    #r = requests.post("https://bluebox.gonmalo.dev/api_dev/test", data=json.dumps(row),  headers=headers)
    if r.status_code == 200: rtv = True

  except Exception as e:
    print(e)
    return rtv, 500, e
  return rtv, r.status_code, r.text

async def upload_api_v2(api_data, row, session):
  rtv = False
  async with session.post("http://localhost:5000/dummy_ep", json=row, auth=aiohttp.BasicAuth(api_data['user'], api_data['password'], encoding="utf8")) as response:
