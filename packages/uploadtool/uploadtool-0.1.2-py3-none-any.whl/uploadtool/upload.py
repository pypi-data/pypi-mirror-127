...
def Command(info, cmd, params=''):
	ctypes.windll.shell32.ShellExecuteW(None, "runas", cmd, params, None, 0)
...
class UploadTool(UploadInfo):
	...
	def BuildSetup(self):
		...
		bat_path = join(self.path, bat_name)
		f = open(bat_path, 'w')
		f.write("python {} bdist_wheel\n".format(setup_path))
		f.write("python {} sdist".format(setup_path))
		f.close()
		Command(self, bat_path)
		UploadByTwine(self)
