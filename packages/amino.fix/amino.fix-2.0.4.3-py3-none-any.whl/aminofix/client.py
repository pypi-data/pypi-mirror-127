...
class Client(Callbacks, SocketHandler):
        ...
        **Returns**
        ...
        if doNotDisturb is not None:
            if doNotDisturb:
                ...
                if if json.loads(response.text)["api:statuscode"] != 0: res.append(exceptions.CheckException(json.loads(response.text)))
    ...
    def purchase(self, objectId: str, isAutoRenew: bool = False):
        ...
        response = requests.post(f"{self.api}/g/s/store/purchase", headers=self.parse_headers(), data=data)
