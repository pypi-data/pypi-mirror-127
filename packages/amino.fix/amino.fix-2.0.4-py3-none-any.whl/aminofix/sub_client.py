...
class SubClient(client.Client):
    ...
    def get_store_chat_bubbles(self, start: int = 0, size: int = 25):
        response = requests.get(f"{self.apis}/x{self.comId}/s/store/items?sectionGroupId=chat-bubble&start={start}&size={size}", headers=self.parse_apis_headers())
        if json.loads(response.text())["api:statuscode"] != 0: return exceptions.CheckException(json.loads(response.text()))
        else: return objects.ChatBubble(json.loads(response.text())["storeItemList"]).ChatBubble

    def get_store_avatar_frames(self, start: int = 0, size: int = 25):
        response = requests.get(f"{self.apis}/x{self.comId}/s/store/items?sectionGroupId=avatar-frame&start={start}&size={size}", headers=self.parse_apis_headers())
        if json.loads(response.text())["api:statuscode"] != 0: return exceptions.CheckException(json.loads(response.text()))
        else: return objects.AvatarFrame(json.loads(response.text())["storeItemList"]).AvatarFrame

    # TODO : Finish this (Fineshed :D)
    def get_store_stickers(self, start: int = 0, size: int = 25):
        response = requests.get(f"{self.apis}/x{self.comId}/s/store/items?sectionGroupId=sticker&start={start}&size={size}", headers=self.parse_apis_headers())
        if json.loads(response.text())["api:statuscode"] != 0: return exceptions.CheckException(json.loads(response.text()))
        else: return objects.StoreStickers(json.loads(response.text())["storeItemList"]).StoreStickers
