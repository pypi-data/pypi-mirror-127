...
def verify_function_input_type(func: typing.Callable):
    ...
    if input_type in basic_types:
        return
    if typing_inspect.get_origin(input_type) in basic_types:
        return
    if typing_inspect.get_origin(input_type) in special_types:
        return
...
def verify_incoming_rpc_call_input_type(msg, rpc_method: str, rpc_input_type_map: dict):  # pragma: no cover
    ...
    origin_type = typing_inspect.get_origin(it)
...
def is_pydantic(cls):  # pragma: no cover
    if cls not in basic_types:
        if not typing_inspect.get_origin(cls) in basic_types:
            if not typing_inspect.get_origin(cls) in special_types:
