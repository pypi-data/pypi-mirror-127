...
def apple_time_converter(time):
    return time / 1000000 + 975542400000


def datefixer(df):
    df['date'] = df['date'].apply(apple_time_converter)
    df['date_read'] = df['date_read'].apply(apple_time_converter)
    df['date_delivered'] = df['date_delivered'].apply(apple_time_converter)
    return df
