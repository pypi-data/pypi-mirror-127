...
class Markovitz(object):
    ...
    def drawing(self):
        ...
        plt.xlim(df_scatter.risk.min()*0.9, df_scatter.risk.max()*1.1)
        plt.ylim(df_scatter.rate.min()*0.9, df_scatter.rate.max()*1.1)
        plt.xlabel('Risk')
        plt.ylabel('Yield')
        plt.show()
...
if __name__ == '__main__':
    ...
    mk = Markovitz(names=['比亚迪', '阳光电源', '璞泰来', '紫光国微', '盛新锂能'],  # 股票组合
                   ...
                   funds=4500000,  # 最大资金限制
                   path=r'C:\Users\19319\Desktop\sly\Data'
                   )
    print(mk.weight_tests(number=3))
