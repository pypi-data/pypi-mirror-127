...
def get_version():
    from acuitylib.utils import get_acuity_path, get_acuity_path_env_variable
    if os.path.exists(get_acuity_path()):
        acuity_path = get_acuity_path()
    else:
        acuity_path = get_acuity_path_env_variable()
    acuity_version_path = os.path.join(acuity_path, 'acuitylib', 'VERSION')
    try:
