...
def write_to_json(result, path):
    with open('/home/dell/PycharmProjects/IGESQL/source_data/splash/test_precessed.json', 'r+') as read:
        ...
        for i, j in enumerate(lines):
            ...
            temp['feedback'] = j['feedback']
            temp['init_sql'] = j['predicted_parse_with_values']
            temp['tgt_sql'] = j['gold_parse']
            temp['init_tree'] = j['predict_tree']
            temp['tgt_tree'] = j['gold_tree']
            temp['tgt_edits'] = copy.deepcopy(j['edits'])
            if i in line:
                temp['predict_edits'] = j['edits']
                new.append(temp)
                continue
            else:
                length = len(j['edits'])
...
# write_to_json(1, './g-IGESQL/T.json')
