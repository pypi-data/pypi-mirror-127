...
class TestServicesList:
    ...
    def test_click_calls_execution_method(
        ...
        mock_get_services_data.assert_called_once_with(DEFAULT_API_URL)
    ...
    def test_services_list_not_authorized_errors(self, mock_get_services):
        with pytest.raises(NotAuthorizedError, match="symflow login"):
            get_services_data(DEFAULT_API_URL)
    ...
    def test_services_list(self, mock_get_services):
        data = get_services_data(DEFAULT_API_URL)
