...
def link_16s(args, config_dict):
    ...
    if no_cluster is False:
        report_and_log(('Round 1: Cluster at %s%s identity and keep only the longest one in each cluster' % (cluster_iden, '%')), pwd_log_file, keep_quiet)
...
10. exit if no 16S passed QC
11. if all MAGs are linked
