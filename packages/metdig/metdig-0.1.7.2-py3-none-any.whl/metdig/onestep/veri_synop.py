...
def veri_gust10m(data_source='cassandra',
    ...
    gust10m_fcst = get_model_grid(data_source=data_source, init_time=init_time, fhour=fhour, data_name=data_name, extent=map_extent, x_percent=0, y_percent=0,
                              var_name='gust10m', throwexp=False)
    if(gust10m_fcst is None):                              
        gust10m_fcst = get_model_grid(data_source=data_source, init_time=init_time, fhour=fhour, data_name=data_name, extent=map_extent, x_percent=0, y_percent=0,
                                var_name='gust10m_3h', throwexp=False)
    if(gust10m_fcst is None):
        gust10m_fcst = get_model_grid(data_source=data_source, init_time=init_time, fhour=fhour, data_name=data_name, extent=map_extent, x_percent=0, y_percent=0,
                                var_name='gust10m_6h', throwexp=False)
    if(gust10m_fcst is None):
    ...
    for ivld in range(0,gust10m_fcst.attrs['valid_time']):
        ...
        temp = get_obs_stations(obs_time=obs_time,data_name='sfc_chn_hor',var_name='gust10m',data_source=data_source,level=None,extent=map_extent,is_save_other_info=None).dropna()
        tempdir = get_obs_stations(obs_time=obs_time,data_name='sfc_chn_hor',var_name='gustdir10m',data_source=data_source,level=None,extent=map_extent,is_save_other_info=None).dropna()
        if (ivld==0):
            gust10m_obs=temp
    ...
    gust10m_obs=gust10m_obs.where(gust10m_obs.iloc[:,6]>8).dropna()

    if is_return_data:
        dataret = {'gust10m_fcst': gust10m_fcst,
                    'gust10m_obs': gust10m_obs,
                    'gustdir10m_obs':gustdir10m_obs}
        ret.update({'data': dataret})
    if is_draw:
...
if __name__ == '__main__':
    ...
    veri_gust10m(init_time='2021110608',fhour=24,area='华北',data_source='cmadaas',data_name='ecmwf',
                mingust_obs=8,add_city=False,
                gust10m_pcolormesh_kwargs={'levels':[8.0, 10.8, 13.9, 17.2, 20.8, 24.5, 28.5, 32.7, 37, 41.5, 46.2, 51.0, 56.1, 61.3]},
                    # gust10m_barb_kwargs={'clim':(30,100)}
            )
    plt.show()        