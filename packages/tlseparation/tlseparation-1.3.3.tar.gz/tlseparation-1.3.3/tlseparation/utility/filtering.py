...
import numpy as np
from tlseparation.utility import knnsearch
from tlseparation.utility import data_utils
from tlseparation.utility import shortpath
from sklearn.neighbors import NearestNeighbors
from sklearn.cluster import DBSCAN
from tlseparation.classification.point_features import svd_evals, knn_features, curvature
...
def feature_filter(arr, feature_id, threshold, knn):
    ...
    nbrs_idx = knnsearch.set_nbrs_knn(arr, arr, knn, False)
...
def plane_filter(arr, rad, threshold):
    ...
    nbrs_idx = knnsearch.set_nbrs_rad(arr, arr, rad, False)
...
def radius_filter(arr, radius, min_points):
    ...
    indices = knnsearch.set_nbrs_rad(arr, arr, radius, return_dist=False)
...
def continuity_filter(wood, leaf, rad=0.05):
    ...
    G = shortpath.array_to_graph(arr, base_id, 3, 100, 0.05, 0.02, 0.5)
    node_ids, dist = shortpath.extract_path_info(G, base_id, return_path=False)
    ...
    while e > e_threshold:
        ...
        gp = np.vstack((gp, pts))
        gp = data_utils.remove_duplicates(gp)
    ...
    wood = data_utils.remove_duplicates(gp)
    not_wood = data_utils.get_diff(wood, arr)
...
def array_majority(arr_1, arr_2, **kwargs):
 ...
 input a value for either "knn" or "rad".'
    ...
    if 'knn' in kwargs.keys():
        indices = knnsearch.set_nbrs_knn(arr, arr, kwargs['knn'], return_dist=False)
    elif 'rad' in kwargs.keys():
        indices = knnsearch.set_nbrs_rad(arr, arr, kwargs['rad'], return_dist=False)
...
def dist_majority(arr_1, arr_2, **kwargs):
 ...
 input a value for either "knn" or "rad".'
    ...
    # in the combined array. Function used is based upon the argument passed.
    if 'knn' in kwargs.keys():
        dist, indices = knnsearch.set_nbrs_knn(arr, arr, kwargs['knn'])
    elif 'rad' in kwargs.keys():
        dist, indices = knnsearch.set_nbrs_rad(arr, arr, kwargs['rad'])
