    ...
    if not response_get["status"]:
        ...
        if response_get["status_code"] == 404:
            ...
            if not response_put["status"]:
                hub.log.debug(f"Could not create Disks {response_put['comment']}")
                return StateReturn(
            ...
            hub.log.debug(f"Could not get Disks {response_get['comment']}")
        ...
        if not response_patch["status"]:
            hub.log.debug(f"Could not update Disks {response_patch['comment']}")
            return StateReturn(
    ...
    **Autogenerated function**
    ...
    if response_get["status"]:
        ...
        if not response_delete["status"]:
            hub.log.debug(f"Could not delete Disks {response_delete['comment']}")
            return StateReturn(
