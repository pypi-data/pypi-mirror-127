...
PRESENT_REQUEST_FORMAT = r"""
    ...
    if not response_get["status"]:
        ...
        if response_get["status_code"] == 404:
            ...
            if not response_put["status"]:
                hub.log.debug(
                    f"Could not create {{ function.hardcoded.resource_name }} {response_put['comment']}"
                )
                return StateReturn(
            ...
            hub.log.debug(
                f"Could not get {{ function.hardcoded.resource_name }} {response_get['comment']}"
            )
            return StateReturn(
                name=name, result=False, comment=response_get["comment"]
            )
    {% if function.hardcoded.patch_parameters %}
        ...
        if not response_patch["status"]:
            hub.log.debug(
                f"Could not update {{ function.hardcoded.resource_name }} {response_patch['comment']}"
            )
            return StateReturn(
...
ABSENT_REQUEST_FORMAT = r"""
    ...
    if response_get["status"]:
        ...
        if not response_delete["status"]:
            hub.log.debug(
                f"Could not delete {{ function.hardcoded.resource_name }} {response_delete['comment']}"
            )
            return StateReturn(
