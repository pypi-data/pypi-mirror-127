...
full_deviation_dx0 = np.array([-0.01386023, 0.06957879])
full_amplitude = 0.01
full_omega = 0.0236
full_phase = 3
def full_du(t):
...
nominal_input_signal_prediction = DiscreteSignal(dynamics.input_dimension, 'Nominal u', total_time_prediction, frequency, signal_shape='External', data=u_nominal(tspan_prediction))
nominal_output_signal_training = DiscreteSignal(dynamics.output_dimension, 'Nominal x', total_time_training, frequency, signal_shape='External', data=nominal_output_signal_prediction.data[:, 0:number_steps_training])
...
# Input signal
prediction_deviation_input_signal = DiscreteSignal(dynamics.input_dimension, 'Prediction Deviation Input Signal', total_time_prediction, frequency, signal_shape='External', data=test_du(tspan_prediction))
prediction_input_signal = ContinuousSignal(dynamics.input_dimension, 'Prediction Input Signal', signal_shape='External', u=test_u)

# Prediction
half_window_size = 10
starting_step = int(frequency * total_time_training)
selected_for_propagation, output_signal_predicted = prediction(nominal_reference, identified_system, nominal, identified_system, prediction_deviation_input_signal, starting_step, left_window_size=half_window_size, right_window_size=half_window_size)
