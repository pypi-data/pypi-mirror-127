...
Parameters are considered as input
True system is discrete-time
"""
...
from ClassesDynamics.ClassDuffingOscillatorDynamics import DuffingOscillatorDynamics7
from SystemIDAlgorithms.DepartureDynamics import departureDynamicsDiscrete
from ClassesGeneral.ClassSystem import DiscreteLinearSystem, DiscreteNonlinearSystem
...
## Import Dynamics
dt_dynamics = 0.01
dynamics = DuffingOscillatorDynamics7(dt_dynamics)



## Parameters for identification
total_time = 50
frequency = np.round(1 / dt_dynamics)
dt = 1 / frequency
...
## Number Experiments
additional_experiments = 4
number_free_decay_experiments = q * dynamics.input_dimension + additional_experiments
number_forced_response_experiments = round(dynamics.input_dimension + max(p + 1 + q - 1, deadbeat_order) * (dynamics.input_dimension + dynamics.output_dimension)) + additional_experiments



## Create System
initial_states = [(np.array([0.1, -0.2]), 0)]
nominal_system = DiscreteNonlinearSystem(np.round(1 / dt_dynamics), dynamics.state_dimension, dynamics.input_dimension, dynamics.output_dimension, initial_states, 'Nominal System', dynamics.F, dynamics.G)
...
def u_nominal(t):
    return np.array([delta(t), alpha(t), beta(t)])
nominal_input_signal = ContinuousSignal(dynamics.input_dimension, 'Nominal Input Signal', signal_shape='External', u=u_nominal)
nominal_input_signal_d = DiscreteSignal(dynamics.input_dimension, 'Nominal Input Signal', total_time, frequency, signal_shape='External', data=u_nominal(tspan))



## Nominal Output Signal
nominal_output_signal = OutputSignal(nominal_input_signal_d, nominal_system, 'Nominal Output Signal', tspan=tspan)
...
tspan_test = tspan[:-r]
# dynamics = DuffingOscillatorDynamics7(dt, tspan=tspan, nominal_x=DiscreteSignal(dynamics.state_dimension, 'Nominal Trajectory', total_time, frequency, signal_shape='External', data=nominal_output_signal.state), nominal_u=DiscreteSignal(dynamics.input_dimension, 'Nominal Input Signal', total_time, frequency, signal_shape='External', data=nominal_input_signal.u(tspan)))
tspan_test = np.round(tspan_test, decimals=2)
nominal_input_signal_d = DiscreteSignal(dynamics.input_dimension, 'Nominal Input Signal', total_time_test, frequency, signal_shape='External', data=u_nominal(tspan_test))
nominal_output_signal_test = OutputSignal(nominal_input_signal_d, nominal_system, 'Nominal Output Signal Test', tspan=tspan_test)
...
interp_du = []
random_inputs_numbers = 0.02 * np.random.randn(dynamics.input_dimension, number_steps_test, number_forced_response_experiments)
    ...
    def make_du(i):
        def du(t):
            if type(t) == np.ndarray:
                ...
                return random_inputs_numbers[:, int(t * frequency), i]
...
## Full experiment
full_deviation_dx0 = 0.05 * np.random.randn(dynamics.state_dimension)
full_amplitude = np.array([0.01, 0.01, 0.01])
full_frequency = np.array([4, 5, 6])
full_phase = np.array([1, 2, 3])
def full_du(t):
    return np.array([full_amplitude[0] * np.cos(2 * np.pi * full_frequency[0] * t + full_phase[0]),
                     full_amplitude[1] * np.cos(2 * np.pi * full_frequency[1] * t + full_phase[1]),
                     full_amplitude[2] * np.cos(2 * np.pi * full_frequency[2] * t + full_phase[2])])
...
# Test Signal
test_amplitude = np.array([0.01, 0.01, 0.01])
test_frequency = np.array([7, 8, 9])
test_phase = np.array([0, 1, 2])
def test_du(t):
    return np.array([test_amplitude[0] * np.cos(2 * np.pi * test_frequency[0] * t + test_phase[0]),
                     test_amplitude[1] * np.cos(2 * np.pi * test_frequency[1] * t + test_phase[1]),
                     test_amplitude[2] * np.cos(2 * np.pi * test_frequency[2] * t + test_phase[2])])
def test_u(t):
...
test_input_signal = DiscreteSignal(dynamics.input_dimension, 'Test Input Signal', total_time_test, frequency, signal_shape='External', u=test_u(tspan_test))



## Test System
test_system = DiscreteNonlinearSystem(frequency, dynamics.state_dimension, dynamics.input_dimension, dynamics.output_dimension, [(initial_states[0][0] + full_deviation_dx0, 0)], 'Test System', dynamics.F, dynamics.G)
...
#
# ## Plotting
plotSignals([[nominal_output_signal, true_output_signal, identified_output_signal], [subtract2Signals(true_output_signal, identified_output_signal)]], 5, percentage=0.001)
