...
Parameters delta and alpha are considered as inputs
"""
...
from ClassesDynamics.ClassDuffingOscillatorDynamics import DuffingOscillatorDynamics6
...
## Import Dynamics
dynamics = DuffingOscillatorDynamics6(beta)
...
def u_nominal(t):
    return np.array([delta(t), alpha(t)])
...
## Full experiment
full_deviation_dx0 = 0.02 * np.random.randn(dynamics.state_dimension)
full_amplitude = np.array([0.01, 0.01])
full_frequency = np.array([4, 5])
full_phase = np.array([1, 2])
def full_du(t):
...
# Test Signal
test_amplitude = np.array([0.01, 0.01])
test_frequency = np.array([7, 8])
test_phase = np.array([0, 1])
def test_du(t):
...
plotSignals([forced_response_experiments_deviated.input_signals, [nominal_output_signal_test] + forced_response_experiments.output_signals, forced_response_experiments_deviated.output_signals], 11)
plotSignals([free_decay_experiments_deviated.input_signals, free_decay_experiments_deviated.output_signals, free_decay_experiments.output_signals], 12)
