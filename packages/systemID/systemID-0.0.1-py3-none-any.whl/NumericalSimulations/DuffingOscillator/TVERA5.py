...
Parameter beta is considered as input
"""
...
from ClassesDynamics.ClassDuffingOscillatorDynamics import DuffingOscillatorDynamics5
...
## Import Dynamics
dynamics = DuffingOscillatorDynamics5(delta, alpha)
...
full_phase = 1
def full_du(t):
    return full_amplitude * np.cos(2 * np.pi * full_frequency * t + full_phase)
...
test_phase = 0
def test_du(t):
    return test_amplitude * np.cos(2 * np.pi * test_frequency * t + test_phase)
def test_u(t):
