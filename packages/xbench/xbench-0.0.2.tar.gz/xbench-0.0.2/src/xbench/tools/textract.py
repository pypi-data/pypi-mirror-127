...
class Textract(Tool):
    ...
    def add_arguments(parser):
        parser.add_argument(
            ...
            default=None,
            type=str,
            help="an existing s3 bucket to use for staging files. if not specified, xbench will try to find an existing one or create one",
        )
        parser.add_argument(
    ...
    def _init_bucket(self):
        ...
        for bucket in s3.list_buckets()["Buckets"]:
            if "xbench-staging-area" not in bucket["Name"]:
                continue
        ...
        if matching_bucket is None:
            log.info(
                "No 'xbench-staging-area' bucket exists, going to try creating one..."
            )
            matching_bucket = "xbench-staging-area-" + str(uuid4())
