...
SENT_EMAILS=[]
EMAIL_BLACKLIST=OkHTm()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",OkHTJ(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=OkHTu,from_name=OkHTu,smtp_host=OkHTu,smtp_user=OkHTu,smtp_pass=OkHTu,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if OkHTq(recipients,OkHTM)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=OkHTu):
 ...
 result["From"]=formataddr((OkHTJ(Header(from_name,"utf-8")),from_email))
 return result
