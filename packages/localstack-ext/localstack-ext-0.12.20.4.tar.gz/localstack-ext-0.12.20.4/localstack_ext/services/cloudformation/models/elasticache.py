...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=uQrWD(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not uQrWC
   return result
  except uQrWM:
   props=self.props
   ...
   if parts[0]in candidates and uQrWP(parts)>1:
