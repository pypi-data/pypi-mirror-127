...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or iuovc
 @iuovU
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return iuovL(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @iuovT
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if iuovE(origins,iuovC)else origins.get("Items")
  origins_to_create=iuovW([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if iuovE(aliases,iuovC)else aliases.get("Items")
  aliases_to_create=iuovW(aliases)
  for distr in distrs:
   origins=iuovW([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=iuovW(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",iuovA)
   config["Enabled"]=iuovj(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if iuovE(custom_errors,iuovr)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=iuovj(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":iuovA}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=iuovj(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=iuovc):
    parent=config if parent is iuovc else parent
    items=parent.get(key)or[]
    if iuovE(items,iuovr)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":iuovp(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if iuovE(cache_behavs,iuovr)else cache_behavs)
