...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[RQAJP])[0]
 def get_physical_resource_id(self,attribute=RQAJP,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if RQAJm(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return RQAJO(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @RQAJb
 def add_defaults(resource,stack_name:RQAJt):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return RQAJO(SESReceiptRule,self).get_cfn_attribute(attribute)
 @RQAJi
