...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return LszYi if e else LszYa
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->LszYV:
...
@click.option("--revert",is_flag=LszYi,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:LszYc):
...
def _cpvcs_initialized(pod_name:LszYb)->LszYc:
...
@click.option("-n","--name",help="Name of the cloud pod",required=LszYi)
def cmd_cpvcs_init(name:LszYb):
...
@click.option("-n","--name",help="Name of the cloud pod",required=LszYi)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=LszYi,default=LszYa)
def cmd_cpvcs_delete(name:LszYb,remote:LszYc):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=LszYi)
def cmd_cpvcs_register_remote(name:LszYb):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=LszYi)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=LszYi)
def cmd_cpvcs_rename(name:LszYb,new_name:LszYb):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=LszYi)
def cmd_cpvcs_commit(message:LszYb,name:LszYb):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=LszYi,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=LszYi)
def cmd_cpvcs_push(squash:LszYc,message:LszYb,name:LszYb):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=LszYI,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=LszYi,default=LszYa,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=LszYi)
def cmd_cpvcs_inject(version:LszYI,reset:LszYc,name:LszYb):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=LszYi,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=LszYi,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=LszYi)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:LszYb,inject:LszYc,reset:LszYc):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=LszYi,default=LszYa)
def cmd_cpvcs_list_pods(remote:LszYc):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=LszYi)
def cmd_cpvcs_versions(name:LszYb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=LszYi,type=LszYI)
@click.option("-n","--name",help="Name of the cloud pod",required=LszYi)
def cmd_cpvcs_version_info(version:LszYI,name:LszYb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=LszYI,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=LszYi)
def cmd_cpvcs_version_metamodel(version:LszYI,name:LszYb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=LszYi,type=LszYI,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=LszYi,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=LszYi,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=LszYa,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=LszYi)
def cmd_cpvcs_set_version(version:LszYI,inject:LszYc,reset:LszYc,commit_before:LszYc,name:LszYb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=LszYi)
def cmd_cpvcs_commits(version:LszYI,name:LszYb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=LszYi)
@click.option("--commit","-c",required=LszYi)
@click.option("-n","--name",help="Name of the cloud pod",required=LszYi)
def cmd_cpvcs_commit_diff(version:LszYI,commit:LszYI,name:LszYb):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(LszYV)
  status.stop()
...
def print_table(columns:Dict[LszYb,LszYb],rows:List[Dict[LszYb,Any]],formatters:Dict[LszYb,Callable[[Any],LszYb]]=LszYV):
 from rich.table import Table
 if formatters is LszYV:
  formatters=LszYr()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  LszYH(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->LszYV:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->LszYV:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->LszYV:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->LszYV:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
