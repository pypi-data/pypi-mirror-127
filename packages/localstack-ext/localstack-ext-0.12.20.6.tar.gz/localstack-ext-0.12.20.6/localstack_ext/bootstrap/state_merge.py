...
def _merge_helper(current,injecting,merge_strategy=tshPv,visited:Set=tshPv):
 if tshPl(current,tshPS)and tshPl(injecting,tshPS):
 ...
 for field_name,inj_field_value in inj_dict.items():
  ...
  if cur_field_value is not tshPv:
   ...
   cur_dict[field_name]=inj_field_value
 return cur_dict
def merge_object_state(current,injecting,merge_strategy=tshPv):
...
def handle_special_case(current,injecting):
 if tshPl(injecting,Queue):
  current.queues[injecting.name]=injecting
  return tshPj
 elif tshPl(injecting,FakeBucket):
  current["global"].buckets[injecting.name]=injecting
  return tshPj
def add_missing_attributes(obj:Any,safe:tshPd=tshPj,visited:Set=tshPv):
 try:
  obj_dict=get_object_dict(obj)
  if obj_dict is tshPv:
   return
  ...
  class_inst_attrs=infer_class_attributes(tshPB(obj))
  for key,value in class_inst_attrs.items():
   if key not in obj_dict:
    LOG.debug("Add missing attribute '%s' to state object of type %s"%(key,tshPB(obj)))
    obj_dict[key]=value
 except tshPu as e:
  if not safe:
   raise
  LOG.warning("Unable to add missing attributes to persistence state object %s: %s",(obj,e))
def infer_class_attributes(clazz:Type)->Dict[tshPV,Any]:
 if clazz in[tshPS,tshPJ]or not inspect.isclass(clazz)or clazz.__name__=="function":
  return{}
 constructor=tshPn(clazz,"__init__",tshPv)
 if not constructor:
  return{}
 try:
  ...
  def get_default_arg_value(arg_name,arg_index=-1):
   ...
   num_non_default_args=tshPz(sig_args.args or[])-tshPz(arg_defaults)
   offset=arg_index-num_non_default_args
   if offset in tshPC(tshPz(arg_defaults)):
  ...
  args=[]
  kwargs={}
  for arg_idx in tshPC(1,tshPz(sig_args.args)):
  ...
  result=tshPJ(instance.__dict__)
  return result
 except tshPu:
  return{}
def merge_sqllite_dbs(file_dest:tshPV,file_src:tshPV)->tshPv:
 def _merge_table(table_name:tshPV,cursor_a,cursor_b)->tshPv:
  tmp_table_name=f"'{table_name}_new'"
  table_name=f"'{table_name}'"
  select_query=f"SELECT * FROM {table_name}"
  if table_name=="'cf'":
   return
  schema=tshPD(tshPR(lambda x:x[1],cursor_b.execute(f"PRAGMA table_info({table_name})")))
  params=f"({('?,' * len(schema))[:-1]})"
  insert_str=f"INSERT INTO {tmp_table_name} {str(schema)} values {params}"
  if table_name=="'dm'":
   excl_tables=(tshPV(tshPS(tshPR(lambda x:x[0],cursor_a.execute(f"SELECT TableName FROM {table_name}")))).replace("[","(",1).replace("]",")",1))
 ...
 with contextlib.closing(sqlite3.connect(file_dest))as db_src,contextlib.closing(sqlite3.connect(file_src))as db_target:
  ...
  table_names=tshPS(tshPR(lambda x:x[0],cursor_dest.execute("SELECT name FROM sqlite_master WHERE type='table'")))
...
def merge_kinesis_state(path_dest:tshPV,path_src:tshPV)->tshPd:
 ...
 if not os.path.isfile(tmp_dir_statefile):
  ...
  return tshPQ
 with tshPN(datadir_statefile)as datadir_kinesis_file,tshPN(tmp_dir_statefile)as tmp_dir_kinesis_file:
  ...
  if tshPz(tmp_dir_streams)>0:
   ...
   with tshPN(datadir_statefile,"w")as mutated_datadir_kinesis_file:
