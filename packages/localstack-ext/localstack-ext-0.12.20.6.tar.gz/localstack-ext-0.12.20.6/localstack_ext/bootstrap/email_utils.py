...
SENT_EMAILS=[]
EMAIL_BLACKLIST=PgTGU()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",PgTGw(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=PgTGM,from_name=PgTGM,smtp_host=PgTGM,smtp_user=PgTGM,smtp_pass=PgTGM,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if PgTGf(recipients,PgTGi)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=PgTGM):
 ...
 result["From"]=formataddr((PgTGw(Header(from_name,"utf-8")),from_email))
 return result
