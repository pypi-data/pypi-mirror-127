...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[aIhnf])[0]
 def get_physical_resource_id(self,attribute=aIhnf,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if aIhne(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return aIhnR(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @aIhnY
 def add_defaults(resource,stack_name:aIhnt):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return aIhnR(SESReceiptRule,self).get_cfn_attribute(attribute)
 @aIhnA
