...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or bNjkU
 @bNjkm
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return bNjkx(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @bNjkr
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if bNjkD(origins,bNjkt)else origins.get("Items")
  origins_to_create=bNjkQ([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if bNjkD(aliases,bNjkt)else aliases.get("Items")
  aliases_to_create=bNjkQ(aliases)
  for distr in distrs:
   origins=bNjkQ([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=bNjkQ(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",bNjkg)
   config["Enabled"]=bNjkO(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if bNjkD(custom_errors,bNjks)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=bNjkO(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":bNjkg}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=bNjkO(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=bNjkU):
    parent=config if parent is bNjkU else parent
    items=parent.get(key)or[]
    if bNjkD(items,bNjks)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":bNjkf(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if bNjkD(cache_behavs,bNjks)else cache_behavs)
