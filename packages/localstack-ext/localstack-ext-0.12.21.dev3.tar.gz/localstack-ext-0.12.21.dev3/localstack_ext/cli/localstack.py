...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return tLciA if e else tLciC
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->tLcir:
...
@click.option("--revert",is_flag=tLciA,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:tLciz):
...
def _cpvcs_initialized(pod_name:tLciU)->tLciz:
...
@click.option("-n","--name",help="Name of the cloud pod",required=tLciA)
def cmd_cpvcs_init(name:tLciU):
...
@click.option("-n","--name",help="Name of the cloud pod",required=tLciA)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=tLciA,default=tLciC)
def cmd_cpvcs_delete(name:tLciU,remote:tLciz):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=tLciA)
def cmd_cpvcs_register_remote(name:tLciU):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=tLciA)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=tLciA)
def cmd_cpvcs_rename(name:tLciU,new_name:tLciU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=tLciA)
def cmd_cpvcs_commit(message:tLciU,name:tLciU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=tLciA,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=tLciA)
def cmd_cpvcs_push(squash:tLciz,message:tLciU,name:tLciU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=tLcin,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=tLciA,default=tLciC,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=tLciA)
def cmd_cpvcs_inject(version:tLcin,reset:tLciz,name:tLciU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=tLciA,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=tLciA,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=tLciA)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:tLciU,inject:tLciz,reset:tLciz):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=tLciA,default=tLciC)
def cmd_cpvcs_list_pods(remote:tLciz):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=tLciA)
def cmd_cpvcs_versions(name:tLciU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=tLciA,type=tLcin)
@click.option("-n","--name",help="Name of the cloud pod",required=tLciA)
def cmd_cpvcs_version_info(version:tLcin,name:tLciU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=tLcin,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=tLciA)
def cmd_cpvcs_version_metamodel(version:tLcin,name:tLciU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=tLciA,type=tLcin,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=tLciA,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=tLciA,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=tLciC,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=tLciA)
def cmd_cpvcs_set_version(version:tLcin,inject:tLciz,reset:tLciz,commit_before:tLciz,name:tLciU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=tLciA)
def cmd_cpvcs_commits(version:tLcin,name:tLciU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=tLciA)
@click.option("--commit","-c",required=tLciA)
@click.option("-n","--name",help="Name of the cloud pod",required=tLciA)
def cmd_cpvcs_commit_diff(version:tLcin,commit:tLcin,name:tLciU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(tLcir)
  status.stop()
...
def print_table(columns:Dict[tLciU,tLciU],rows:List[Dict[tLciU,Any]],formatters:Dict[tLciU,Callable[[Any],tLciU]]=tLcir):
 from rich.table import Table
 if formatters is tLcir:
  formatters=tLciI()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  tLcip(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->tLcir:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->tLcir:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->tLcir:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->tLcir:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
