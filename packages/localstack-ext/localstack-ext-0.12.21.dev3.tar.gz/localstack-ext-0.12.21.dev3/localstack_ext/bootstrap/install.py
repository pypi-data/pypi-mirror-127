...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[wBulg],wBulg],api_name:wBulg):
 if not(is_debian()and in_docker()):
  ...
  return
 if wBulF(packages,wBulg):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=wBulq)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=wBulq)
def check_or_install(command:wBulg,packages:Union[List[wBulg],wBulg],api:wBulg):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=wBulq):
  try:
