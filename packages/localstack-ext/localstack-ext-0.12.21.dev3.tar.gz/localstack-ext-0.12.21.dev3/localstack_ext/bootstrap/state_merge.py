...
def _merge_helper(current,injecting,merge_strategy=psDjy,visited:Set=psDjy):
 if psDjN(current,psDjw)and psDjN(injecting,psDjw):
 ...
 for field_name,inj_field_value in inj_dict.items():
  ...
  if cur_field_value is not psDjy:
   ...
   cur_dict[field_name]=inj_field_value
 return cur_dict
def merge_object_state(current,injecting,merge_strategy=psDjy):
...
def handle_special_case(current,injecting):
 if psDjN(injecting,Queue):
  current.queues[injecting.name]=injecting
  return psDjo
 elif psDjN(injecting,FakeBucket):
  current["global"].buckets[injecting.name]=injecting
  return psDjo
def add_missing_attributes(obj:Any,safe:psDjS=psDjo,visited:Set=psDjy):
 try:
  obj_dict=get_object_dict(obj)
  if obj_dict is psDjy:
   return
  ...
  class_inst_attrs=infer_class_attributes(psDjF(obj))
  for key,value in class_inst_attrs.items():
   if key not in obj_dict:
    LOG.debug("Add missing attribute '%s' to state object of type %s"%(key,psDjF(obj)))
    obj_dict[key]=value
 except psDjX as e:
  if not safe:
   raise
  LOG.warning("Unable to add missing attributes to persistence state object %s: %s",(obj,e))
def infer_class_attributes(clazz:Type)->Dict[psDjA,Any]:
 if clazz in[psDjw,psDjz]or not inspect.isclass(clazz)or clazz.__name__=="function":
  return{}
 constructor=psDjO(clazz,"__init__",psDjy)
 if not constructor:
  return{}
 try:
  ...
  def get_default_arg_value(arg_name,arg_index=-1):
   ...
   num_non_default_args=psDjm(sig_args.args or[])-psDjm(arg_defaults)
   offset=arg_index-num_non_default_args
   if offset in psDjU(psDjm(arg_defaults)):
  ...
  args=[]
  kwargs={}
  for arg_idx in psDjU(1,psDjm(sig_args.args)):
  ...
  result=psDjz(instance.__dict__)
  return result
 except psDjX:
  return{}
def merge_sqllite_dbs(file_dest:psDjA,file_src:psDjA)->psDjy:
 def _merge_table(table_name:psDjA,cursor_a,cursor_b)->psDjy:
  tmp_table_name=f"'{table_name}_new'"
  table_name=f"'{table_name}'"
  select_query=f"SELECT * FROM {table_name}"
  if table_name=="'cf'":
   return
  schema=psDjW(psDjK(lambda x:x[1],cursor_b.execute(f"PRAGMA table_info({table_name})")))
  params=f"({('?,' * len(schema))[:-1]})"
  insert_str=f"INSERT INTO {tmp_table_name} {str(schema)} values {params}"
  if table_name=="'dm'":
   excl_tables=(psDjA(psDjw(psDjK(lambda x:x[0],cursor_a.execute(f"SELECT TableName FROM {table_name}")))).replace("[","(",1).replace("]",")",1))
 ...
 with contextlib.closing(sqlite3.connect(file_dest))as db_src,contextlib.closing(sqlite3.connect(file_src))as db_target:
  ...
  table_names=psDjw(psDjK(lambda x:x[0],cursor_dest.execute("SELECT name FROM sqlite_master WHERE type='table'")))
...
def merge_kinesis_state(path_dest:psDjA,path_src:psDjA)->psDjS:
 ...
 if not os.path.isfile(tmp_dir_statefile):
  ...
  return psDjt
 with psDjQ(datadir_statefile)as datadir_kinesis_file,psDjQ(tmp_dir_statefile)as tmp_dir_kinesis_file:
  ...
  if psDjm(tmp_dir_streams)>0:
   ...
   with psDjQ(datadir_statefile,"w")as mutated_datadir_kinesis_file:
