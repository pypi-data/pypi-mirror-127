...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=fRIEL
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:fRIEC)->fRIEC:
  pass
 @fRIEK
 def get(cls,requested_file_locator:fRIEC):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=fRIEL
 @fRIEK
 def get(cls,state_file_locator:fRIEC,requested_storage:fRIEC,serializers:Dict[fRIEC,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[fRIEC,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:fRIEC)->fRIEC:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:fRIEC)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:fRIEC)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:fRIEC)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:fRIEC)->fRIEC:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[fRIEC]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:fRIEC,new_key:fRIEC,referenced_by_version:fRIEC=fRIEL):
  pass
 @abc.abstractmethod
 def version_exists(self,key:fRIEC)->fRIEz:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:fRIEC,key:fRIEC):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:fRIEC,new_key:fRIEC,base_path:fRIEC)->fRIEz:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[fRIEC]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[fRIEC,fRIEC],remote_location:fRIEC=fRIEL)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[fRIEC]:
  ...
  for cpvcs_obj in args:
   if fRIEk(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif fRIEk(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[fRIEC,fRIEC],remote_location:fRIEC=fRIEL)->List[Union[Revision,Version]]:
