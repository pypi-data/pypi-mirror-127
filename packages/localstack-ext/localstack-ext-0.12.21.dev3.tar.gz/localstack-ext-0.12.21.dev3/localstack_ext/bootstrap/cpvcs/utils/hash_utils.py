...
def random_hash()->UScMD:
 return UScMo(random.getrandbits(160))
def compute_file_hash(file_path:UScMD,accum=UScME)->Optional[UScMD]:
 try:
  with UScMq(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:UScMY,state_archive:UScMD)->UScMD:
 ...
 m.update(UScMD(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_node_hash(cpvcs_node:CPVCSNode)->UScMD:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=UScMF(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with UScMq(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except UScMm as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if UScMI(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(UScMD(cpvcs_node.revision_number).encode("utf-8"))
 elif UScMI(cpvcs_node,Version):
  m.update(UScMD(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
