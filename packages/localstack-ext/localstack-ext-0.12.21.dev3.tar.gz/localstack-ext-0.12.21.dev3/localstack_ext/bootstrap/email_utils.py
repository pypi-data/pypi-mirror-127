...
SENT_EMAILS=[]
EMAIL_BLACKLIST=pvYwk()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",pvYwI(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=pvYwV,from_name=pvYwV,smtp_host=pvYwV,smtp_user=pvYwV,smtp_pass=pvYwV,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if pvYwr(recipients,pvYwN)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=pvYwV):
 ...
 result["From"]=formataddr((pvYwI(Header(from_name,"utf-8")),from_email))
 return result
