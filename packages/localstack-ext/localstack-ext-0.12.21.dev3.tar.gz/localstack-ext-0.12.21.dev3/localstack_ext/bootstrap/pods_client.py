...
class PodInfo:
 def __init__(self,name=JGcgU,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(JGcgS):
 BACKEND="_none_"
 def __init__(self,pod_name=JGcgU,config=JGcgU):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise JGcgI("Not implemented")
 def delete(self,remote:JGcgN)->JGcgN:
  raise JGcgI("Not implemented")
 def push(self,comment:JGcgy=JGcgU)->PodInfo:
  raise JGcgI("Not implemented")
 def pull(self,inject_version_state:JGcgN=JGcgm,reset_state_before:JGcgN=JGcgm,fetch_all:JGcgN=JGcgU):
  raise JGcgI("Not implemented")
 def commit(self,message:JGcgy=JGcgU):
  raise JGcgI("Not implemented")
 def inject(self,version:JGcgu,reset_state:JGcgN):
  raise JGcgI("Not implemented")
 def list_versions(self)->List[JGcgy]:
  raise JGcgI("Not implemented")
 def version_info(self,version:JGcgu):
  raise JGcgI("Not implemented")
 def version_metamodel(self,version:JGcgu):
  raise JGcgI("Not implemented")
 def set_version(self,version:JGcgu,inject_version_state:JGcgN,reset_state:JGcgN,commit_before:JGcgN):
  raise JGcgI("Not implemented")
 def list_version_commits(self,version:JGcgu)->List[JGcgy]:
  raise JGcgI("Not implemented")
 def get_commit_diff(self,version:JGcgu,commit:JGcgu)->JGcgy:
  raise JGcgI("Not implemented")
 def register_remote(self,pod_name:JGcgy)->JGcgN:
  raise JGcgI("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->JGcgN:
  raise JGcgI("Not implemented")
 def list_pods(self,fetch_remote:JGcgN)->Set[JGcgy]:
  raise JGcgI("Not implemented")
 ...
 def get(cls,pod_name,pre_config=JGcgU):
  ...
  raise JGcgI('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=JGcgm
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=JGcgT)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=JGcgT
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise JGcgI("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @JGcgO
 def get_state_zip_from_instance(get_content=JGcgm):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise JGcgI("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:JGcgy=JGcgU):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @JGcgO
 def parse_pod_name_from_qualifying_name(qualifying_name:JGcgy)->JGcgy:
 ...
 @JGcgO
 def _get_max_version_for_pod_from_platform(pod_name:JGcgy,auth_headers):
  ...
  remote_max_ver=JGcgu(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with JGcgv(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with JGcgv(meta_archive,"rb")as meta_archive_content:
   ...
   with JGcgv(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=JGcgU)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:JGcgy):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def push(self,comment:JGcgy=JGcgU)->PodInfo:
  ...
  return PodInfo()
 def pull(self,inject_version_state:JGcgN=JGcgm,reset_state_before:JGcgN=JGcgm,fetch_all:JGcgN=JGcgT):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=JGcgm):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(JGcgQ(lambda ver:JGcgy(ver),JGcgq(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:JGcgu,reset_state:JGcgN):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=JGcgT,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def list_pods(self,fetch_remote:JGcgN)->Set[JGcgy]:
  ...
  return result
 @JGcgR
 def upload_content(cls,presigned_url:JGcgy,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise JGcgI("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @JGcgO
 def create_platform_url(request:JGcgy)->JGcgy:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:JGcgy=JGcgU)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=JGcgT)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(JGcge(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=JGcge(zip_data_content)
  return result
 def pull(self,inject_version_state:JGcgN=JGcgm,reset_state_before:JGcgN=JGcgm,fetch_all:JGcgN=JGcgU):
  ...
  rm_rf(zip_path)
 @JGcgO
 def presigned_url(pod_name:JGcgy,mode:JGcgy)->JGcgy:
  ...
  if response.status_code>=400:
   raise JGcgI("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise JGcgI("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:JGcgy=JGcgU):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=JGcgT)
 ...
 def pull(self,inject_version_state:JGcgN=JGcgm,reset_state_before:JGcgN=JGcgm,fetch_all:JGcgN=JGcgU):
  ...
  if remote_ref not in remote_refs:
   raise JGcgI('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=JGcgb(self,"clone_dir",JGcgU)
...
class PodConfigManagerMeta(JGcgz):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=JGcgU
   for manager in cls.CHAIN:
    try:
     tmp=JGcgb(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif JGcgC(tmp,JGcgo)and JGcgC(result,JGcgo):
       result.extend(tmp)
    except JGcgI:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not JGcgU:
    return result
   raise JGcgI('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(JGcgS,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @JGcgR
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise JGcgI('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(JGcgS):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=JGcgT).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=JGcgr([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=JGcgm):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except JGcgI:
   if safe:
    return{}
   raise JGcgI('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(JGcgS):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise JGcgI("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise JGcgI("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:JGcgy,pre_config:Dict[JGcgy,JGcgy],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:JGcgy,remote:JGcgN,pre_config:Dict[JGcgy,JGcgy])->JGcgN:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:JGcgy,pre_config:Dict[JGcgy,JGcgy],**kwargs)->JGcgN:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name)
 return result
def rename_pod(current_pod_name:JGcgy,new_pod_name:JGcgy,pre_config:Dict[JGcgy,JGcgy],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:JGcgN,pre_config:Dict[JGcgy,JGcgy],**kwargs)->List[JGcgy]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:JGcgy,pre_config:Dict[JGcgy,JGcgy],message:JGcgy=JGcgU,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:JGcgy,version:JGcgu,reset_state:JGcgN,pre_config:Dict[JGcgy,JGcgy],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:JGcgy,pre_config:Dict[JGcgy,JGcgy],**kwargs)->List[JGcgy]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:JGcgu,pod_name:JGcgy,pre_config:Dict[JGcgy,JGcgy],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:JGcgu,pod_name:JGcgy,pre_config:Dict[JGcgy,JGcgy],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:JGcgu,inject_version_state:JGcgN,reset_state:JGcgN,commit_before:JGcgN,pod_name:JGcgy,pre_config:Dict[JGcgy,JGcgy],**kwargs)->JGcgN:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:JGcgu,pod_name:JGcgy,pre_config:Dict[JGcgy,JGcgy])->List[JGcgy]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:JGcgu,commit:JGcgu,pod_name:JGcgy,pre_config:Dict[JGcgy,JGcgy])->JGcgy:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_state(pod_name,pre_config=JGcgU,squash_commits=JGcgm,comment=JGcgU,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=JGcgm,reset_state_before=JGcgm,**kwargs):
 pre_config=kwargs.get("pre_config",JGcgU)
 if not pod_name:
  raise JGcgI("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=JGcgm,exclude_from_reset:List[JGcgy]=JGcgU):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise JGcgI("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->JGcgy:
 try:
...
def get_persisted_resource_names(data_dir)->List[JGcgy]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and JGcge(os.listdir(entry.path))>0:
    names.append(entry.name)
