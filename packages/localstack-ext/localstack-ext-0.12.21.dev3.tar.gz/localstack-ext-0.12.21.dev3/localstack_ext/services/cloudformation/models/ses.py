...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[PLibB])[0]
 def get_physical_resource_id(self,attribute=PLibB,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if PLibq(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return PLibw(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @PLibG
 def add_defaults(resource,stack_name:PLibD):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return PLibw(SESReceiptRule,self).get_cfn_attribute(attribute)
 @PLibJ
