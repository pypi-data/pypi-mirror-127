...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=feCLJ(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not feCLu
   return result
  except feCLR:
   props=self.props
   ...
   if parts[0]in candidates and feCLE(parts)>1:
