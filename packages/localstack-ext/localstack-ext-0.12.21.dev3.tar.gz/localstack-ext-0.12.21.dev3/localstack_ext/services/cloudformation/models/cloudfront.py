...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or BybAt
 @BybAM
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return BybAx(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @BybAo
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if BybAH(origins,BybAp)else origins.get("Items")
  origins_to_create=BybAk([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if BybAH(aliases,BybAp)else aliases.get("Items")
  aliases_to_create=BybAk(aliases)
  for distr in distrs:
   origins=BybAk([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=BybAk(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",BybAI)
   config["Enabled"]=BybAS(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if BybAH(custom_errors,BybAF)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=BybAS(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":BybAI}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=BybAS(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=BybAt):
    parent=config if parent is BybAt else parent
    items=parent.get(key)or[]
    if BybAH(items,BybAF)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":BybAN(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if BybAH(cache_behavs,BybAF)else cache_behavs)
