...
def apply_patches():
 ...
 extended_proto_cmds.update({"SITE ADDUSER":mgStE(perm="m",auth=mgStv,arg=mgStv,help="Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.")})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  thread=ShellCommandThread(cmd,outfile=subprocess.PIPE,env_vars=env_vars,quiet=mgStU)
  thread.start()
  time.sleep(2)
 else:
  thread=do_start_ftp(port,asynchronous=mgStv)
 time.sleep(2)
 wait_for_port_open(port,retries=10,sleep_time=1.5)
 TMP_THREADS.append(thread)
 return thread
def do_start_ftp(port,asynchronous=mgStv):
...
def main():
 do_start_ftp(mgStR(sys.argv[1]),asynchronous=mgStU)
