...
def extract_meta_and_state_archives(meta_archives:Dict[VcaQg,VcaQg],state_archives:Dict[VcaQg,VcaQg]):
  ...
  for version_no,archive in product_space_archive.items():
   with zipfile.ZipFile(archive)as meta_zip:
    if product_space_archive==meta_archives:
     archive_dest=config_context.get_version_meta_archive_path(version_no=version_no,with_format=VcaQI)
    else:
     archive_dest=config_context.get_version_state_archive_path(version_no=version_no,with_format=VcaQI)
...
def register_remote(remote_info:Dict[VcaQg,VcaQg]):
 if config_context.is_remotly_managed():
  LOG.warning("Pod is already remotely managed")
  return
 with VcaQD(config_context.get_remote_info_path(),"w")as fp:
...
def merge_version_space(version_space_archive):
 ...
 for version_ref_file in os.listdir(remote_config_context.get_ver_refs_path()):
  ...
  with VcaQD(remote_version_ref_file_path,"r")as fp:
...
def _create_metamodel_helper(obj,width=25,visited:set=VcaQM):
 if obj is VcaQM:
  return obj
 ...
 elif VcaQp(obj,VcaQX):
  result=[_create_metamodel_helper(o,width=width,visited=visited)for o in obj]
  if VcaQW(result)>width:
   result={"size":VcaQW(result),"items":result[:width]}
 return result
