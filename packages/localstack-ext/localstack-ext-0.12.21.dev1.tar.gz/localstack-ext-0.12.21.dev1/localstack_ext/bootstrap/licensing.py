...
TEST_AUTH_HEADERS=DsbWI
class KeyActivationError(DsbWw):
 def __init__(self,message:DsbWc=DsbWI):
...
class InvalidDecryptionKeyError(KeyActivationError):
 pass
def read_api_key(raise_if_missing=DsbWM):
 key=(os.environ.get(ENV_LOCALSTACK_API_KEY)or "").strip()
 if not key and raise_if_missing:
  raise DsbWw("Unable to retrieve API key. Please configure $%s in your environment"%ENV_LOCALSTACK_API_KEY)
 return key
def truncate_api_key(api_key:DsbWc):
 return '"%s..."(%s)'%(api_key[:3],DsbWJ(api_key))
def fetch_key()->DsbWV:
 ...
 try:
  ...
  result=requests.post("%s/activate"%config.API_URL,json.dumps(data),verify=DsbWO)
  ...
  if log_license_issues():
   api_key=DsbWc(api_key_configured()or "")
...
def cache_key_locally(api_key,key_b64):
 ...
 timestamp=DsbWc(DsbWu(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in DsbWh(DsbWJ(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 configs["cached_key"]={"timestamp":DsbWu(timestamp),"key_hash":md5(api_key),"key":key_b64}
...
def load_cached_key(api_key):
 ...
 timestamp=DsbWc(cached_key["timestamp"])
 ...
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 return key_b64
def enable_file_decryption(key:DsbWV):
 ...
 try:
  ...
  if "import" not in to_str(file_content):
   raise DsbWj("Decryption resulted in invalid python file!")
 except DsbWw:
...
def prepare_environment():
 ...
 if not ENV_PREPARED.get("finalized"):
  try:
   key=fetch_key()
   if not key:
    raise DsbWw("Unable to fetch and validate API key from environment")
   if to_str(key)!="test":
...
def api_key_configured():
 return read_api_key(raise_if_missing=DsbWO)
def is_logged_in():
 configs=localstack_config.load_config_file()
...
def get_auth_headers():
 ...
 api_key=read_api_key(raise_if_missing=DsbWO)
 if api_key:
  return{"ls-api-key":api_key,"ls-version":__version__}
 raise DsbWw("Please log in first")
