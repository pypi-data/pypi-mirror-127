...
SENT_EMAILS=[]
EMAIL_BLACKLIST=ENslx()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",ENslo(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=ENslk,from_name=ENslk,smtp_host=ENslk,smtp_user=ENslk,smtp_pass=ENslk,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if ENsly(recipients,ENsli)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=ENslk):
 ...
 result["From"]=formataddr((ENslo(Header(from_name,"utf-8")),from_email))
 return result
