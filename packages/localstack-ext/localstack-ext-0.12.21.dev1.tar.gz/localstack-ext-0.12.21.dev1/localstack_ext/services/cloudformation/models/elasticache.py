...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=TcCMo(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not TcCMN
   return result
  except TcCMf:
   props=self.props
   ...
   if parts[0]in candidates and TcCMh(parts)>1:
