...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or FatMg
 @FatMU
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return FatME(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @FatMx
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if FatMD(origins,FatMS)else origins.get("Items")
  origins_to_create=FatMn([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if FatMD(aliases,FatMS)else aliases.get("Items")
  aliases_to_create=FatMn(aliases)
  for distr in distrs:
   origins=FatMn([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=FatMn(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",FatMJ)
   config["Enabled"]=FatMs(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if FatMD(custom_errors,FatMO)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=FatMs(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":FatMJ}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=FatMs(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=FatMg):
    parent=config if parent is FatMg else parent
    items=parent.get(key)or[]
    if FatMD(items,FatMO)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":FatMY(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if FatMD(cache_behavs,FatMO)else cache_behavs)
