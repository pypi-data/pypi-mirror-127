...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[hBpRl])[0]
 def get_physical_resource_id(self,attribute=hBpRl,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if hBpRo(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return hBpRQ(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @hBpRf
 def add_defaults(resource,stack_name:hBpRD):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return hBpRQ(SESReceiptRule,self).get_cfn_attribute(attribute)
 @hBpRX
