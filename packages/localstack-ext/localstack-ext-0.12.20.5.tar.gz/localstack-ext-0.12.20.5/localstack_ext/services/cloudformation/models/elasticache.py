...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=ViFcM(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not ViFca
   return result
  except ViFcX:
   props=self.props
   ...
   if parts[0]in candidates and ViFcm(parts)>1:
