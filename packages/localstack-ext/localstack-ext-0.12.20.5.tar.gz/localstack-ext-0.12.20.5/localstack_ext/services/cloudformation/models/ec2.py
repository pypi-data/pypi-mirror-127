...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=RgVcP,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if result:
   ...
   result=[]
   for perm in perms:
    if RgVcB(perm["IpProtocol"])!=RgVcB(rp["IpProtocol"]):
     continue
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",RgVcP)
   vpc_id=RgVcP
   if cls.is_ingress()and source_group_id and not source_group_name:
    ...
    result["SourceSecurityGroupName"]=groups and groups[0]["GroupName"]or RgVcP
    vpc_id=groups and groups[0].get("VpcId")or RgVcP
   dst_group_id=result.pop("DestinationSecurityGroupId",RgVcP)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=RgVcB(result.get("IpProtocol"))
   description=result.pop("Description",RgVcP)
   cidr_ipv6=result.pop("CidrIpv6",RgVcP)
