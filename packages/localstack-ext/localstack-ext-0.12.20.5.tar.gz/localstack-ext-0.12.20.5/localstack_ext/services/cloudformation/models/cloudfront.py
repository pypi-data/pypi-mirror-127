...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or DbIVW
 @DbIVR
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return DbIVu(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @DbIVn
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if DbIVy(origins,DbIVX)else origins.get("Items")
  origins_to_create=DbIVw([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if DbIVy(aliases,DbIVX)else aliases.get("Items")
  aliases_to_create=DbIVw(aliases)
  for distr in distrs:
   origins=DbIVw([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=DbIVw(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",DbIVk)
   config["Enabled"]=DbIVA(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if DbIVy(custom_errors,DbIVr)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=DbIVA(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":DbIVk}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=DbIVA(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=DbIVW):
    parent=config if parent is DbIVW else parent
    items=parent.get(key)or[]
    if DbIVy(items,DbIVr)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":DbIVB(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if DbIVy(cache_behavs,DbIVr)else cache_behavs)
