...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[Cmuwf])[0]
 def get_physical_resource_id(self,attribute=Cmuwf,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if Cmuwy(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return Cmuws(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @Cmuwj
 def add_defaults(resource,stack_name:CmuwB):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return Cmuws(SESReceiptRule,self).get_cfn_attribute(attribute)
 @Cmuwo
