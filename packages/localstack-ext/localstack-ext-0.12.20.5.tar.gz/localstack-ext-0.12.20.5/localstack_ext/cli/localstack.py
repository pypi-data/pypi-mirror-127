...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return oSAOV if e else oSAOL
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->oSAOw:
...
@click.option("--revert",is_flag=oSAOV,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:oSAOa):
...
def _cpvcs_initialized(pod_name:oSAOv)->oSAOa:
...
@click.option("-n","--name",help="Name of the cloud pod",required=oSAOV)
def cmd_cpvcs_init(name:oSAOv):
...
@click.option("-n","--name",help="Name of the cloud pod",required=oSAOV)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=oSAOV,default=oSAOL)
def cmd_cpvcs_delete(name:oSAOv,remote:oSAOa):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=oSAOV)
def cmd_cpvcs_register_remote(name:oSAOv):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=oSAOV)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=oSAOV)
def cmd_cpvcs_rename(name:oSAOv,new_name:oSAOv):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=oSAOV)
def cmd_cpvcs_commit(message:oSAOv,name:oSAOv):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=oSAOV,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=oSAOV)
def cmd_cpvcs_push(squash:oSAOa,message:oSAOv,name:oSAOv):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=oSAOB,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=oSAOV,default=oSAOL,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=oSAOV)
def cmd_cpvcs_inject(version:oSAOB,reset:oSAOa,name:oSAOv):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=oSAOV,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=oSAOV,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=oSAOV)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:oSAOv,inject:oSAOa,reset:oSAOa):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=oSAOV,default=oSAOL)
def cmd_cpvcs_list_pods(remote:oSAOa):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=oSAOV)
def cmd_cpvcs_versions(name:oSAOv):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=oSAOV,type=oSAOB)
@click.option("-n","--name",help="Name of the cloud pod",required=oSAOV)
def cmd_cpvcs_version_info(version:oSAOB,name:oSAOv):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=oSAOB,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=oSAOV)
def cmd_cpvcs_version_metamodel(version:oSAOB,name:oSAOv):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=oSAOV,type=oSAOB,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=oSAOV,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=oSAOV,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=oSAOL,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=oSAOV)
def cmd_cpvcs_set_version(version:oSAOB,inject:oSAOa,reset:oSAOa,commit_before:oSAOa,name:oSAOv):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=oSAOV)
def cmd_cpvcs_commits(version:oSAOB,name:oSAOv):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=oSAOV)
@click.option("--commit","-c",required=oSAOV)
@click.option("-n","--name",help="Name of the cloud pod",required=oSAOV)
def cmd_cpvcs_commit_diff(version:oSAOB,commit:oSAOB,name:oSAOv):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(oSAOw)
  status.stop()
...
def print_table(columns:Dict[oSAOv,oSAOv],rows:List[Dict[oSAOv,Any]],formatters:Dict[oSAOv,Callable[[Any],oSAOv]]=oSAOw):
 from rich.table import Table
 if formatters is oSAOw:
  formatters=oSAOH()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  oSAOd(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->oSAOw:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->oSAOw:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->oSAOw:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->oSAOw:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
