...
SENT_EMAILS=[]
EMAIL_BLACKLIST=JLGNz()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",JLGNY(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=JLGNs,from_name=JLGNs,smtp_host=JLGNs,smtp_user=JLGNs,smtp_pass=JLGNs,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if JLGNn(recipients,JLGNX)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=JLGNs):
 ...
 result["From"]=formataddr((JLGNY(Header(from_name,"utf-8")),from_email))
 return result
