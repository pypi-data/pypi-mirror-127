...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[ygFRo])[0]
 def get_physical_resource_id(self,attribute=ygFRo,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if ygFRQ(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return ygFRS(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @ygFRW
 def add_defaults(resource,stack_name:ygFRb):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return ygFRS(SESReceiptRule,self).get_cfn_attribute(attribute)
 @ygFRm
