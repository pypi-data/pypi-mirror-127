...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or BhMvN
 @BhMvr
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return BhMvo(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @BhMvP
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if BhMvE(origins,BhMvO)else origins.get("Items")
  origins_to_create=BhMvw([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if BhMvE(aliases,BhMvO)else aliases.get("Items")
  aliases_to_create=BhMvw(aliases)
  for distr in distrs:
   origins=BhMvw([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=BhMvw(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",BhMvU)
   config["Enabled"]=BhMvJ(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if BhMvE(custom_errors,BhMvI)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=BhMvJ(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":BhMvU}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=BhMvJ(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=BhMvN):
    parent=config if parent is BhMvN else parent
    items=parent.get(key)or[]
    if BhMvE(items,BhMvI)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":BhMvl(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if BhMvE(cache_behavs,BhMvI)else cache_behavs)
