...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=xCEiY(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not xCEip
   return result
  except xCEiD:
   props=self.props
   ...
   if parts[0]in candidates and xCEis(parts)>1:
