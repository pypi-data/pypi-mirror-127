...
SENT_EMAILS=[]
EMAIL_BLACKLIST=SFgYR()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",SFgYr(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=SFgYA,from_name=SFgYA,smtp_host=SFgYA,smtp_user=SFgYA,smtp_pass=SFgYA,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if SFgYu(recipients,SFgYE)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=SFgYA):
 ...
 result["From"]=formataddr((SFgYr(Header(from_name,"utf-8")),from_email))
 return result
