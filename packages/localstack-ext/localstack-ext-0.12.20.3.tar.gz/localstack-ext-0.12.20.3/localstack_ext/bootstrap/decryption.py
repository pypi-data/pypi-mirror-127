...
class DecryptionHandler:
 decryption_key:rOnzf
 def __init__(self,decryption_key:rOnzf):
  self.decryption_key=decryption_key
 def decrypt(self,content)->rOnzf:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=rOnzG):
  if path and not rOnzY(path,rOnzc):
   path=rOnzc(rOnzT(path,"_path",[]))
  if not path:
   return rOnzG
...
def patch_traceback_lines():
 if rOnzT(traceback.FrameSummary,"_ls_patch_applied",rOnzG):
  return
 @rOnze
 def line(self):
  try:
 ...
 rOnzm(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=rOnzN
def patch_inspect_findsource():
 if rOnzT(inspect,"_ls_patch_applied",rOnzG):
  return
 ...
 inspect._ls_patch_applied=rOnzN
