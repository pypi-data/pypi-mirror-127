...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return hsUMr if e else hsUMi
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->hsUML:
...
@click.option("--revert",is_flag=hsUMr,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:hsUMK):
...
def _cpvcs_initialized(pod_name:hsUMj)->hsUMK:
...
@click.option("-n","--name",help="Name of the cloud pod",required=hsUMr)
def cmd_cpvcs_init(name:hsUMj):
...
@click.option("-n","--name",help="Name of the cloud pod",required=hsUMr)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=hsUMr,default=hsUMi)
def cmd_cpvcs_delete(name:hsUMj,remote:hsUMK):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=hsUMr)
def cmd_cpvcs_register_remote(name:hsUMj):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=hsUMr)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=hsUMr)
def cmd_cpvcs_rename(name:hsUMj,new_name:hsUMj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=hsUMr)
def cmd_cpvcs_commit(message:hsUMj,name:hsUMj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=hsUMr,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=hsUMr)
def cmd_cpvcs_push(squash:hsUMK,message:hsUMj,name:hsUMj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=hsUMd,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=hsUMr,default=hsUMi,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=hsUMr)
def cmd_cpvcs_inject(version:hsUMd,reset:hsUMK,name:hsUMj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=hsUMr,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=hsUMr,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=hsUMr)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:hsUMj,inject:hsUMK,reset:hsUMK):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=hsUMr,default=hsUMi)
def cmd_cpvcs_list_pods(remote:hsUMK):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=hsUMr)
def cmd_cpvcs_versions(name:hsUMj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=hsUMr,type=hsUMd)
@click.option("-n","--name",help="Name of the cloud pod",required=hsUMr)
def cmd_cpvcs_version_info(version:hsUMd,name:hsUMj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=hsUMd,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=hsUMr)
def cmd_cpvcs_version_metamodel(version:hsUMd,name:hsUMj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=hsUMr,type=hsUMd,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=hsUMr,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=hsUMr,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=hsUMi,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=hsUMr)
def cmd_cpvcs_set_version(version:hsUMd,inject:hsUMK,reset:hsUMK,commit_before:hsUMK,name:hsUMj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=hsUMr)
def cmd_cpvcs_commits(version:hsUMd,name:hsUMj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=hsUMr)
@click.option("--commit","-c",required=hsUMr)
@click.option("-n","--name",help="Name of the cloud pod",required=hsUMr)
def cmd_cpvcs_commit_diff(version:hsUMd,commit:hsUMd,name:hsUMj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(hsUML)
  status.stop()
...
def print_table(columns:Dict[hsUMj,hsUMj],rows:List[Dict[hsUMj,Any]],formatters:Dict[hsUMj,Callable[[Any],hsUMj]]=hsUML):
 from rich.table import Table
 if formatters is hsUML:
  formatters=hsUMO()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  hsUMB(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->hsUML:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->hsUML:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->hsUML:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->hsUML:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
