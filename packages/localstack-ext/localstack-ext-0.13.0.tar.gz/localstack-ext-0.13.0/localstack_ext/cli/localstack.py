...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return SAPfm if e else SAPfs
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->SAPfL:
...
@click.option("--revert",is_flag=SAPfm,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:SAPfq):
...
def _cpvcs_initialized(pod_name:SAPfk)->SAPfq:
...
@click.option("-n","--name",help="Name of the cloud pod",required=SAPfm)
def cmd_cpvcs_init(name:SAPfk):
...
@click.option("-n","--name",help="Name of the cloud pod",required=SAPfm)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=SAPfm,default=SAPfs)
def cmd_cpvcs_delete(name:SAPfk,remote:SAPfq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=SAPfm)
def cmd_cpvcs_register_remote(name:SAPfk):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=SAPfm)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=SAPfm)
def cmd_cpvcs_rename(name:SAPfk,new_name:SAPfk):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=SAPfm)
def cmd_cpvcs_commit(message:SAPfk,name:SAPfk):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=SAPfm,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=SAPfm)
def cmd_cpvcs_push(squash:SAPfq,message:SAPfk,name:SAPfk):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=SAPfc,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=SAPfm,default=SAPfs,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=SAPfm)
def cmd_cpvcs_inject(version:SAPfc,reset:SAPfq,name:SAPfk):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=SAPfm,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=SAPfm,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=SAPfm)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:SAPfk,inject:SAPfq,reset:SAPfq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=SAPfm,default=SAPfs)
def cmd_cpvcs_list_pods(remote:SAPfq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=SAPfm)
def cmd_cpvcs_versions(name:SAPfk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=SAPfm,type=SAPfc)
@click.option("-n","--name",help="Name of the cloud pod",required=SAPfm)
def cmd_cpvcs_version_info(version:SAPfc,name:SAPfk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=SAPfc,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=SAPfm)
def cmd_cpvcs_version_metamodel(version:SAPfc,name:SAPfk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=SAPfm,type=SAPfc,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=SAPfm,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=SAPfm,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=SAPfs,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=SAPfm)
def cmd_cpvcs_set_version(version:SAPfc,inject:SAPfq,reset:SAPfq,commit_before:SAPfq,name:SAPfk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=SAPfm)
def cmd_cpvcs_commits(version:SAPfc,name:SAPfk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=SAPfm)
@click.option("--commit","-c",required=SAPfm)
@click.option("-n","--name",help="Name of the cloud pod",required=SAPfm)
def cmd_cpvcs_commit_diff(version:SAPfc,commit:SAPfc,name:SAPfk):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(SAPfL)
  status.stop()
...
def print_table(columns:Dict[SAPfk,SAPfk],rows:List[Dict[SAPfk,Any]],formatters:Dict[SAPfk,Callable[[Any],SAPfk]]=SAPfL):
 from rich.table import Table
 if formatters is SAPfL:
  formatters=SAPfl()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  SAPfo(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->SAPfL:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->SAPfL:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->SAPfL:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->SAPfL:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
