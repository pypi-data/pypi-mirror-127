...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=uTICr
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:uTICH)->uTICH:
  pass
 @uTICf
 def get(cls,requested_file_locator:uTICH):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=uTICr
 @uTICf
 def get(cls,state_file_locator:uTICH,requested_storage:uTICH,serializers:Dict[uTICH,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[uTICH,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:uTICH)->uTICH:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:uTICH)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:uTICH)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:uTICH)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:uTICH)->uTICH:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[uTICH]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:uTICH,new_key:uTICH,referenced_by_version:uTICH=uTICr):
  pass
 @abc.abstractmethod
 def version_exists(self,key:uTICH)->uTICW:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:uTICH,key:uTICH):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:uTICH,new_key:uTICH,base_path:uTICH)->uTICW:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[uTICH]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[uTICH,uTICH],remote_location:uTICH=uTICr)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[uTICH]:
  ...
  for cpvcs_obj in args:
   if uTICy(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif uTICy(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[uTICH,uTICH],remote_location:uTICH=uTICr)->List[Union[Revision,Version]]:
