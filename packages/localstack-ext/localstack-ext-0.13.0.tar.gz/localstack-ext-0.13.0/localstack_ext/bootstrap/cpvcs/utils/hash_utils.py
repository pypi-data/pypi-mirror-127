...
def random_hash()->xUOjf:
 return xUOjk(random.getrandbits(160))
def compute_file_hash(file_path:xUOjf,accum=xUOjs)->Optional[xUOjf]:
 try:
  with xUOjb(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:xUOjD,state_archive:xUOjf)->xUOjf:
 ...
 m.update(xUOjf(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_node_hash(cpvcs_node:CPVCSNode)->xUOjf:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=xUOja(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with xUOjb(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except xUOjV as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if xUOjQ(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(xUOjf(cpvcs_node.revision_number).encode("utf-8"))
 elif xUOjQ(cpvcs_node,Version):
  m.update(xUOjf(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
