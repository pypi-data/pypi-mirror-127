...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[duJPb],duJPb],api_name:duJPb):
 if not(is_debian()and in_docker()):
  ...
  return
 if duJPe(packages,duJPb):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=duJPT)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=duJPT)
def check_or_install(command:duJPb,packages:Union[List[duJPb],duJPb],api:duJPb):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=duJPT):
  try:
