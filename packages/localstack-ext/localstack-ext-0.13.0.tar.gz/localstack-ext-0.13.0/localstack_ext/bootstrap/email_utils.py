...
SENT_EMAILS=[]
EMAIL_BLACKLIST=cJtRb()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",cJtRv(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=cJtRH,from_name=cJtRH,smtp_host=cJtRH,smtp_user=cJtRH,smtp_pass=cJtRH,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if cJtRG(recipients,cJtRe)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=cJtRH):
 ...
 result["From"]=formataddr((cJtRv(Header(from_name,"utf-8")),from_email))
 return result
