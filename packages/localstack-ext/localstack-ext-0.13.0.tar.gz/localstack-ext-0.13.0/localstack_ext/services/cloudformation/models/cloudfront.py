...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or xcFbE
 @xcFbX
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return xcFbT(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @xcFbe
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if xcFbM(origins,xcFbV)else origins.get("Items")
  origins_to_create=xcFbn([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if xcFbM(aliases,xcFbV)else aliases.get("Items")
  aliases_to_create=xcFbn(aliases)
  for distr in distrs:
   origins=xcFbn([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=xcFbn(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",xcFbh)
   config["Enabled"]=xcFbd(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if xcFbM(custom_errors,xcFbS)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=xcFbd(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":xcFbh}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=xcFbd(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=xcFbE):
    parent=config if parent is xcFbE else parent
    items=parent.get(key)or[]
    if xcFbM(items,xcFbS)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":xcFbA(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if xcFbM(cache_behavs,xcFbS)else cache_behavs)
