...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[iTCBf])[0]
 def get_physical_resource_id(self,attribute=iTCBf,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if iTCBP(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return iTCBO(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @iTCBQ
 def add_defaults(resource,stack_name:iTCBn):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return iTCBO(SESReceiptRule,self).get_cfn_attribute(attribute)
 @iTCBo
