...
def canonicalize_name(name:zMQXH)->zMQXH:
 if name[-1]!=".":
  return f"{name}."
 return name
class Route53HostedZone(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[zMQXt])[0]
 def get_physical_resource_id(self,attribute=zMQXt,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return zMQXh(Route53HostedZone,self).get_cfn_attribute(attribute_name)
 @zMQXn
