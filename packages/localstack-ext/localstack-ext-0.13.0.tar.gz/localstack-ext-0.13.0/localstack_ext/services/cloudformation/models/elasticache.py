...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=wdgUJ(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not wdgUs
   return result
  except wdgUI:
   props=self.props
   ...
   if parts[0]in candidates and wdgUp(parts)>1:
