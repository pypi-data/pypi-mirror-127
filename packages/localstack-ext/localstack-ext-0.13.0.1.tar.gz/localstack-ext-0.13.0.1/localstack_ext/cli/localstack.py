...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return TpOzP if e else TpOzl
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->TpOzI:
...
@click.option("--revert",is_flag=TpOzP,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:TpOzG):
...
def _cpvcs_initialized(pod_name:TpOzM)->TpOzG:
...
@click.option("-n","--name",help="Name of the cloud pod",required=TpOzP)
def cmd_cpvcs_init(name:TpOzM):
...
@click.option("-n","--name",help="Name of the cloud pod",required=TpOzP)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=TpOzP,default=TpOzl)
def cmd_cpvcs_delete(name:TpOzM,remote:TpOzG):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=TpOzP)
def cmd_cpvcs_register_remote(name:TpOzM):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=TpOzP)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=TpOzP)
def cmd_cpvcs_rename(name:TpOzM,new_name:TpOzM):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=TpOzP)
def cmd_cpvcs_commit(message:TpOzM,name:TpOzM):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=TpOzP,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=TpOzP)
def cmd_cpvcs_push(squash:TpOzG,message:TpOzM,name:TpOzM):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=TpOzr,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=TpOzP,default=TpOzl,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=TpOzP)
def cmd_cpvcs_inject(version:TpOzr,reset:TpOzG,name:TpOzM):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=TpOzP,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=TpOzP,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=TpOzP)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:TpOzM,inject:TpOzG,reset:TpOzG):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=TpOzP,default=TpOzl)
def cmd_cpvcs_list_pods(remote:TpOzG):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=TpOzP)
def cmd_cpvcs_versions(name:TpOzM):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=TpOzP,type=TpOzr)
@click.option("-n","--name",help="Name of the cloud pod",required=TpOzP)
def cmd_cpvcs_version_info(version:TpOzr,name:TpOzM):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=TpOzr,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=TpOzP)
def cmd_cpvcs_version_metamodel(version:TpOzr,name:TpOzM):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=TpOzP,type=TpOzr,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=TpOzP,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=TpOzP,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=TpOzl,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=TpOzP)
def cmd_cpvcs_set_version(version:TpOzr,inject:TpOzG,reset:TpOzG,commit_before:TpOzG,name:TpOzM):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=TpOzP)
def cmd_cpvcs_commits(version:TpOzr,name:TpOzM):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=TpOzP)
@click.option("--commit","-c",required=TpOzP)
@click.option("-n","--name",help="Name of the cloud pod",required=TpOzP)
def cmd_cpvcs_commit_diff(version:TpOzr,commit:TpOzr,name:TpOzM):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(TpOzI)
  status.stop()
...
def print_table(columns:Dict[TpOzM,TpOzM],rows:List[Dict[TpOzM,Any]],formatters:Dict[TpOzM,Callable[[Any],TpOzM]]=TpOzI):
 from rich.table import Table
 if formatters is TpOzI:
  formatters=TpOzg()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  TpOzm(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->TpOzI:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->TpOzI:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->TpOzI:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->TpOzI:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
