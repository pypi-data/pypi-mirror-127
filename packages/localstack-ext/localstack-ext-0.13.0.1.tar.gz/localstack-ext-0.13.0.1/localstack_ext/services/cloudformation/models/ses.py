...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[mDdpe])[0]
 def get_physical_resource_id(self,attribute=mDdpe,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if mDdpS(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return mDdpv(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @mDdpB
 def add_defaults(resource,stack_name:mDdps):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return mDdpv(SESReceiptRule,self).get_cfn_attribute(attribute)
 @mDdpz
