...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=iOFnh,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if result:
   ...
   result=[]
   for perm in perms:
    if iOFne(perm["IpProtocol"])!=iOFne(rp["IpProtocol"]):
     continue
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",iOFnh)
   vpc_id=iOFnh
   if cls.is_ingress()and source_group_id and not source_group_name:
    ...
    result["SourceSecurityGroupName"]=groups and groups[0]["GroupName"]or iOFnh
    vpc_id=groups and groups[0].get("VpcId")or iOFnh
   dst_group_id=result.pop("DestinationSecurityGroupId",iOFnh)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=iOFne(result.get("IpProtocol"))
   description=result.pop("Description",iOFnh)
   cidr_ipv6=result.pop("CidrIpv6",iOFnh)
