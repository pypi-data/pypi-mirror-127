...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=YJGot(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not YJGoQ
   return result
  except YJGop:
   props=self.props
   ...
   if parts[0]in candidates and YJGol(parts)>1:
