...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or zUAfF
 @zUAfX
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return zUAfS(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @zUAfp
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if zUAfs(origins,zUAfr)else origins.get("Items")
  origins_to_create=zUAfv([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if zUAfs(aliases,zUAfr)else aliases.get("Items")
  aliases_to_create=zUAfv(aliases)
  for distr in distrs:
   origins=zUAfv([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=zUAfv(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",zUAfK)
   config["Enabled"]=zUAfN(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if zUAfs(custom_errors,zUAfy)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=zUAfN(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":zUAfK}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=zUAfN(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=zUAfF):
    parent=config if parent is zUAfF else parent
    items=parent.get(key)or[]
    if zUAfs(items,zUAfy)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":zUAfa(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if zUAfs(cache_behavs,zUAfy)else cache_behavs)
