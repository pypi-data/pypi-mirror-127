...
class DecryptionHandler:
 decryption_key:kptBc
 def __init__(self,decryption_key:kptBc):
  self.decryption_key=decryption_key
 def decrypt(self,content)->kptBc:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=kptBS):
  if path and not kptBH(path,kptBm):
   path=kptBm(kptBO(path,"_path",[]))
  if not path:
   return kptBS
...
def patch_traceback_lines():
 if kptBO(traceback.FrameSummary,"_ls_patch_applied",kptBS):
  return
 @kptBe
 def line(self):
  try:
 ...
 kptBU(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=kptBP
def patch_inspect_findsource():
 if kptBO(inspect,"_ls_patch_applied",kptBS):
  return
 ...
 inspect._ls_patch_applied=kptBP
