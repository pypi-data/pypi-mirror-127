...
SENT_EMAILS=[]
EMAIL_BLACKLIST=qjCFk()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",qjCFg(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=qjCFe,from_name=qjCFe,smtp_host=qjCFe,smtp_user=qjCFe,smtp_pass=qjCFe,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if qjCFQ(recipients,qjCFS)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=qjCFe):
 ...
 result["From"]=formataddr((qjCFg(Header(from_name,"utf-8")),from_email))
 return result
