...
TEST_AUTH_HEADERS=vfCaS
class KeyActivationError(vfCab):
 def __init__(self,message:vfCaR=vfCaS):
...
class InvalidDecryptionKeyError(KeyActivationError):
 pass
def read_api_key(raise_if_missing=vfCaN):
 key=(os.environ.get(ENV_LOCALSTACK_API_KEY)or "").strip()
 if not key and raise_if_missing:
  raise vfCab("Unable to retrieve API key. Please configure $%s in your environment"%ENV_LOCALSTACK_API_KEY)
 return key
def truncate_api_key(api_key:vfCaR):
 return '"%s..."(%s)'%(api_key[:3],vfCao(api_key))
def fetch_key()->vfCaV:
 ...
 try:
  ...
  result=requests.post("%s/activate"%config.API_URL,json.dumps(data),verify=vfCaA)
  ...
  if log_license_issues():
   api_key=vfCaR(api_key_configured()or "")
...
def cache_key_locally(api_key,key_b64):
 ...
 timestamp=vfCaR(vfCad(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in vfCau(vfCao(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 configs["cached_key"]={"timestamp":vfCad(timestamp),"key_hash":md5(api_key),"key":key_b64}
...
def load_cached_key(api_key):
 ...
 timestamp=vfCaR(cached_key["timestamp"])
 ...
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 return key_b64
def enable_file_decryption(key:vfCaV):
 ...
 try:
  ...
  if "import" not in to_str(file_content):
   raise vfCai("Decryption resulted in invalid python file!")
 except vfCab:
...
def prepare_environment():
 ...
 if not ENV_PREPARED.get("finalized"):
  try:
   key=fetch_key()
   if not key:
    raise vfCab("Unable to fetch and validate API key from environment")
   if to_str(key)!="test":
...
def api_key_configured():
 return read_api_key(raise_if_missing=vfCaA)
def is_logged_in():
 configs=localstack_config.load_config_file()
...
def get_auth_headers():
 ...
 api_key=read_api_key(raise_if_missing=vfCaA)
 if api_key:
  return{"ls-api-key":api_key,"ls-version":__version__}
 raise vfCab("Please log in first")
