...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[TaMRs],TaMRs],api_name:TaMRs):
 if not(is_debian()and in_docker()):
  ...
  return
 if TaMRK(packages,TaMRs):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=TaMRq)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=TaMRq)
def check_or_install(command:TaMRs,packages:Union[List[TaMRs],TaMRs],api:TaMRs):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=TaMRq):
  try:
