...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=juPVB
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:juPVH)->juPVH:
  pass
 @juPVq
 def get(cls,requested_file_locator:juPVH):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=juPVB
 @juPVq
 def get(cls,state_file_locator:juPVH,requested_storage:juPVH,serializers:Dict[juPVH,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[juPVH,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:juPVH)->juPVH:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:juPVH)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:juPVH)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:juPVH)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:juPVH)->juPVH:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[juPVH]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:juPVH,new_key:juPVH,referenced_by_version:juPVH=juPVB):
  pass
 @abc.abstractmethod
 def version_exists(self,key:juPVH)->juPVY:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:juPVH,key:juPVH):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:juPVH,new_key:juPVH,base_path:juPVH)->juPVY:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[juPVH]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[juPVH,juPVH],remote_location:juPVH=juPVB)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[juPVH]:
  ...
  for cpvcs_obj in args:
   if juPVD(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif juPVD(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[juPVH,juPVH],remote_location:juPVH=juPVB)->List[Union[Revision,Version]]:
