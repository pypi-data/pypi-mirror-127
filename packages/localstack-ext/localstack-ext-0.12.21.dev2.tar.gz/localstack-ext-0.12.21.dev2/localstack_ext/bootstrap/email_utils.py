...
SENT_EMAILS=[]
EMAIL_BLACKLIST=rpwLO()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",rpwLj(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=rpwLh,from_name=rpwLh,smtp_host=rpwLh,smtp_user=rpwLh,smtp_pass=rpwLh,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if rpwLm(recipients,rpwLt)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=rpwLh):
 ...
 result["From"]=formataddr((rpwLj(Header(from_name,"utf-8")),from_email))
 return result
