...
class PodInfo:
 def __init__(self,name=EYFde,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(EYFdQ):
 BACKEND="_none_"
 def __init__(self,pod_name=EYFde,config=EYFde):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise EYFdA("Not implemented")
 def delete(self,remote:EYFdJ)->EYFdJ:
  raise EYFdA("Not implemented")
 def push(self,comment:EYFdO=EYFde)->PodInfo:
  raise EYFdA("Not implemented")
 def pull(self,inject_version_state:EYFdJ=EYFdq,reset_state_before:EYFdJ=EYFdq,fetch_all:EYFdJ=EYFde):
  raise EYFdA("Not implemented")
 def commit(self,message:EYFdO=EYFde):
  raise EYFdA("Not implemented")
 def inject(self,version:EYFdj,reset_state:EYFdJ):
  raise EYFdA("Not implemented")
 def list_versions(self)->List[EYFdO]:
  raise EYFdA("Not implemented")
 def version_info(self,version:EYFdj):
  raise EYFdA("Not implemented")
 def version_metamodel(self,version:EYFdj):
  raise EYFdA("Not implemented")
 def set_version(self,version:EYFdj,inject_version_state:EYFdJ,reset_state:EYFdJ,commit_before:EYFdJ):
  raise EYFdA("Not implemented")
 def list_version_commits(self,version:EYFdj)->List[EYFdO]:
  raise EYFdA("Not implemented")
 def get_commit_diff(self,version:EYFdj,commit:EYFdj)->EYFdO:
  raise EYFdA("Not implemented")
 def register_remote(self,pod_name:EYFdO)->EYFdJ:
  raise EYFdA("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->EYFdJ:
  raise EYFdA("Not implemented")
 def list_pods(self,fetch_remote:EYFdJ)->Set[EYFdO]:
  raise EYFdA("Not implemented")
 ...
 def get(cls,pod_name,pre_config=EYFde):
  ...
  raise EYFdA('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=EYFdq
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=EYFdf)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=EYFdf
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise EYFdA("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @EYFdu
 def get_state_zip_from_instance(get_content=EYFdq):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise EYFdA("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:EYFdO=EYFde):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @EYFdu
 def parse_pod_name_from_qualifying_name(qualifying_name:EYFdO)->EYFdO:
 ...
 @EYFdu
 def _get_max_version_for_pod_from_platform(pod_name:EYFdO,auth_headers):
  ...
  remote_max_ver=EYFdj(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with EYFdD(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with EYFdD(meta_archive,"rb")as meta_archive_content:
   ...
   with EYFdD(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=EYFde)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:EYFdO):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def push(self,comment:EYFdO=EYFde)->PodInfo:
  ...
  return PodInfo()
 def pull(self,inject_version_state:EYFdJ=EYFdq,reset_state_before:EYFdJ=EYFdq,fetch_all:EYFdJ=EYFdf):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=EYFdq):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(EYFdX(lambda ver:EYFdO(ver),EYFdR(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:EYFdj,reset_state:EYFdJ):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=EYFdf,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def list_pods(self,fetch_remote:EYFdJ)->Set[EYFdO]:
  ...
  return result
 @EYFdi
 def upload_content(cls,presigned_url:EYFdO,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise EYFdA("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @EYFdu
 def create_platform_url(request:EYFdO)->EYFdO:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:EYFdO=EYFde)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=EYFdf)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(EYFdM(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=EYFdM(zip_data_content)
  return result
 def pull(self,inject_version_state:EYFdJ=EYFdq,reset_state_before:EYFdJ=EYFdq,fetch_all:EYFdJ=EYFde):
  ...
  rm_rf(zip_path)
 @EYFdu
 def presigned_url(pod_name:EYFdO,mode:EYFdO)->EYFdO:
  ...
  if response.status_code>=400:
   raise EYFdA("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise EYFdA("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:EYFdO=EYFde):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=EYFdf)
 ...
 def pull(self,inject_version_state:EYFdJ=EYFdq,reset_state_before:EYFdJ=EYFdq,fetch_all:EYFdJ=EYFde):
  ...
  if remote_ref not in remote_refs:
   raise EYFdA('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=EYFdn(self,"clone_dir",EYFde)
...
class PodConfigManagerMeta(EYFdk):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=EYFde
   for manager in cls.CHAIN:
    try:
     tmp=EYFdn(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif EYFdh(tmp,EYFdL)and EYFdh(result,EYFdL):
       result.extend(tmp)
    except EYFdA:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not EYFde:
    return result
   raise EYFdA('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(EYFdQ,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @EYFdi
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise EYFdA('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(EYFdQ):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=EYFdf).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=EYFds([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=EYFdq):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except EYFdA:
   if safe:
    return{}
   raise EYFdA('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(EYFdQ):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise EYFdA("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise EYFdA("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:EYFdO,pre_config:Dict[EYFdO,EYFdO],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:EYFdO,remote:EYFdJ,pre_config:Dict[EYFdO,EYFdO])->EYFdJ:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:EYFdO,pre_config:Dict[EYFdO,EYFdO],**kwargs)->EYFdJ:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name)
 return result
def rename_pod(current_pod_name:EYFdO,new_pod_name:EYFdO,pre_config:Dict[EYFdO,EYFdO],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:EYFdJ,pre_config:Dict[EYFdO,EYFdO],**kwargs)->List[EYFdO]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:EYFdO,pre_config:Dict[EYFdO,EYFdO],message:EYFdO=EYFde,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:EYFdO,version:EYFdj,reset_state:EYFdJ,pre_config:Dict[EYFdO,EYFdO],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:EYFdO,pre_config:Dict[EYFdO,EYFdO],**kwargs)->List[EYFdO]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:EYFdj,pod_name:EYFdO,pre_config:Dict[EYFdO,EYFdO],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:EYFdj,pod_name:EYFdO,pre_config:Dict[EYFdO,EYFdO],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:EYFdj,inject_version_state:EYFdJ,reset_state:EYFdJ,commit_before:EYFdJ,pod_name:EYFdO,pre_config:Dict[EYFdO,EYFdO],**kwargs)->EYFdJ:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:EYFdj,pod_name:EYFdO,pre_config:Dict[EYFdO,EYFdO])->List[EYFdO]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:EYFdj,commit:EYFdj,pod_name:EYFdO,pre_config:Dict[EYFdO,EYFdO])->EYFdO:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_state(pod_name,pre_config=EYFde,squash_commits=EYFdq,comment=EYFde,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=EYFdq,reset_state_before=EYFdq,**kwargs):
 pre_config=kwargs.get("pre_config",EYFde)
 if not pod_name:
  raise EYFdA("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=EYFdq,exclude_from_reset:List[EYFdO]=EYFde):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise EYFdA("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->EYFdO:
 try:
...
def get_persisted_resource_names(data_dir)->List[EYFdO]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and EYFdM(os.listdir(entry.path))>0:
    names.append(entry.name)
