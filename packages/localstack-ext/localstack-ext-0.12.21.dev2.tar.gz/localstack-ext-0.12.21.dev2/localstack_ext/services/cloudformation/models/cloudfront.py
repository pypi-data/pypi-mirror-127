...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or DbRNO
 @DbRNB
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return DbRNf(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @DbRNk
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if DbRNA(origins,DbRNI)else origins.get("Items")
  origins_to_create=DbRNn([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if DbRNA(aliases,DbRNI)else aliases.get("Items")
  aliases_to_create=DbRNn(aliases)
  for distr in distrs:
   origins=DbRNn([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=DbRNn(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",DbRNH)
   config["Enabled"]=DbRNq(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if DbRNA(custom_errors,DbRNU)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=DbRNq(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":DbRNH}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=DbRNq(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=DbRNO):
    parent=config if parent is DbRNO else parent
    items=parent.get(key)or[]
    if DbRNA(items,DbRNU)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":DbRNr(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if DbRNA(cache_behavs,DbRNU)else cache_behavs)
