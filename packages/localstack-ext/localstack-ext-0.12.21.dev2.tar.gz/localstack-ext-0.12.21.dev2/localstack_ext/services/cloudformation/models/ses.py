...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[BXHFS])[0]
 def get_physical_resource_id(self,attribute=BXHFS,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if BXHFV(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return BXHFx(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @BXHFQ
 def add_defaults(resource,stack_name:BXHFm):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return BXHFx(SESReceiptRule,self).get_cfn_attribute(attribute)
 @BXHFI
