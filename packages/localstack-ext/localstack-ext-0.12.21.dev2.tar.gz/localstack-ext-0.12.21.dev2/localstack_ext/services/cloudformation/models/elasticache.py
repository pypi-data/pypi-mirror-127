...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=GMlmX(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not GMlmj
   return result
  except GMlmY:
   props=self.props
   ...
   if parts[0]in candidates and GMlmT(parts)>1:
