...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return QDsAx if e else QDsAw
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->QDsAc:
...
@click.option("--revert",is_flag=QDsAx,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:QDsAm):
...
def _cpvcs_initialized(pod_name:QDsAh)->QDsAm:
...
@click.option("-n","--name",help="Name of the cloud pod",required=QDsAx)
def cmd_cpvcs_init(name:QDsAh):
...
@click.option("-n","--name",help="Name of the cloud pod",required=QDsAx)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=QDsAx,default=QDsAw)
def cmd_cpvcs_delete(name:QDsAh,remote:QDsAm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=QDsAx)
def cmd_cpvcs_register_remote(name:QDsAh):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=QDsAx)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=QDsAx)
def cmd_cpvcs_rename(name:QDsAh,new_name:QDsAh):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=QDsAx)
def cmd_cpvcs_commit(message:QDsAh,name:QDsAh):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=QDsAx,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=QDsAx)
def cmd_cpvcs_push(squash:QDsAm,message:QDsAh,name:QDsAh):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=QDsAN,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=QDsAx,default=QDsAw,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=QDsAx)
def cmd_cpvcs_inject(version:QDsAN,reset:QDsAm,name:QDsAh):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=QDsAx,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=QDsAx,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=QDsAx)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:QDsAh,inject:QDsAm,reset:QDsAm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=QDsAx,default=QDsAw)
def cmd_cpvcs_list_pods(remote:QDsAm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=QDsAx)
def cmd_cpvcs_versions(name:QDsAh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=QDsAx,type=QDsAN)
@click.option("-n","--name",help="Name of the cloud pod",required=QDsAx)
def cmd_cpvcs_version_info(version:QDsAN,name:QDsAh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=QDsAN,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=QDsAx)
def cmd_cpvcs_version_metamodel(version:QDsAN,name:QDsAh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=QDsAx,type=QDsAN,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=QDsAx,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=QDsAx,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=QDsAw,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=QDsAx)
def cmd_cpvcs_set_version(version:QDsAN,inject:QDsAm,reset:QDsAm,commit_before:QDsAm,name:QDsAh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=QDsAx)
def cmd_cpvcs_commits(version:QDsAN,name:QDsAh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=QDsAx)
@click.option("--commit","-c",required=QDsAx)
@click.option("-n","--name",help="Name of the cloud pod",required=QDsAx)
def cmd_cpvcs_commit_diff(version:QDsAN,commit:QDsAN,name:QDsAh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(QDsAc)
  status.stop()
...
def print_table(columns:Dict[QDsAh,QDsAh],rows:List[Dict[QDsAh,Any]],formatters:Dict[QDsAh,Callable[[Any],QDsAh]]=QDsAc):
 from rich.table import Table
 if formatters is QDsAc:
  formatters=QDsAy()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  QDsAM(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->QDsAc:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->QDsAc:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->QDsAc:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->QDsAc:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
