...
class BatchedTreeEnsemble(BaseEstimator):
    ...
    def __init__(
        ...
        self.params = BatchedTreeEnsemble.PARAMS
    ...
    def _validate_parameters(self):
        ...
        for varname, value in self.get_params().items():
            if varname in map(lambda x: x["name"], self.params):
                _param_check(self.params, varname, value)

        # handle case of Lale operators
        be_name = type(self.base_ensemble).__name__
        if be_name == "TrainedIndividualOp" or be_name == "TrainableIndividualOp":
            self.base_ensemble = self.base_ensemble.impl

        if not isinstance(self.base_ensemble, self.valid_base_ensembles):
        ...
        if hasattr(self.base_ensemble, "predict_proba"):
            self.base_ensemble_reg = mapping[type(self.base_ensemble).__name__]()
            ...
            self.base_ensemble_reg = clone(self.base_ensemble)
