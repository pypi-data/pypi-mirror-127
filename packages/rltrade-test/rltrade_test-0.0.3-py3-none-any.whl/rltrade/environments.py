...
class SmartPortfolioEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            df = pd.DataFrame(self.portfolio_return_memory)
            df.columns = ["daily_return"]
            weight_mean = np.mean(self.weights_memory,axis=0)
            best_stock_weights = np.max(weight_mean)
            print("weight mean",weight_mean)
            print("best stock",best_stock_weights)
            filter_stock = [i for i,j in enumerate(weight_mean) if j >= (best_stock_weights * self.filter_threshold)]
            self.ticker_list = np.array(self.ticker_list)[filter_stock].tolist()
            self.best_weight = best_stock_weights * len(self.ticker_list)

            df = self.get_reward_per_stock()
    ...
    def add_vix_fix(self,data,years):
        ...
        for ticker in unique_ticker:
            temp = df[(df['tic'] == ticker)].copy()
            temp[f'vix_fix_{years}year'] = ((temp['account_value'].rolling(days).max() - temp['low'])/temp['close'].rolling(days).max()) * 100
            indicator_df = indicator_df.append(temp, ignore_index=True)
