...
import numpy as np
import pandas as pd
from rltrade import environments
from rltrade.data import FeatureEngineer
...
class SmartDRLAgent:
    ...
    def __init__(self,model_name,df,ticker_list,add_ticker_list,ticker_col_name,
                tech_indicators,additional_indicators,train_period,test_period,
                 env_kwargs,model_kwargs,tb_log_name,
                 total_timesteps=500):

        self.df = df
        self.model_name = model_name
        self.tech_indicators = tech_indicators
        self.add_ticker_list=add_ticker_list
    ...
    def get_model(
        ...
        model = MODELS[self.model_name](
            policy=policy,
            env=train_env,
            tensorboard_log=f"{config.TENSORBOARD_LOG_DIR}/{self.model_name}",
            verbose=verbose,
            policy_kwargs=policy_kwargs,
            **self.model_kwargs,
        )
    ...
    def make_prediction(self):
        assert self.model is not None, "please train the model before making prediction"
    ...
    def train_model(self):
        ...
        model = model.learn(total_timesteps=self.total_timesteps, 
    ...
    def train_model_filter(self):
        ...
        self.model = model.learn(total_timesteps=self.total_timesteps, 
                            tb_log_name=self.tb_log_name)
        
    def train_model_filter_add(self):
        print("Ticker list is",self.ticker_list)
        print("Preprocessing Data...")
        self.train,self.test = self.fe.train_test_split(self.df,self.train_period,self.test_period)
        self.model_kwargs['n_steps'] = self.train.index.nunique()
