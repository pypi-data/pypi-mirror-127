...
class Trades(Boredom):
    def __init__(self, **kwargs):
        ...
        self.if_then_total = {
            ("accepted", "1"): [("%s_total_quantity" %rs,"quantity" , int) for rs in self.rss]
        }
    ...
    def spec_collection(self, data: list[dict], ignore=False):
        ...
        for line in data:
            resource = self.rss.find(line["resource"])
            if accepted != "1":
                continue
            if resource == -1:
                raise KeyError
            self.collected[sell %self.rss[resource]] += quantity*price*(line["buy_or_sell"]=="sell")
            self.collected[buy %self.rss[resource]] += quantity*price*(line["buy_or_sell"]=="buy")
