...
Functions:
    ...
    - install_packages: Installs missing but needed packages.
"""
import sys
import subprocess
import calendar
...
def read_config(file: Optional[str] = None,
    ...
    """ Loads and reads in the configuration file specified by the user.
...
def signal_handler(signal_number: Any, stack: Any) -> None:
    """ Helper function to exit the program. When CTRL+C is hit,
    the program will shut down with exit code(0).
    """
    KillSwitch().reset()
    print("\nExiting program.")
    raise SystemExit
...
def init_logger(path: str, program_config: dict) -> None:
    # ToDO: Only create log-file if something is actually logged. Or delete empty log-files afterwards.
    """ Initializes the logger, specifies the path to the logging files,
    the logging massage as well as the logging level.
    ...
    if not read_config(file=None, section="operation_settings").get('enable_logging', True):
        ...
        file_name = path + dirname + f"{TimeHelper.now().strftime(time_format)}.log"
...
def install_packages(package_name: str) -> None:
    ...
    """
    permission: str = input(f"Module '{package_name}' not found. It will be installed. Proceed (Y / n)?")

    if permission.lower() in ["y", "j", "yes"]:
        try:
        ...
        print("Module will not be installed. Either install it manually or change the database\n"
              "management system, e.g use 'Sqlite'.")
        raise SystemExit
