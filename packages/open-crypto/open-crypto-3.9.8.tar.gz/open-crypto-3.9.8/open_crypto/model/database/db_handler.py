...
from sqlalchemy import create_engine, MetaData, tuple_, func, inspect
...
from model.utilities.utilities import split_str_to_list, install_packages
...
class DatabaseHandler:
    ...
    def get_readable_query(self,
        ...
        """ #ToDo Debug, not working when filters are applied.
        ...
        with self.session_scope() as session:
            ...
            if not all([isinstance(item, list) for item in [exchanges, first_currencies, second_currencies]]):
                if exchanges and isinstance(exchanges, str):
                    exchanges = split_str_to_list(exchanges)
                if first_currencies and isinstance(first_currencies, str):
                    first_currencies = split_str_to_list(first_currencies)
                if second_currencies and isinstance(second_currencies, str):
                    second_currencies = split_str_to_list(second_currencies)
                if currency_pairs and isinstance(currency_pairs, str):
                    currency_pairs = split_str_to_list(currency_pairs)

            if exchanges:
                exchange_names = [name.upper() for name in exchanges]
                data = data.filter(Exchange.name.in_(exchange_names))

            if first_currencies:
                first_currency_names = [name.upper() for name in first_currencies]
                data = data.filter(first.name.in_(first_currency_names))
            if second_currencies:
                second_currency_names = [name.upper() for name in second_currencies]
                data = data.filter(second.name.in_(second_currency_names))
            if currency_pairs:
                currency_pairs_names = [(pair.split("-")[0].upper(), pair.split("-")[1].upper()) for pair in
                                        currency_pairs]
                currency_pairs_names = [[item.replace(" ", "") for item in pair] for pair in currency_pairs_names]

                data = data.filter(tuple_(first.name, second.name).in_(currency_pairs_names))

            if from_timestamp:
                data = data.filter(db_table.time >= from_timestamp)
            if to_timestamp:
                data = data.filter(db_table.time <= to_timestamp)
    ...
    def get_first_timestamp(self, table: DatabaseTable, exchange_pair_id: int, last_row_id: int) -> datetime:
        ...
        with self.session_scope() as session:
            ...
            (earliest_timestamp,) = session \
                ...
                .first()
            (newest_timestamp,) = session \
                .query(func.max(table.time)) \
                .filter(table.exchange_pair_id == exchange_pair_id) \
                .first()

        # two days as some exchanges lag behind one day for historic_rates
        if earliest_timestamp and (TimeHelper.now() - newest_timestamp) < timedelta(days=2):
