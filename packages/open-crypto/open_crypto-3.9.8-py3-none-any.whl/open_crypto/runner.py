...
def _check_path(path: Union[Path, str], check_only: bool = False) -> bool:
    """
    ...
    @type path: str
    @return True if path exists, else either return False or copy_resources.
    """
    destination = Path(path).joinpath("resources")
...
def copy_resources(directory: Union[Path, str] = os.getcwd()) -> None:
    ...
    source = Path(_paths.all_paths.get("package_path")).joinpath("/resources").__str__()
    destination = Path(directory).joinpath("resources").__str__()
...
def get_config_template(csv: bool = False) -> None:
    ...
    path_exists = os.path.exists(_paths.all_paths.get("user_config_path"))
    if not path_exists:
...
def run(configuration_file: Optional[str] = None, kill_after: int = None) -> None:
    ...
    _check_path(Path(working_directory))
