...
def get_prices(ticker,fromdate,todate,adj=False,retry_count=3,pause=1):
    ...
    try:
        ...
        print("  #Warning(get_prices): price retrieving failed from Sina for",printmsg)
        return None 
    else:
    ...
    try:
        ...
        print("  #Warning(get_prices): price retrieving failed from stooq for",printmsg)
        return None                 
...
def get_price_ak_cn(ticker,fromdate,todate,adjust='none'):
    ...
    if adjust == 'none':
        try:
            ...
            try:
                ...
                print("  #Error(get_price_ak_cn): failed to find prices for",printmsg)
                return None
    ...
    num=len(df2)
    if num==0:
        print("  #Error(get_price_ak_cn): found zero record for",printmsg)
        return None
    ...
    df2['footnote']=adjust    
    print("  Successfully retrieved",num,"records for",printmsg)
...
def get_price_ak_us(symbol, fromdate, todate, adjust=""):
    ...
    try:
        ...
        print("  #Error(get_price_ak_us): no info found for",printmsg)
        return None
    ...
    df2=df1[df1.index <=end] 
    if df2 is None:
        print("  #Error(get_price_ak_us): failed to find prices for",printmsg)
        return None    
    num=len(df2)
    if num==0:
        print("  #Error(get_price_ak_us): found zero record for",printmsg)
        return None 
...
def get_price_ak_hk(symbol, fromdate, todate, adjust=""):
    ...
    df2=df1[df1.index <=end] 
    if df2 is None:
        print("  #Error(get_price_ak_hk): failed to find prices for",printmsg)
        return None
    num=len(df2)
    if num==0:
        print("  #Error(get_price_ak_hk): found zero record for",printmsg)
        return None
