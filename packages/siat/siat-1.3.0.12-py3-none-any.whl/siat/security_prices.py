...
def get_prices(ticker,fromdate,todate,adj=False,retry_count=3,pause=1):
    ...
    #替代雅虎数据源：基于FRED拦截部分国际指数
    print("  Trying to capture info from FRED for",ticker)
    prices=get_index_fred(ticker,fromdate,todate)
    ...
    #替代雅虎数据源：临时使用恒生指数ETF替代恒生指数，价格可能不对，但收益率和波动基本一致
    if ticker=='^HSI':
        ticker='2833.HK'
        prices=get_price_ak_hk(ticker,fromdate,todate)
        prices['source']='Sina Finance'
        prices['ticker']=str(ticker)
        prices['footnote']='临时使用恒指ETF替代恒生指数'
        num=len(prices)
        if num > 0:
            print("  Successfully retrieved",num,"records for",ticker)
            print("  #Warning(get_prices): tentatively replaced Hang Seng Index by",ticker)
            return prices
        else:
            print("  #Error(get_prices): zero record found for",ticker)
            return None        
