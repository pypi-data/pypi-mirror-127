...
def ignore_weekends(df , from_date, to_date):
  if from_date in df.index:
      if to_date in df.index:
        ...
        df1 = df[str(from_date) : str(date1)]
        return df1
      ...
      if to_date in df.index:
        ...
        date += datetime.timedelta(days=2)
        date1 = date.strftime("%d/%m/%Y")
        df1 = df[str(date1) : str(to_date)]
        return df1
      else:
        date = datetime.datetime.strptime(from_date, '%d/%m/%Y')
        date += datetime.timedelta(days=2)
        date1 = date.strftime("%d/%m/%Y")
        date11 = datetime.datetime.strptime(to_date, '%d/%m/%Y')
        date11 += datetime.timedelta(days=2)
        date2 = date.strftime("%d/%m/%Y")
        df1 = df[str(date1) : str(date2)]
        return df1
