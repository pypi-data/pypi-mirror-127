...
class LuceneTextCleanerTools:
    ...
    def create_sanitized_property_for_lucene_index(
        ...
        cyper_action = "SET n.{target_property} = {property}"
        ...
        cyper_action = f"SET n.{target_property} = apoc.text.regreplace(n.{target_property},'[^\S ]+|([ ]{2,})',' ')"
    ...
    def find_sanitized_properties_unsuitable_for_lucene_index(
        ...
        if match_properties_equal_to_common_word:
            if delete_node_instead_of_tagging:
                # delete action
                cypherAction = (
                    f"WITH w MATCH (n:{':'.join(match_labels)}) WHERE t.{check_property} = w.word DELETE n",
                )
            else:
                # tag action
                cypherAction = (
                    f"WITH w MATCH (n:{':'.join(match_labels)}) WHERE t.{check_property} = w.word SET n:{':'.join(tag_with_labels)}",
                )
            run_periodic_iterate(
                self.graph,
    ...
    def import_common_words(
        ...
        for lang, word_list_url in self.word_list_url.items():
            for index, line in enumerate(urllib.request.urlopen(word_list_url)):
                if index > 3:
                    ...
                    # todo: you are here. create :Word nodes from these vars
        ...
        # self.graph.run(f"MATCH (n:{':'.join(labels)}) WHERE ")
