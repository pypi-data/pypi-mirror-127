...
class FullTextIndexBucketMultiProcessor:
    ...
    def add_iterate_node_collection(self, name, query):
        self.iterate_collection_buckets = nodes_to_buckets_distributor(
            self.graph,
            query,
    ...
    def add_fulltext_node_collection(self, name, query, fulltext_properties: List[str]):
        self.fulltext_collection_buckets = nodes_to_buckets_distributor(
            self.graph,
            query,
            bucket_count=self.buckets_count_per_collection,
            bucket_label_prefix=f"{self._full_text_bucket_prefix}{self.id}",
        )
    ...
    def _wait_for_fulltextindex_build(self):
        pass
