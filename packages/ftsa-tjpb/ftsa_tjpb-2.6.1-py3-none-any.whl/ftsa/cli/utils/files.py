...
def get_os_lib_base_dir():
    for path in site.getsitepackages():
        if 'site-packages' in path or 'dist-packages' in path:
            if os.path.isdir(os.path.join(path, 'ftsa')):
                return os.path.join(path, f'ftsa{os.sep}cli{os.sep}projects{os.sep}')
...
OS_LIB_BASE_DIR = get_os_lib_base_dir()
...
def get_base_project_path():
    return DIR_LIB_BASE_PROJECT


def get_service_project_path():
    return DIR_LIB_SERVICE_PROJECT


def get_example_project_path():
    return DIR_LIB_EXAMPLE_PROJECT


def get_commons_project_path():
    return DIR_LIB_COMMONS


def get_commons_project_properties_file():
    return DIR_LIB_PROP_FILE
