...
def trimmerDroneHover(dynamic, HE, nprops):
    """
    Trimmer for horizontal steady flight (cruize)
    ...
    wlim = [-20, 20]
    vlim = [-50, 50]
    thetalim = [radians(-50), radians(50)]
    plim = [radians(-20), radians(20)]
    qlim = [radians(-20), radians(20)]
    rlim = [radians(-20), radians(20)]
    delta_p_lim = [0,1]
    
    boundary1 = [wlim[0], vlim[0], thetalim[0], plim[0], qlim[0], rlim[0]]
    boundary2 = [wlim[1], vlim[1], thetalim[1], plim[1], qlim[1], rlim[1]]
    
    for i in range(nprops):
        boundary1.append(delta_p_lim[0])
        boundary2.append(delta_p_lim[1])

    boundary2 = tuple(boundary2)
    boundary1 = tuple(boundary1)
...
def Mysolverodeint(dinamica, estados, time, dt):
    pass