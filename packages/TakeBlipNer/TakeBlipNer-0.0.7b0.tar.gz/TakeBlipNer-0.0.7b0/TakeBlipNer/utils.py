...
def unlexicalize(sentence, vocab):
    return [vocab.f2i[word] if word in vocab else vocab.f2i['<unk>'] for word
            in sentence]


def lexicalize_sequence(sequences, lens, vocab):
    return [lexicalize(sequence, vocab) for sequence in
            tighten(sequences, lens)]


def line_pre_process(sentence):
    return pre_process(sentence, PREPROCESS_OPTION)


def create_save_file(path, use_lstm_pred=False):
    with open(path, 'w') as file:
        writer = csv.writer(file, delimiter='|', lineterminator='\n')
        writer_parameter = ['Sentence', 'POS Tagging', 'NER', 'LSTM'] if \
            use_lstm_pred else ['Sentence', 'POS Tagging', 'NER']
