...
class LSTMCRF_NER(nn.Module):
    def __init__(self, crf, vocab_sizes, word_dims, hidden_dim, layers,
        ...
        self.embeddings = nn.ModuleList([nn.Embedding(vocab_size, word_dim).to(self.device)
                                         for vocab_size, word_dim in zip(vocab_sizes, word_dims)]).to(self.device)
    ...
    def update_embedding(self, vocab_size):
        self.embeddings[0] = nn.Embedding(vocab_size, self.word_dims[0]).to(self.device)
    ...
    def _embeddings(self, sequences_batch):
        ...
        embedded_sequence_combination = [emb(sequence) for emb, sequence in zip(self.embeddings.to(self.device), sequences_batch.to(self.device))]
        embedded_sequence_combination_cat = torch.cat(embedded_sequence_combination, 2)

        return embedded_sequence_combination_cat

    def _forward_bilstm(self, sequences_batch, lens):
        n_feats, batch_size, seq_len = sequences_batch.size()

        embedded_sequences_batch = self._embeddings(sequences_batch)
        embedded_sequences_batch = embedded_sequences_batch.view(-1, self.total_word_dim)
