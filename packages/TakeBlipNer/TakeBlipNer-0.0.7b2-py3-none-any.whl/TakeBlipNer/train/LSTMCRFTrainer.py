...
class LSTMCRFTrainer(object):
    def __init__(self, bilstmcrf_model: nermodel.LSTMCRF_NER, input_vocab,
                 input_path, label_vocab, postag_model, postag_label_vocab,
                 save_dir, tensorboard, ckpt_period, val, val_period, samples,
    ...
    def validate(self, steps=None):
        ...
        for i_idx, (batch, lens, _) in enumerate(val_data):
            ...
            sequence_tighten = [utils.tighten(sub_list, lens_s) for
                                sub_list in sequence_batch]
            sequence_batch_all = [
                utils.lexicalize_sequence(sequence, lens_s, vocab) for
                sequence, vocab in zip(sequence_tighten, self.inputs_vocab)]
            ...
            if not sampled and self.samples > 0:
                ...
                self.display_samples(sequence_batch_smp, label_batch_smp,
                                     lstm_smp, crf_smp)
