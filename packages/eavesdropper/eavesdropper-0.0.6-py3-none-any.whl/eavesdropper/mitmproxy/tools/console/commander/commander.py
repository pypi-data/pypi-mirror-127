...
import eavesdropper.mitmproxy.command
import eavesdropper.mitmproxy.flow
import eavesdropper.mitmproxy.master
...
class CompletionState(typing.NamedTuple):
    completer: Completer
    parsed: typing.Sequence[eavesdropper.mitmproxy.command.ParseResult]


class CommandBuffer:
    def __init__(self, master: eavesdropper.mitmproxy.master.Master, start: str = "") -> None:
        self.master = master
        self.text = start
    ...
    def render(self):
        ...
        if not parts:
            ...
            for p in parts:
                if p.valid:
                    if p.type == eavesdropper.mitmproxy.types.Cmd:
            ...
            if remaining:
                if parts[-1].type != eavesdropper.mitmproxy.types.Space:
                    ret.append(("text", " "))
    ...
    def cycle_completion(self, forward: bool = True) -> None:
        if not self.completion:
            ...
            if parts and parts[-1].type != eavesdropper.mitmproxy.types.Space:
                ...
                return
            ct = eavesdropper.mitmproxy.types.CommandTypes.get(type_to_complete, None)
            if ct:
...
class CommandEdit(urwid.WidgetWrap):
    ...
    def __init__(self, master: eavesdropper.mitmproxy.master.Master, text: str) -> None:
