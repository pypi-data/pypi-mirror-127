...
import eavesdropper.mitmproxy.net.http.url
from eavesdropper.mitmproxy.coretypes import multidict
from eavesdropper.mitmproxy.net.http import cookies, multipart
from eavesdropper.mitmproxy.net.http import message
from eavesdropper.mitmproxy.net.http.headers import Headers
from eavesdropper.mitmproxy.utils.strutils import always_bytes, always_str
...
class Request(message.Message):
    ...
    def host(self, val: Union[str, bytes]) -> None:
        ...
        if self.data.authority:
            self.authority = eavesdropper.mitmproxy.net.http.url.hostport(self.scheme, self.host, self.port)
    ...
    def url(self) -> str:
        ...
        return eavesdropper.mitmproxy.net.http.url.unparse(self.scheme, self.host, self.port, self.path)

    @url.setter
    def url(self, val: Union[str, bytes]) -> None:
        val = always_str(val, "utf-8", "surrogateescape")
        self.scheme, self.host, self.port, self.path = eavesdropper.mitmproxy.net.http.url.parse(val)
    ...
    def pretty_host(self) -> str:
        ...
        if authority:
            return eavesdropper.mitmproxy.net.http.url.parse_authority(authority, check=False)[0]
        else:
            return self.host
    ...
    def pretty_url(self) -> str:
        ...
        pretty_host, pretty_port = eavesdropper.mitmproxy.net.http.url.parse_authority(host_header, check=False)
        pretty_port = pretty_port or eavesdropper.mitmproxy.net.http.url.default_port(self.scheme) or 443

        return eavesdropper.mitmproxy.net.http.url.unparse(self.scheme, pretty_host, pretty_port, self.path)

    def _get_query(self):
        query = urllib.parse.urlparse(self.url).query
        return tuple(eavesdropper.mitmproxy.net.http.url.decode(query))

    def _set_query(self, query_data):
        query = eavesdropper.mitmproxy.net.http.url.encode(query_data)
    ...
    def path_components(self):
        ...
        return tuple(eavesdropper.mitmproxy.net.http.url.unquote(i) for i in path.split("/") if i)

    @path_components.setter
    def path_components(self, components):
        components = map(lambda x: eavesdropper.mitmproxy.net.http.url.quote(x, safe=""), components)
    ...
    def _get_urlencoded_form(self):
        ...
        if is_valid_content_type:
            return tuple(eavesdropper.mitmproxy.net.http.url.decode(self.get_text(strict=False)))
        return ()
    ...
    def _set_urlencoded_form(self, form_data):
        ...
        self.content = eavesdropper.mitmproxy.net.http.url.encode(form_data, self.get_text(strict=False)).encode()
    ...
    def _set_multipart_form(self, value):
        self.content = eavesdropper.mitmproxy.net.http.multipart.encode(self.headers, value)
