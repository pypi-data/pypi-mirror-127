...
import eavesdropper.mitmproxy.types
from eavesdropper.mitmproxy import exceptions, command_lexer
from eavesdropper.mitmproxy.command_lexer import unquote
...
def typename(t: type) -> str:
    ...
    to = eavesdropper.mitmproxy.types.CommandTypes.get(t, None)
    if not to:
...
class Command:
    ...
    def __init__(self, manager: "CommandManager", name: str, func: typing.Callable) -> None:
        ...
        for name, parameter in self.signature.parameters.items():
            ...
            if not eavesdropper.mitmproxy.types.CommandTypes.get(parameter.annotation, None):
                raise exceptions.CommandError(f"Argument {name} has an unknown type ({_empty_as_none(t)}) in {func}.")
        if self.return_type and not eavesdropper.mitmproxy.types.CommandTypes.get(self.return_type, None):
    ...
    def call(self, args: typing.Sequence[str]) -> typing.Any:
        ...
        if ret is None and self.return_type is None:
            return
        typ = eavesdropper.mitmproxy.types.CommandTypes.get(self.return_type)
        assert typ
...
class CommandManager:
    ...
    def parse_partial(
        ...
        next_params: typing.List[CommandParameter] = [
            CommandParameter("", eavesdropper.mitmproxy.types.Cmd),
            CommandParameter("", eavesdropper.mitmproxy.types.CmdArgs),
        ]
        ...
        for part in parts:
            if part.isspace():
                parsed.append(
                    ParseResult(
                        value=part,
                        type=eavesdropper.mitmproxy.types.Space,
                        valid=True,
                    )
                )
                continue
            ...
            if expected and expected.kind is inspect.Parameter.VAR_POSITIONAL:
                ...
                expected = CommandParameter("", eavesdropper.mitmproxy.types.Unknown)

            arg_is_known_command = (
                    expected.type == eavesdropper.mitmproxy.types.Cmd and part in self.commands
            )
            arg_is_unknown_command = (
                    expected.type == eavesdropper.mitmproxy.types.Cmd and part not in self.commands
            )
            command_args_following = (
                    next_params and next_params[0].type == eavesdropper.mitmproxy.types.CmdArgs
            )
            ...
            to = eavesdropper.mitmproxy.types.CommandTypes.get(expected.type, None)
            valid = False
            if to:
                try:
    ...
    def execute(self, cmdstr: str) -> typing.Any:
        ...
        command_name, *args = [
            unquote(part.value)
            for part in parts
            if part.type != eavesdropper.mitmproxy.types.Space
        ]
...
def parsearg(manager: CommandManager, spec: str, argtype: type) -> typing.Any:
    ...
    """
    t = eavesdropper.mitmproxy.types.CommandTypes.get(argtype, None)
    if not t:
