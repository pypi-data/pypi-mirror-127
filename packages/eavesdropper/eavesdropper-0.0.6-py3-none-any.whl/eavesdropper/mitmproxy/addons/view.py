...
import eavesdropper.mitmproxy.flow
from eavesdropper.mitmproxy import flowfilter
from eavesdropper.mitmproxy import exceptions
from eavesdropper.mitmproxy import command
from eavesdropper.mitmproxy import connections
from eavesdropper.mitmproxy import ctx
from eavesdropper.mitmproxy import io
...
class _OrderKey:
    ...
    def generate(self, f: eavesdropper.mitmproxy.flow.Flow) -> typing.Any:  # pragma: no cover
        pass
...
class OrderRequestStart(_OrderKey):
    def generate(self, f: eavesdropper.mitmproxy.flow.Flow) -> float:
        return f.timestamp_start


class OrderRequestMethod(_OrderKey):
    def generate(self, f: eavesdropper.mitmproxy.flow.Flow) -> str:
...
class OrderKeySize(_OrderKey):
    def generate(self, f: eavesdropper.mitmproxy.flow.Flow) -> int:
        if isinstance(f, http.HTTPFlow):
            size = 0
...
class View(collections.abc.Sequence):
    ...
    def _bisect(self, f: eavesdropper.mitmproxy.flow.Flow) -> int:
        v = self._view.bisect_right(f)
        return self._rev(v - 1) + 1

    def index(self, f: eavesdropper.mitmproxy.flow.Flow, start: int = 0, stop: typing.Optional[int] = None) -> int:
    ...
    def getvalue(self, flow: eavesdropper.mitmproxy.flow.Flow, key: str, default: str) -> str:
        """
    ...
    def setvalue_toggle(
        self,
        flows: typing.Sequence[eavesdropper.mitmproxy.flow.Flow],
        key: str
    ) -> None:
        """
    ...
    def duplicate(self, flows: typing.Sequence[eavesdropper.mitmproxy.flow.Flow]) -> None:
        """
    ...
    def remove(self, flows: typing.Sequence[eavesdropper.mitmproxy.flow.Flow]) -> None:
        """
    ...
    def resolve(self, flow_spec: str) -> typing.Sequence[eavesdropper.mitmproxy.flow.Flow]:
        """
    ...
    def load_file(self, path: eavesdropper.mitmproxy.types.Path) -> None:
        """
    ...
    def add(self, flows: typing.Sequence[eavesdropper.mitmproxy.flow.Flow]) -> None:
        """
    ...
    def get_by_id(self, flow_id: str) -> typing.Optional[eavesdropper.mitmproxy.flow.Flow]:
        """
    ...
    def update(self, flows: typing.Sequence[eavesdropper.mitmproxy.flow.Flow]) -> None:
        """
...
class Focus:
    ...
    def __init__(self, v: View) -> None:
        self.view = v
        self._flow: typing.Optional[eavesdropper.mitmproxy.flow.Flow] = None
    ...
    @property
    def flow(self) -> typing.Optional[eavesdropper.mitmproxy.flow.Flow]:
        return self._flow

    @flow.setter
    def flow(self, f: typing.Optional[eavesdropper.mitmproxy.flow.Flow]):
...
class Settings(collections.abc.Mapping):
    ...
    def __getitem__(self, f: eavesdropper.mitmproxy.flow.Flow) -> dict:
