...
from eavesdropper.mitmproxy import connections, flow  # noqa
from eavesdropper.mitmproxy import exceptions
from eavesdropper.mitmproxy import http
from eavesdropper.mitmproxy.proxy.protocol import base
from eavesdropper.mitmproxy.proxy.protocol import http as httpbase
import eavesdropper.mitmproxy.net.http
from eavesdropper.mitmproxy.net import tcp
from eavesdropper.mitmproxy.coretypes import basethread
from eavesdropper.mitmproxy.net.http import http2, headers, url
...
class Http2Layer(base.Layer):
    ...
    def _handle_request_received(self, eid, event):
        headers = eavesdropper.mitmproxy.net.http.Headers([[k, v] for k, v in event.headers])
    ...
    def _handle_trailers(self, eid, event, is_server, other_conn):
        trailers = eavesdropper.mitmproxy.net.http.Headers([[k, v] for k, v in event.headers])
...
class Http2SingleStreamLayer(httpbase._HttpTransmissionLayer, basethread.BaseThread):
    ...
    class Message:
        def __init__(self, headers=None):
            self.headers: Optional[eavesdropper.mitmproxy.net.http.Headers] = headers  # headers are the first thing to be received on a new stream
            ...
            self.trailers: Optional[eavesdropper.mitmproxy.net.http.Headers] = None  # trailers are received after stream_ended is set
    ...
    def __init__(self, ctx, h2_connection, stream_id: int, request_headers: eavesdropper.mitmproxy.net.http.Headers) -> None:
        super().__init__(
