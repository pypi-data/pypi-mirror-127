...
import enum
import time
from typing import NamedTuple, List, Set, Iterable, Dict
import re
import struct
import functools
...
class EntityRef():
    def __init__(self, parent: 'EntityRef', name: str, key, entity_type: EntityType, net_type: NetType):
        self.parent = parent
        self.name = name
        self.key = key
...
def parse_expr(val, unit, default):
    val = val.strip()
    if val == '':
        if default is None:
            return None
...
class MotorConfig(ConfigType):
    ...
    def parse_scale(self, unit=''): return parse_expr(self.scale, unit, 1.0)
    def parse_phase_resistance(self, unit=''): return parse_expr(self.phase_resistance, unit, None)
    def parse_phase_inductance(self, unit=''): return parse_expr(self.phase_inductance, unit, None)
...
class AxisConfig(ConfigType):
    ...
    def parse_calib_scan_vel(self, unit=''): return parse_expr(self.calib_scan_vel, unit, '0.275 rps')
    def parse_calib_scan_distance(self, unit=''): return parse_expr(self.calib_scan_distance, unit, '1.1 turns')
    def parse_calib_scan_range(self, unit=''): return parse_expr(self.calib_scan_range, unit, '0.02')
    def parse_calib_torque(self, unit=''): return parse_expr(self.calib_torque, unit, None) # TODO: define default
    ...
    def get_status(self, odrives):
        ...
        for k, o in odrives.items():
            if not k in [c.serial_number for c in self.devices]:
                issues.append(ref, "Unused ODrive: " + k, IssueType.WARN)
        ...
        for net in self.get_nets(NetType.RS485):
            ...
            for enc_ref in enc_refs:
                ...
                encoder_ids[rs485_intf_ref.parent.num][enc_ref] = ENCODER_ID_AMT21_ENCODER0
        ...
        for net in self.get_nets(NetType.AB):
            ...
            for inc_enc_intf_ref in inc_enc_intf_refs:
                ...
                encoder_ids[inc_enc_intf_ref.parent.num][enc_ref] = [ENCODER_ID_INC_ENCODER0, ENCODER_ID_INC_ENCODER1][inc_enc_intf_ref.key]
        ...
        for axis_ref in ref.axes():
            ...
            axis_calib[axis_ref.num].append(CalibrationTask(
                ...
                (CalibrationStatus.OK if ((not axis is None) and axis.commutation_mapper.status == COMPONENT_STATUS_NOMINAL) else
                CalibrationStatus.NEEDED if ((not axis is None) and axis.commutation_mapper.status == COMPONENT_STATUS_RELATIVE_MODE) else
...
def _load_py_obj(data, py_type):
    ...
    elif hasattr(py_type, '_name') and py_type._name == 'List':
