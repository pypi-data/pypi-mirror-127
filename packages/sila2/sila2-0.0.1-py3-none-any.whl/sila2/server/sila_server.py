...
from sila2.server.features.silaservice_impl import SiLAService, SiLAServiceImpl
...
silaservice_fdl_tree = parse_feature_definition(SiLAService._feature_definition)
...
class SilaServer:
    ...
    def __init__(
        ...
        self.child_task_executor = ThreadPoolExecutor(
            max_workers=max_child_task_workers, thread_name_prefix=f"observable-command-executor-{self.server_uuid}"
        )

        self.children_by_fully_qualified_identifier = {}

        self.set_feature_implementation(SiLAService, SiLAServiceImpl(parent_server=self))
