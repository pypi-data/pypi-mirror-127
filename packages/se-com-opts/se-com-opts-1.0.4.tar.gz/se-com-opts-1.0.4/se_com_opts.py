...
class SetChromeOptions(object):
    ...
    Usage:
        ...
        SetChromeOptions(webdriverm, extension_path = <extension_path>).options: 返回使用crx插件的设置项
    """
    options = None
    def __init__(self,webdriver,
        ...
        Args:
            ...
            extension_path：浏览器插件的地址，默认为None，插件必须为打包好的crx格式，不能与无窗口模式同时使用
        ...
        if extension_path != None:
            if headless == True:
                ...
                for e in os.listdir(extension_path):
                    path = (extension_path + '\\' + e).replace('\\','/')
