...
class xmmanalysis(object):
    ...
    def sas_get_extraction_region(self, min_psf=3, max_psf=30, default_psf=1.0,
        ...
        min_psf=3  The minimum PSF in pixels of 80 raw units
        max_psf=30 The minimum PSF in pixels of 80 raw units
        default_psf=1.0 
        relative_x_offset=-2 X Localisation of the background relative to the source position in units of the starting 
                              extraction radius (shon in dashed line)
        relative_y_offset=+2 Y Localisation of the background relative to the source position in units of the starting 
                                extraction radius (shon in dashed line)
        forced_r_pix=-1  Force the radius of extraction region if larger than zero (80 raw units) 
        use_max_coord=False Use the maximum value in the image rather than the source position
        ra_target=np.nan Override the source position derived from the observation if different from nan
        dec_target=np.nan Override the source position derived from the observation if different from nan
        input_image=None Input image (default uses the full range)
        criterion='surface' The surface brightness criterion is the default and most used (Extraction radius is 
        make_plots=True Keep it true
        input_exp_map=None The exposure map to be used
        delta_r=2 Step of the exploring the surface brightness (units of 80 raw)
        critical_surface_brightness=1 parameter to regulate the level of getting the radius of extraction region 
