...
class ESAMLearner(SAMLearner):
    ...
    def _update_preconditions_cnfs(self, action_name: str,
        ...
        current_triplet_predicates_names = [predicate.name for predicate in possible_preconditions]
        stored_precondition_predicate_names = [name for name in self.precondition_cnfs[action_name]]
        intersection_predicates = set(stored_precondition_predicate_names).intersection(
            current_triplet_predicates_names)

        new_precondition_predicates = {}
        for predicate_name in intersection_predicates:
            filtered_predicates = []
            stored_predicates = sort_predicates(action_preconditions[predicate_name])
            for predicate in stored_predicates:
                if predicate in possible_preconditions:
                    filtered_predicates.append(predicate)

            new_precondition_predicates[predicate_name] = set(filtered_predicates)

        self.precondition_cnfs[action_name] = new_precondition_predicates
