...
class VesslRun(object):
    ...
    def _send_without_collector(self, payloads):
        assert self._mode == MODE_NOT_STARTED
    ...
    def log(self, payload: Dict[str, Any], step: Optional[int] = None) -> int:
        ...
        if metric_payload or step is not None:
        ...
        if metric_payload:
            invalid_keys = [
                k for k in metric_payload.keys() if not self._is_metric_key_valid(k)
            ]
            if invalid_keys:
                logger.warning(
    ...
    def progress(self, value: float):
        ...
        if not 0 < value <= 1:
            logger.warning(f"Invalid progress value {value}. Must be between 0 and 1.")
            return

        logger.debug(f"Updating experiment progress: {value}")
        self.api.experiment_progress_update_api(
            self._experiment.id,
