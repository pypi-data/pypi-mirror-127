...
class BlurDetector:
    ...
    def __call__(
        ...
        self._size = int(self._h/size_ratio)
        ...
        self._magnitude = self._thresh * np.log(
            np.abs(self._recon)
        )
...
class BlurDetectorSpat(object):
    ...
    def __computeFrequencyBands(self):
        for current_scale in self.scales:
            ...
            for i in range(current_scale):
                matrixInds[0: max(
                    0, int(((current_scale-1)/2) - i + 1)), i] = 1

            for i in range(current_scale):
                if (current_scale-((current_scale-1)/2) - i) <= 0:
                    matrixInds[0:current_scale - i - 1, i] = 2
                else:
                    matrixInds[int(current_scale - ((current_scale - 1) / 2) -
                                   i - 1): int(current_scale - i - 1), i] = 2
            matrixInds[0, 0] = 3
            self.__freqBands.append(matrixInds)

    def __dctmtx(self, n):
        [mesh_cols, mesh_rows] = np.meshgrid(
            np.linspace(0, n-1, n), np.linspace(0, n-1, n))
        dct_matrix = np.sqrt(
            2/n) * np.cos(np.pi * np.multiply((2 * mesh_cols + 1), mesh_rows) / (2*n))
    ...
    def __createDCT_Matrices(self):
        if(len(self.__dct_matrices) > 0):
            raise TypeError(
                "dct matrices are already defined. Redefinition is not allowed.")
    ...
    def RF(self, img, joint_img):
        ...
        for i in range(self.num_iterations_RF_filter):
            ...
            sigma_H_i = self.sigma_s_RF_filter * \
                np.sqrt(3) * 2 ** (N - (i + 1)) / np.sqrt(4 ** N - 1)
            F = self.TransformedDomainRecursiveFilter_Horizontal(
                F, dHdx, sigma_H_i)
            F = np.transpose(F)
    ...
    def detectBlur(
        ...
        __padded_image = np.pad(__gradient_image, int(
            np.floor(max(self.scales)/2)), mode='constant')

        rows, cols = np.shape(__padded_image)
        L = []

        total_num_points = len([i for i in range(int(max(self.scales)/2), rows - int(max(self.scales)/2), self.downsampling_factor)]) * \
            len([j for j in range(int(max(self.scales) / 2), cols -
                int(max(self.scales) / 2), self.downsampling_factor)])
        ...
        for i in range(int(max(self.scales)/2), rows - int(max(self.scales)/2), self.downsampling_factor):
            ...
            for j in range(int(max(self.scales) / 2), cols - int(max(self.scales) / 2), self.downsampling_factor):
                ...
                for ind, curr_scale in enumerate(self.scales):
                    Patch = __padded_image[i-np.int(curr_scale/2): i+np.int(
                        curr_scale/2) + 1, j-np.int(curr_scale/2): j+np.int(curr_scale/2) + 1]
                    dct_coefficients = np.abs(
                        self.__getDCTCoefficients(Patch, ind))

                    # store all high frequency components
                    high_freq_components.append(
                        dct_coefficients[self.freq_index[ind]])
                ...
                result = np.argpartition(
                    high_freq_components, total_num_layers)
        ...
        aSmooth = cv2.GaussianBlur(
            resized_input_image, (3, 3), sigmaX=1, sigmaY=1)
