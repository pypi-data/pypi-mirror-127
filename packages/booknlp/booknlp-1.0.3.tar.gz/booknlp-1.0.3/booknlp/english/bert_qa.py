...
class QuotationAttribution:
	...
	def get_representation(self, quotes, entities, tokens):
		...
		for q_id, (start_tok, end_tok) in enumerate(quotes):
			...
			lastPar=None
			while start > 0 and count < window and wp_tok_count < 450:
				if in_quotes[start] == 0:
					count+=1
					wp_tok_count+=len(self.model.tokenizer.tokenize(tokens[start].text))
			...
			while end < len(tokens) and count < window and wp_tok_count < 475:
				if in_quotes[end] == 0:
					count+=1
					wp_tok_count+=len(self.model.tokenizer.tokenize(tokens[end].text))
