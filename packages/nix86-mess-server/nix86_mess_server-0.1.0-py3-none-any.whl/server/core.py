...
class MessageProcessor(threading.Thread):
    ...
    def process_message(self, message):
        ...
        if message[DESTINATION] in self.names and self.names[message[DESTINATION]] in self.listen_sockets:
            ...
            logger.error(
                ...
                'сервере, отправка сообщения невозможна.')
    ...
    def process_client_message(self, message, client):
        ...
        # Если это запрос контакт-листа
            ...
            response[LIST_INFO] = self.database.get_contacts(message[SENDER])
            try:
                send_message(client, response)
            except OSError:
                self.remove_client(client)
        ...
        # Если это добавление контакта
            ...
            and SENDER in message and CONTACT in message \
                and self.names[message[SENDER]] == client:
            self.database.add_contact(message[SENDER], message[CONTACT])
            try:
                send_message(client, RESPONSE_200)
            except OSError:
                self.remove_client(client)
        ...
        # Если это удаление контакта
            ...
            and SENDER in message and CONTACT in message \
                and self.names[message[SENDER]] == client:
            self.database.remove_contact(message[SENDER], message[CONTACT])
            try:
                send_message(client, RESPONSE_200)
            except OSError:
                self.remove_client(client)
        ...
        # Если это запрос публичного ключа пользователя
            ...
            response[DATA] = self.database.get_pubkey(message[SENDER])
    ...
    def autorize_user(self, message, sock):
        ...
        logger.debug(f'Start auth process for {message[SENDER]}')
        if message[SENDER] in self.names.keys():
            response = RESPONSE_400
        ...
        # Проверяем что пользователь зарегистрирован на сервере.
        elif not self.database.check_user(message[SENDER]):
            response = RESPONSE_400
            ...
            hash = hmac.new(self.database.get_hash(message[SENDER]), random_str, 'MD5')
            digest = hash.digest()
            ...
            if RESPONSE in ans and ans[RESPONSE] == 511 \
                ...
                self.names[message[SENDER]] = sock
