...
from clean_architecture_basic_classes.basic_routes.exceptions import (
    ValidationErrorException, UnexpectedErrorException, PutException)
...
def test_ber_put(mock_inter, mock_req):
    ...
    mock_inter.assert_called_with(mock_req.return_value, mock_adapter, mock_class)
    mock_inter.return_value.run.assert_called_once()
    assert result == mock_inter.return_value.run.return_value


@patch(Patches.PutRequest)
@patch(Patches.PutInteractor, return_value=MagicMock(run=MagicMock(side_effect=UpdateEntityException('gravou não'))))
def test_ber_put_exception(mock_inter, mock_req):
    ...
    assert 'Error while update' in str(exc_info.value)
    assert 'gravou não' in str(exc_info.value)


@patch(Patches.PutRequest)
@patch(Patches.PutInteractor, return_value=MagicMock(run=MagicMock(side_effect=ValidationError('123inválido321'))))
def test_ber_put_validation_exception(mock_inter, mock_req):
    ...
    assert 'Validation error while update' in str(exc_info.value)
    assert '123inválido321' in str(exc_info.value)


@patch(Patches.PutRequest)
@patch(Patches.PutInteractor, return_value=MagicMock(run=MagicMock(side_effect=ValueError('Oops!'))))
