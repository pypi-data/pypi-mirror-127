...
class DataSourceName(dsnparse.ParseResult):
    ...
    def create_connection(self, from_dsn):
        """Create a new connection instance using the factory function
        `from_dsn`.
        """
        return from_dsn(self.geturl())


def encode_dsn(opts: DTO) -> str:
    """Encodes the database configuration into a Data Source Name (DSN)."""
    dsn = dsnparse.parse(f'{opts.engine}://{opts.host}', DataSourceName)
