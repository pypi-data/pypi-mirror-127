...
if __name__ == "__main__":
    ...
    standard_noise = RandomVariable(np.random.normal, 0.5, 0.1)

    direct_x_clones, _ = standard_noise.apply_bias(size=[10, 100], min=0.0)
    biased_x_clones, _ = standard_noise.apply_bias(size=[10, 100], signal_prop=0.1, bias=0.1, min=0.0, max=1.0)
    ...
    my_dls = DependentLocalizedSignals(signal_dist=[dict1, dict2, dict3], noise_dist=noise, non_overlapping_obs=True,
                                       cloning_mult=10, n_crossproducts=0, n_noisecolumns=50, nsamples=100)
    foob = my_dls()
    tst.test_missing_proportion(my_dls._samples, ncomp=4, name='Random_S3x10_N50')
    pass
