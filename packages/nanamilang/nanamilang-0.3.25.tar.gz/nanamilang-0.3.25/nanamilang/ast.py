...
class AST:
    ...
    def evaluate(self) -> datatypes.Base:
        ...
        def recursive_evaluate(tree: list) -> datatypes.Base:
            ...
            if isinstance(identifier, Token):
                if isinstance(identifier.dt(), datatypes.Function):
                    return identifier.dt().reference()(arguments)
                else:
                    return datatypes.Nil('nil')
            elif isinstance(identifier, list):
                evaluated = recursive_evaluate(identifier)
                if isinstance(evaluated, datatypes.Function):
                    return evaluated.reference()(arguments)
                else:
                    return datatypes.Nil('nil')
