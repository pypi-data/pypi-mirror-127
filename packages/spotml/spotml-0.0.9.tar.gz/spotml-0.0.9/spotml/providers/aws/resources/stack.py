...
class Stack(object):
    ...
    def wait_tasks(self, tasks: List[Task], resource_success_status: str, resource_fail_status: str,
        ...
        resource_statuses = self._get_resource_statuses_and_set_error(resource_fail_status)
    ...
    def _get_resource_statuses_and_set_error(self, resource_fail_status: str) -> Dict[str, str]:
        stack_resources = self._cf.list_stack_resources(StackName=self.stack_id)

        for row in stack_resources['StackResourceSummaries']:
            if row['ResourceStatus'] == resource_fail_status and 'ResourceStatusReason' in row:
                self.error = row['ResourceStatusReason']
