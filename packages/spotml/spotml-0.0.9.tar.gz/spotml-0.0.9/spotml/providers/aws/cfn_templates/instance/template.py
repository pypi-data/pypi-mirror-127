...
from spotml.config.tmp_dir_volume import TmpDirVolume
from spotml.config.validation import is_subdir
from spotml.config.abstract_instance_volume import AbstractInstanceVolume
from spotml.deployment.container.docker.docker_commands import DockerCommands
from spotml.deployment.container.docker.scripts.container_bash_script import ContainerBashScript
from spotml.deployment.abstract_cloud_instance.file_structure import INSTANCE_SPOTML_TMP_DIR, \
    CONTAINER_BASH_SCRIPT_PATH, \
    INSTANCE_STARTUP_SCRIPTS_DIR, CONTAINERS_TMP_DIR
from spotml.providers.aws.cfn_templates.instance.start_container_script import StartContainerScriptWithCfnSignals
from spotml.providers.aws.helpers.ami import get_ami
from spotml.providers.aws.helpers.vpc import get_vpc_id
from spotml.providers.aws.resources.snapshot import Snapshot
from spotml.providers.aws.resources.volume import Volume
from spotml.providers.aws.config.instance_config import InstanceConfig
from spotml.providers.aws.config.ebs_volume import EbsVolume
from spotml.providers.aws.helpers.logs import get_logs_s3_path
...
def prepare_instance_template(ec2, instance_config: InstanceConfig, docker_commands: DockerCommands,
    ...
    cfn_init_configs = [
                ...
                INSTANCE_STARTUP_SCRIPTS_DIR + '/01_prepare_instance.sh': {
                    ...
                    'content': {
                        'Fn::Sub': _read_template_file(os.path.join('startup_scripts', '01_prepare_instance.sh'), {
                            ...
                            'SPOTML_TMP_DIR': INSTANCE_SPOTML_TMP_DIR,
        ...
        },
        {
            ...
            'files': {
                INSTANCE_STARTUP_SCRIPTS_DIR + '/07_check_host_status.py': {
                    'owner': 'ubuntu',
                    'group': 'ubuntu',
                    'mode': '000755',
                    'content': {
                        'Fn::Sub': _read_template_file(os.path.join('startup_scripts', '07_check_host_status.py')),
                    },
                },
            },
            'command': 'pwd',
