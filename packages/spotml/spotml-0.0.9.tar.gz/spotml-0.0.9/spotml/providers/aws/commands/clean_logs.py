...
class CleanLogsCommand(AbstractCommand):
    ...
    name = 'clean-logs'
    description = 'Delete expired CloudFormation log groups with spotml prefixes'

    def configure(self, parser: ArgumentParser):
        super().configure(parser)
        parser.add_argument('-r', '--region', type=str, required=True, help='AWS region')
        parser.add_argument('-a', '--delete-all', action='store_true', help='Delete all spotml log groups, '
    ...
    def run(self, args: Namespace, output: AbstractOutputWriter):
        ...
        prefixes = ['spotml-', '/aws/lambda/spotml-']
        only_empty = not args.delete_all

        output.write('Deleting %s spotml log groups...' % ('empty' if only_empty else 'all'))
