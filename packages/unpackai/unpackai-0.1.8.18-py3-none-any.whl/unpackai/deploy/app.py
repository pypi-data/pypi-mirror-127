...
select = st.sidebar.radio("How to load {{ input }}{% if multiple %}s{% endif %}?", ["from URL", "from file{% if multiple %}s{% endif %}"])
st.sidebar.write("---")
...
if select == "from URL":
    ...
    {% if multiple -%}
    files = st.sidebar.file_uploader("Choose {{ input }}{% if multiple %}s{% endif %}", accept_multiple_files=True)
    for file in files:  # type:ignore # this is an iterable
        display_prediction(file)
    {% else -%}
    file = st.sidebar.file_uploader("Choose {{ input }}", accept_multiple_files=True)
    if file:
        display_prediction(file)
    {% endif %}
"""
...
TEMPLATE_CV_FASTAI = TemplateCode(
    specific_import="""
        from .cv import get_image, get_learner, PathStr, dummy_function
    """
        ,
    load_model="""\
        {{ implem_4_model }}
...
class StreamlitApp:
    ...
    def _render(
        ...
        # We will need multiple for CV (load several images),
        # but not Tabular (single CSV) or NLP (text entry)
        multiple = (self.application == "CV")
        self.content = self.template.render(
            title=title,
            author=author,
            model=model,
            input=input_types.get(self.application),
            multiple=multiple,
            **kwargs,
        )
        return self
