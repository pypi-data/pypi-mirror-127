...
class Application(fastapi.FastAPI):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Expose the signing key that is used for the actor_token in a
        # RFC 8693 OAuth 2.0 Token Exchange. This is exposed at the URL
        # .well-known/jwks.json (TODO: prevent clashes).
        actor_key = os.getenv('OAUTH2_ACTOR_KEY')
        if actor_key is not None and os.path.exists(actor_key):
            key = crypto.fromfile(actor_key, public=True)
            jwk = key.jwk

            @self.get('/.well-known/jwks.json', tags=['OAuth 2.0'])
            def jwks():
                return {"keys": [jwk]}
