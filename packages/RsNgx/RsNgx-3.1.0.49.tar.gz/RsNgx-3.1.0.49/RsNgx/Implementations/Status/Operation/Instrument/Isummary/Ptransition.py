...
class Ptransition:
	...
	def set(self, arg_0: int, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.status.operation.instrument.isummary.ptransition.set(arg_0 = 1, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'STATus:OPERation:INSTrument:ISUMmary{channel_cmd_val}:PTRansition {param}')

	def get(self, channel=repcap.Channel.Default) -> int:
		"""SCPI: STATus:OPERation:INSTrument:ISUMmary<Channel>:PTRansition \n
		Snippet: value: int = driver.status.operation.instrument.isummary.ptransition.get(channel = repcap.Channel.Default) \n
		Sets or queries the positive transition filter. Setting a bit in the positive transition filter shall cause a 0 to 1
		transition in the corresponding bit of the associated condition register to cause a 1 to be written in the associated bit
		of the corresponding event register. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Isummary')
		...
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'STATus:OPERation:INSTrument:ISUMmary{channel_cmd_val}:PTRansition?')
