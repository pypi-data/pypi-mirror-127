...
class Dio:
	...
	def set(self, arg_0: enums.LowHigh, digitalIo=repcap.DigitalIo.Default) -> None:
		"""SCPI: TRIGger:LOGic:DIO<IO> \n
		Snippet: driver.trigger.logic.dio.set(arg_0 = enums.LowHigh.HIGH, digitalIo = repcap.DigitalIo.Default) \n
		...
		"""
		param = Conversions.enum_scalar_to_str(arg_0, enums.LowHigh)
		digitalIo_cmd_val = self._cmd_group.get_repcap_cmd_value(digitalIo, repcap.DigitalIo)
		self._core.io.write(f'TRIGger:LOGic:DIO{digitalIo_cmd_val} {param}')

	# noinspection PyTypeChecker
	def get(self, digitalIo=repcap.DigitalIo.Default) -> enums.LowHigh:
		"""SCPI: TRIGger:LOGic:DIO<IO> \n
		Snippet: value: enums.LowHigh = driver.trigger.logic.dio.get(digitalIo = repcap.DigitalIo.Default) \n
		Sets or queries the trigger logic (Active High/Active Low) of the specified Digital I/O line. \n
			:param digitalIo: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Dio')
			:return: arg_0: No help available"""
		digitalIo_cmd_val = self._cmd_group.get_repcap_cmd_value(digitalIo, repcap.DigitalIo)
		response = self._core.io.query_str(f'TRIGger:LOGic:DIO{digitalIo_cmd_val}?')
