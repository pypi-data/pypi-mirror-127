...
class RegisterUser(QDialog):
    ...
    def __init__(self, database, server):
        ...
        self.label_conf = QLabel('Please confirm:', self)
    ...
    def save_data(self):
        ...
        if not self.client_name.text():
            self.messages.critical(
                self, 'Error', 'Username is not indicated.')
            return
        elif self.client_passwd.text() != self.client_conf.text():
            self.messages.critical(
                self, 'Error', 'Inserted passwords are not matching.')
            return
        elif self.database.check_user(self.client_name.text()):
            self.messages.critical(
                self, 'Error', 'User is already exist.')
            return
        else:
            ...
            self.messages.information(
                self, 'Success', 'User is registered successfully.')
