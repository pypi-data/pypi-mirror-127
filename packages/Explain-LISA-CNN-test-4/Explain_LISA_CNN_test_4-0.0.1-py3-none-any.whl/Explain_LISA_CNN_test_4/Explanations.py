class Explanations:
    ...
    def anchor_explanations(self,n_segments,compactness,sigma,image,model,segmentation_fn="slic",
        ...
        if segmentation_fn == "slic":
            ...
            explainer = alibi.explainers.AnchorImage(predict_fn, image_shape=image_shape, segmentation_fn=segmentation_fn,
    ...
    def LIME_explanations(self,model,image,prediction,class_name=None,positive_only=True,num_features=10000,
        ...
        if positive_only==False and hide_rest==False:
            ...
            image, mask = explanation.get_image_and_mask(model.predict(image.reshape((1,224,224,3))).argmax(axis=1)[0],
                                                            ...
                                                            num_features=num_features,min_weight=min_weight)
        plt.axis('off')
        plt.imshow(skimage.segmentation.mark_boundaries(image, mask))
    ...
    def integrated_Gradients(self,model,img,prediction,class_name="Not given",n_steps=20,internal_batch_size=20,
        ...
        global explanation_list
        import alibi.explainers 
        import alibi.utils.visualization 
        ...
        ig  = alibi.explainers.IntegratedGradients(model,n_steps=n_steps,method=method,internal_batch_size=internal_batch_size)
        ...
        alibi.utils.visualization.visualize_image_attr(attr=None, original_image=(image * 255).astype(np.uint8), method='original_image',
                            title=f'Prediction {prediction}, Actual {class_name}', plt_fig_axis=(fig, ax[0]), 
                            use_pyplot=False);

        alibi.utils.visualization.visualize_image_attr(attr=attrs.squeeze(), original_image=image, method='blended_heat_map',
