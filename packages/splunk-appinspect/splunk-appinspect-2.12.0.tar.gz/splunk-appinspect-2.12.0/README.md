...
* There is a known bug with the testing suite. If you are not running the tests in a Python 3.7 context some stuff is going to fail. The bug is tracked [here](https://jira.splunk.com/browse/A3-188). The CI/CD pipeline will run the tests on every commit. The link to CI/CD jobs is [here](https://cd.splunkdev.com/appinspect/appinspect-cli/-/jobs)
