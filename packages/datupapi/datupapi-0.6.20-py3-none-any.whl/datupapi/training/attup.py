...
class Attup(Config):
    ...
    def training(self, data_m,n_features, n_train, verbose=0):
        ...
        for i in range(self.n_backtests+1):
            ...
            #------------------------------Define the model---------------------------------------------------
            model, checkpoint_best,LearningScheduler, earlyStopping=self.get_model(X=X, y=y,units=self.units, dropout_train=self.dropout_train, momentum=self.momentum, lr=self.lr, epochs=self.epochs, verbose=0 )
