...
def _test_validate_standalone(plugin_class: Type, input_path, result_path, regenerate):
    with log_context():
        # setup test
...
def _test_validate_manual(server, port, input_path, result_path, regenerate):
    with log_context():
...
def _test_validate_docker(docker_repository, input_path, result_path, regenerate):
    with log_context():
        # setup test
        test_runner = FlitsTestRunner(input_path, result_path, regenerate)
...
def main():
    ...
    if test_standalone:
        ...
        _test_validate_standalone(plugin_class, input_path, result_path, regenerate)
    if test_docker:
        _test_validate_docker(test_docker, input_path, result_path, regenerate)
    # if test_manual:
