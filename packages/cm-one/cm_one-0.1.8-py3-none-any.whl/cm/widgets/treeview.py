...
class MainTreeview:
	def __init__(self, master, operator, font='"Roboto" 12', height=8,
				 **kwargs):
		self.master = master
		...
		self.metaData = {}
		self.screenwidth = operator.terminal.w
		self.screenheight = operator.terminal.h
		self.height = self.get_screen_kf(height,
										 screen_height=operator.terminal.h,
										 screen_height_standart=1080)

	def get_screen_kf(self, table_height, screen_height,
					  screen_height_standart=1080):
		""" В связи с тем, что height в Treeview различный, для разных экранов,
		эта функция стандартизирует их, относительно
		заданного размера (screen_height_standart). Таким образом,
		высота таблицы в 18 единиц на экране 1080, будет
		выглядеть точно так-же и на разрешении 1366 (и других),
		поскольку превратится в 13: screen_kf = 1080/18 = 60
	...
	def sortTime(self, tv, col):
		...
		newList.sort(
			key=lambda x: datetime.datetime.strptime(x[1], '%H:%M:%S'),
	...
	def sortDate(self, tv, col):
		...
		try:
			newList.sort(
				key=lambda x: datetime.datetime.strptime(x[0], '%H:%M %d.%m'),
				reverse=self.reverse)
		except:
			newList.sort(
				key=lambda x: datetime.datetime.strptime(x[0], '%H:%M %d.%m'),
	...
	def sortId(self, tv, col, reverse=False):
		l = [(tv.item(k)["text"], k) for k in
			 tv.get_children()]  # Display column #0 cannot be set
		if reverse == False:
	...
	def OnClick(self, event):
		...
		if self.reverse:
			...
			self.img = PhotoImage(
				file=dirpath + os.sep + 'imgs' + os.sep + 'tric.png')
		else:
			print('not rev', self.reverse)
			self.img = PhotoImage(
				file=dirpath + os.sep + 'imgs' + os.sep + 'tricR.png')
	...
	def getRangeHistory(self, start_date, end_date, carrier=None,
						trash_cat=None, trash_type=None,
						car_number=None, alerts=None):
		ident = "time_in > '{}' AND time_in <= '{}'".format(start_date,
															end_date)
		if trash_cat:
			ident += " and trash_cat={}".format(trash_cat)
		if trash_type:
			ident += " and trash_type={}".format(trash_type)
		if carrier:
			ident += " and carrier={}".format(carrier)
		if car_number:
			ident += " and car_number='{}'".format(car_number)
...
class NotificationTreeview(MainTreeview):
	def __init__(self, operator, master, **kwargs):
		self.errors_desctription = {True: 'Подключение успешно',
									False: 'Подключение утеряно'}
		super().__init__(operator, master, **kwargs)

	def createTree(self):
		self.tree = ttk.Treeview(self.master, style="Custom.Treeview")
		self.tree["columns"] = ("one")
		self.tree.column("#0", width=int(self.operator.terminal.w / 4.8),
						 minwidth=int(self.operator.terminal.h / 36),
						 stretch='NO')
		self.tree.column("one", width=int(self.operator.terminal.w / 1.669),
						 minwidth=int(self.operator.terminal.h / 36),
						 stretch='NO')
...
class CurrentTreeview(MainTreeview):
	def init(self, operator, master, **kwargs):
		MainTreeview(self, operator, master, **kwargs)

	def createTree(self):
		self.tree = ttk.Treeview(self.master, style="Custom.Treeview")
		self.tree["columns"] = (
			"one", "two", "two1", "two2", "three", "four", "five")
		self.tree.column("#0", width=int(self.screenwidth / 14.0146),
						 minwidth=int(self.screenwidth / 64), stretch='NO')
		self.tree.column("one", width=int(self.screenwidth / 13.913),
						 minwidth=int(self.screenwidth / 64), stretch='NO')
		self.tree.column("two", width=int(self.screenwidth / 17.1428),
						 minwidth=int(self.screenwidth / 64),
						 stretch='NO')
		self.tree.column("two1", width=int(self.screenwidth / 17.1428),
						 minwidth=int(self.screenwidth / 64),
						 stretch='NO')
		self.tree.column("two2", width=int(self.screenwidth / 18.8235),
						 minwidth=int(self.screenwidth / 64),
						 stretch='NO')
		self.tree.column("three", width=int(self.screenwidth / 13.913),
						 minwidth=int(self.screenwidth / 64),
						 stretch='NO')
		self.tree.column("four", width=int(self.screenwidth / 13.913),
						 minwidth=int(self.screenwidth / 64),
						 stretch='NO')
		self.tree.column("five", width=int(self.screenwidth / 12.15189),
						 minwidth=int(self.screenwidth / 64),
						 stretch='NO')
	...
	def fillTree(self):
		...
		for rec in record:
			self.tree.insert("", 1, text=rec[0],
							 values=(self.getFrmtDate(rec[6]),
									 rec[2], rec[3], rec[4], rec[5], rec[1],
									 rec[7]))
...
class BansTreeview(MainTreeview):
	...
	def fillTree(self, debtorsList):
		...
		for debtor in debtorsList:
			self.tree.insert("", 1, text=debtor['name'],
							 values=(debtor['reason'], debtor['time']))
...
class HistroryTreeview(MainTreeview):
	...
	def createTree(self):
		...
		self.tree.column("1", width=int(self.screenwidth / 14),
						 minwidth=int(self.screenwidth / 64), stretch='NO')
		self.tree.column("2", width=int(self.screenwidth / 11),
						 minwidth=int(self.screenwidth / 64), anchor='w')
		self.tree.column("3", width=int(self.screenwidth / 18),
						 minwidth=int(self.screenwidth / 64), stretch='NO')
		self.tree.column("4", width=int(self.screenwidth / 18),
						 minwidth=int(self.screenwidth / 64), stretch='NO')
		self.tree.column("5", width=int(self.screenwidth / 18),
						 minwidth=int(self.screenwidth / 64), stretch='NO')
		self.tree.column("6", width=int(self.screenwidth / 14),
						 minwidth=int(self.screenwidth / 64), stretch='NO')
		self.tree.column("7", width=int(self.screenwidth / 14),
						 minwidth=int(self.screenwidth / 64))
		self.tree.column("8", width=int(self.screenwidth / 14),
						 minwidth=int(self.screenwidth / 64), stretch='NO')
		self.tree.column("9", width=int(self.screenwidth / 14),
						 minwidth=int(self.screenwidth / 64), stretch='NO')
		self.tree.column("10", width=int(self.screenwidth / 22),
						 minwidth=int(self.screenwidth / 64), stretch='NO')
		self.tree.column("11", width=int(self.screenwidth / 10),
						 minwidth=int(self.screenwidth / 64), stretch='NO')
		self.tree.bind("<Button-1>", self.OnClick)
	...
	def insertRec(self, rec):
		if rec[8] != None and rec[8] > '' and rec[14] == None:
			...
			self.tree.insert("", 1, text=rec[0], tags=('green',),
							 values=(rec[1],
