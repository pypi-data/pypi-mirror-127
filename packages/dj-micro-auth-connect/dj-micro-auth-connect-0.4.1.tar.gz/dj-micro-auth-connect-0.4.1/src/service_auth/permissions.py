...
class TokenValidation:
    def verify_token(request):
        ...
        if request.session.has_key("user") and request.session["user"] == token:
            return True
        verify = RemoteModel(request, 'auth', 'verify_token', token,tenant).verify_token(token)
    ...
    def get_user_permission(request):
        ...
        try:
            verify = bool(RemoteModel(request, 'user_perm', 'check_perm', token,request.tenant).get_permission(user))
            if verify:
                print(verify)
                request.session["user_type"] = verify
                return True
