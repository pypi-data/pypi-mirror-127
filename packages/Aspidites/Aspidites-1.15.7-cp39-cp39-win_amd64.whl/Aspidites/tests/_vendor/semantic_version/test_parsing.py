...
from Aspidites._vendor.semantic_version import Version, compare
    ...
    def test_invalid(self):
        for invalid in self.invalids:
            with self.subTest(version=invalid):
                self.assertRaises(ValueError, Version, invalid)
...
class ComparisonTestCase(unittest.TestCase):
    ...
    def test_comparisons(self):
        for i, first in enumerate(self.order):
            first_ver = Version(first)
            for j, second in enumerate(self.order):
                second_ver = Version(second)
                with self.subTest(first=first, second=second):
                    ...
                    self.assertEqual(cmp_res, compare(first, second))
