...
setattr(my_ob, 'my_attr', 'bar')"""
    ...
    with pytest.raises(TypeError) as excinfo:
        restricted_exec(setattr_without_guarded_writes_code, restricted_globals)
...
setattr(myobj_with_guarded_setattr, 'my_attr', 'bar')
    ...
    restricted_exec(set_attribute_using_guarded_setattr_code, restricted_globals)
...
@pytest.mark.skipif(
    IS_PY3, reason="__builtins__ has been renamed in Python3 to builtins."
)
...
def test_safer_getattr__underscore_name():
    ...
    assert '"__class__" is an invalid attribute name because it starts with "_"' == str(
        err.value
    )
