...
class CaptureTest(unittest.TestCase):
    ...
    def test_syntactic_sugar_rshift_operator(self):
                ...
                "containers": Each(
                    {
                        "image": _ >> "image",
                        "name": _ >> "name",
                        "ports": Each({"containerPort": _ >> "port"}),
                    }
                )
            },
        )
        self.assertTrue(result)
        self.assertEqual(
            "k8s.gcr.io/metrics-server/metrics-server:v0.4.1", result["image"]
        )
                ...
                "containers": Each(
                    {
                        "image": "image" @ _,
                        "name": "name" @ _,
                        "ports": Each({"containerPort": "port" @ _}),
                    }
                )
            },
        )
    ...
    def test_regex_bind_groups(self):
        result = match(
            "abcdef", Regex(r"(?P<one>[a-c]+)(?P<two>[a-z]+)", bind_groups=True)
        )
        self.assertTrue(result)
...
sample_k8s_response = {
    ...
    "volumes": [
        ...
        {
            "name": "metrics-server-token-2j86f",
            "secret": {"defaultMode": 420, "secretName": "metrics-server-token-2j86f"},
        },
    ],
}
