...
private_def_decl = (
    Literal("(").setParseAction(replaceWith("def ")).setName("function declarator")
)
args_end = (
    Group(Literal(")") + Literal(")"))
    .setParseAction(replaceWith(") -> "))
    .setName("return annotation")
)
...
bool_literal = (
    oneOf("True False", asKeyword=True)
    .setParseAction(lambda s, l, t: t[0] == "True")
    .setName("boolean literal")
)
...
persist = (
    Literal("[.]")
    .setParseAction(replaceWith(".persistent()"))
    .setName("persist trigram")
)
...
match_none = (
    Literal("(!)").setParseAction(replaceWith("__match")).setName("match trigram")
)
context_stmt = (
    Literal("<!>").setParseAction(replaceWith("with ")).setName("context trigram")
)
return_none = (
    Literal("<*>").setParseAction(replaceWith("return ")).setName("return trigram")
)
yield_none = (
    Literal("<^>").setParseAction(replaceWith("yield ")).setName("yield trigram")
)
...
cont_stmt = (
    Literal("<$>").setParseAction(replaceWith("continue")).setName("continue trigram")
)
break_stmt = (
    Literal("<%>").setParseAction(replaceWith("break")).setName("break trigram")
)
if_cond = (
    Literal("<?>").setParseAction(replaceWith("if ")).setName("conditional trigram")
)
respects = (
    Keyword("->").setParseAction(lambda s, l, t: ":").setName("contract assignment")
)
imposes = (
    Keyword("<-")
    .setParseAction(lambda s, l, t: "__new_contract")
    .setName("contract imposition")
)
