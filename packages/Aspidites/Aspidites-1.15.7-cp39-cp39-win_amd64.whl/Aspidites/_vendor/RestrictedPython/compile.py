...
def _compile_restricted_mode(
    ...
    if policy is None:
        ...
        if not issubclass(type(source), tuple(allowed_source_types)):
            raise TypeError(
                "Not allowed source type: " '"{0.__class__.__name__}".'.format(source)
            )
        c_ast = None
        ...
        if c_ast:
            policy_instance = policy(collected_errors, collected_warnings, used_names)
...
def compile_restricted_function(
    ...
    try:
        ...
        return CompileResult(code=None, errors=(error,), warnings=(), used_names=())
