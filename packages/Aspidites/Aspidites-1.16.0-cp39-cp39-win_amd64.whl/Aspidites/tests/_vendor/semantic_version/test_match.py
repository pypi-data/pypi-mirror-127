...
from Aspidites._vendor.semantic_version import Spec, SpecItem, Version, match
        ...
        for invalid in self.invalid_specs:
            with self.subTest(spec=invalid):
                with self.assertRaises(
                    ValueError, msg="Spec(%r) should be invalid" % invalid
                ):
                    Spec(invalid)
    ...
    def test_simple(self):
        for valid in self.valid_specs:
            with self.subTest(spec=valid):
                spec = SpecItem(valid)
                normalized = str(spec)
                self.assertEqual(spec, SpecItem(normalized))
        ...
        for spec_text, versions in self.matches.items():
            for version_text in versions:
                with self.subTest(spec=spec_text, version=version_text):
                    ...
                    self.assertTrue(
                        spec.match(version), "%r does not match %r" % (version, spec)
                    )
                    self.assertTrue(match(spec_text, version_text))

    def test_contains(self):
        spec = Spec("<=0.1.1")
        self.assertFalse("0.1.0" in spec, "0.1.0 should not be in %r" % spec)

        version = Version("0.1.1+4.2")
        self.assertTrue(version in spec, "%r should be in %r" % (version, spec))

        version = Version("0.1.1-rc1+4.2")
        self.assertTrue(version in spec, "%r should be in %r" % (version, spec))

    def test_prerelease_check(self):
        strict_spec = Spec(">=0.1.1-")
        lax_spec = Spec(">=0.1.1")
        version = Version("0.1.1-rc1+4.2")
        self.assertFalse(
            version in lax_spec, "%r should not be in %r" % (version, lax_spec)
        )
        self.assertFalse(
            version in strict_spec, "%r should not be in %r" % (version, strict_spec)
        )
