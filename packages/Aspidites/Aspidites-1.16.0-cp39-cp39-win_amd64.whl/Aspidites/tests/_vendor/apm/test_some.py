...
class Glob(unittest.TestCase):
    def test_empty(self):
        self.assertTrue(match([1, 2, 3], [1, Many(...), 2, 3]))
    ...
    def test_exactly(self):
        result = match(
            range(1, 10), [1, Many(..., exactly=3) >> "a", Many(...) >> "b", 6, 7, 8, 9]
        )
        self.assertTrue(result)
    ...
    def test_exactly_empty_following(self):
        result = match(
            range(1, 10), [1, Many(..., exactly=4) >> "a", Many(...) >> "b", 6, 7, 8, 9]
        )
        self.assertTrue(result)
        self.assertEqual([2, 3, 4, 5], result.a)
        self.assertEqual([], result.b)
        self.assertEqual(
            "MatchResult(matches=True, groups={'a': [2, 3, 4, 5], 'b': []})",
            repr(result),
        )
