...
def setup_code(inject_config):
    if Path(woma_file).exists():
        ...
        try:
            code_ = parse_module(
                open(str(Path("Aspidites/tests") / woma_file), "r").read()
            )
...
def test_cli_examples():
    ...
    python_file = (
        python_file_
        if Path(woma_file).exists()
        else (Path("Aspidites/tests") / python_file_)
    )
    with pt.raises(SystemExit):
...
@hypothesis.settings(
    suppress_health_check=[hypothesis.HealthCheck.filter_too_much], deadline=None
)
@hypothesis.given(
...
def test_parse_collections(t, u, v, w, x, y):
    ...
    assert (
        "".join(list_item.parseString(f"[{u}, 4, 6, {x}, {y}]"))
        == f"__pvector([__pvector({u}), 4, 6, {x}, {y}])"
    )
...
def test_parse_evolvers(w, x, y, z):
    ...
    assert (
        ...
        == "__pset({'a', 'b', 'c'}).evolver()"
    )
    assert (
        "".join(list_item.parseString("[2, 4, 6, 8, 10]..."))
        == "__pvector([2, 4, 6, 8, 10]).evolver()"
    )
...
def test_parse_bitwise(p, q, bitwise):
    assert str(p) + bitwise + str(q) == "".join(
        list_item.parseString(str(p) + bitwise + str(q))
    )
