...
class TerseStyleTest(unittest.TestCase):
    def test_fibonacci(self):
        def fibonacci(n):
            return match(
                n, 1, 1, 2, 1, _, lambda x: fibonacci(x - 1) + fibonacci(x - 2)
            )
    ...
    def test_underscore_as_key(self):
        ...
        self.assertEqual(3, match(pet, {"details": {"age": _}}, lambda age: age))
        self.assertEqual(
            ("details", 3), match(pet, {_: {"age": _}}, lambda a, b: (a, b))
        )
        self.assertEqual(
            ("details", 3), match(pet, {_ >> "key": {"age": _}}, lambda a, b: (a, b))
        )
    ...
    def test_dataclasses(self):
        ...
        self.assertEqual(
            ("rover", 7), match(pet, Pet(_, _), lambda name, age: (name, age))
        )
    ...
    def test_so_many_things(self):
        ...
        for val, expected in expectations:
            result = match(
                val,
                3,
                "this matches the number 3",
                int,
                "matches any integer",
                (str, int),
                lambda a, b: "a tuple (a, b) you can use in a function",
                [1, 2, _],
                "any list of 3 elements that begins with [1, 2]",
                {"x": _},
                "any dict with a key 'x' and any value associated",
                _,
                "anything else",
            )
    ...
    def test_regex(self):
        def what_is(pet):
            return match(
                pet,
                re.compile(r"(\w+)-(\w+)-cat$"),
                lambda name, my: "cat " + name,
                re.compile(r"(\w+)-(\w+)-dog$"),
    ...
    def test_lambda_with_too_many_parameters(self):
        self.assertEqual(
            "anything(1, None, None)",
            match(1, _, lambda a, b, c: f"anything({a}, {b}, {c})"),
        )
    ...
    def test_default_action(self):
        self.assertEqual(
            "fallback", match(1, 2, lambda: "something", lambda: "fallback")
        )
