...
class SymbolTableEntry(object):
    ...
    def attribute_str(self):
        ...
        return (
            "{0}={1}".format(self.attribute_name, self.attribute)
...
class CodeGenerator(object):
    ...
    def move(self, operand1, operand2):
        ...
        self.newline_text(
            "MOV \t{0},{1}".format(self.symbol(operand1), self.symbol(operand2)), True
        )
        if isinstance(operand2, int):
    ...
    def function_call(self, function, arguments):
        ...
        if args > 0:
            args_space = self.symtab.insert_constant(
                "{0}".format(args * 4), SharedData.TYPES.UNSIGNED
            )
...
class MicroC(object):
    ...
    def relexp_action(self, text, loc, arg):
        ...
        if not self.symtab.same_types(arg[0], arg[2]):
            raise SemanticException(
                "Invalid operands for operator '{0}'".format(arg[1])
            )
    ...
    def andexp_action(self, text, loc, arg):
        ...
        label = self.codegen.label(
            "false{0}".format(self.false_label_number), True, False
        )
    ...
    def logexp_action(self, text, loc, arg):
        ...
        self.codegen.newline_label(
            "false{0}".format(self.false_label_number), True, True
        )
        self.false_label_number += 1
    ...
    def if_else_action(self, text, loc, arg):
        ...
        self.codegen.newline_label(
            "false{0}".format(self.label_stack.pop()), True, True
        )
        self.label_stack.append(self.label_number)
