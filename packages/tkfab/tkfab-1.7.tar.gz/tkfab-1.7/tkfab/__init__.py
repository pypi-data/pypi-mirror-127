...
def _pkg_checkout(module, branch):
    local(f'cd {REPO_DICT[module]}')
    local(f'git checkout {branch}')
    logging.critical(f'module "{module}" has checkout to branch "{branch}"')
...
@task
def checkout(repo: str, branch: str = 'master'):
    _pkg_checkout(repo, branch)
