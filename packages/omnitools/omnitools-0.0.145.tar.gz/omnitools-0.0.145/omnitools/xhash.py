from hashlib import sha1, md5, sha256, sha384, sha512, blake2b, sha224
try:
...
def machd(key: str_or_bytes, content: str_or_bytes, method=sha3_512) -> str:
    return hmac.new(try_utf8e(key), try_utf8e(content), method).hexdigest()


def macd(key: str_or_bytes, content: str_or_bytes, method=sha3_512) -> bytes:
    return hmac.new(try_utf8e(key), try_utf8e(content), method).digest()
