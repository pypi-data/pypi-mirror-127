...
class Walker:
    ...
    def __init__(
        ...
        Note:
            ``reload`` and ``stop_level`` are mainly used to improve
            performance by requiring less work.  ``stop_level`` is used to
            filter out sections of the tree not needed, and ``reload`` speeds
            up execution by not calling ``<container>.reload()`` on each
            container, however this will also not leave the ``info`` field on
            the container.

        Args:
            root (Container):
                The root container, one of `flywheel.Project,flywheel.Subject,
                flywheel.Session, flywheel.Acquisition, flywheel.FileEntry,
                flywheel.AnalysisOutput`
            depth_first (bool): Depth first of breadth first traversal, True for
                depth first, False fo breadth first
            reload (bool): If `True`, reload containers when walking to load all
                metadata (default: True)
            stop_level (Optional[str]): Optional container type at which to stop walking.
                Default None. If specified, the children of this level will not be added
                to the walker, and thus will not be visited.
        """
        self.reload = reload
    ...
    def next(self, callback: Optional[Callable[[Any], bool]] = None) -> Any:
        ...
        Note:
            ``callback`` is used to filter out sections of the tree using a
            given function.  This can be used to speed up execution.

        Args:
            callback (Optional[Callable[[Container],bool]]).  Optional callback that
                takes in the ``next_element`` and returns a boolean of whether or
                not to queue its' children.  Defaults to None.
    ...
    def add(self, element: Union[List[Any], Any]):
        """Adds an element to the walker.

        Args:
            element (List[Container] or Container): Element or list of elements to add
                to deque
        """
        try:
    ...
    def _reload_container(self, container):
        """Returns reloaded container if `self.reload` is True."""
        if self.reload:
            return container.reload()
        return container

    def queue_children(self, element: Any) -> None:
    ...
    def is_empty(self):
        ...
        Returns:
            bool: Whether or not walker is empty.
        """
        return len(self.deque) == 0

    def walk(self, callback: Optional[Callable[[Any], bool]] = None) -> Iterable[Any]:
        """Walks the hierarchy from a root container.

        Note:
            ``callback`` is used to filter out sections of the tree using a
            given function.  This can be used to speed up/optimize execution.
        ...
        Yields:
            Iterator[Container]: Next element in walker.
        """
