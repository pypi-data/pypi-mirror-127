...
def start_download_by_password(url=None, password=[], show_info=True):
    pass
...
def ajax_form_info_get(initial_url, url, Qporm, title, show_info):
    ...
    try:
        if re_findall("var pdownload = (.*?);", handled_fu_html)[0] == []:
        ...
        kernel_form_postdown = re_findall("var ispostdowns = (.*?);", handled_fu_html)[0]
