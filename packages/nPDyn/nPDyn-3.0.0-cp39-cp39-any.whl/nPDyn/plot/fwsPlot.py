        ...
        for idx, subplot in enumerate(ax):
            ...
            if not self.noFit:
                if self.fitBox.isChecked():
                    if self.oRadioButton.isChecked():
                        Y = data.fit_best()[:, qIdx, eIdx]
                    elif self.qRadioButton.isChecked():
                        Y = data.fit_best()[obsIdx, :, eIdx]
                    elif self.eRadioButton.isChecked():
                        Y = data.fit_best()[obsIdx, qIdx]

                    # Plot the model
                    subplot.plot(
                        x, Y, label=data.model.name, zorder=3
                    )
