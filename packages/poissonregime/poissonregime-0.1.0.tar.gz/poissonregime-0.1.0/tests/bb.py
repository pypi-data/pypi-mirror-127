...
class EventsWithBackground(FitnessFunc):
    """Fitness for binned or unbinned events
    ...
    gamma : float or None
        ...
        p ~ gamma^N.  If gamma is specified, p0 is ignored.
    """
    def fitness(self, N_k, T_k, B_k):
        # eq. 19 from Scargle 2012
        lam_min = (N_k - B_k) / T_k
        lam_min[~(lam_min > 1e-4)] = 1e-4
        lam_max = (N_k + 1) / T_k
        alpha = 0.01
        rates = np.linspace(0, 1, 40).reshape((1, -1)) * (lam_max.reshape((-1, 1)) - lam_min.reshape((-1, 1))) + lam_min.reshape((-1, 1))
        likes = poissonregime.posterior(rates, N_k.reshape((-1, 1)), B_k.reshape((-1, 1)), alpha, exposure=T_k.reshape((-1, 1)))
        print(rates, N_k, B_k, T_k, likes)
        return np.log(likes.max(axis=1))
        #lam = (N_k - B_k) / T_k
        #lam[~(lam > 1e-4)] = 1e-4
        lamtot = lam * T_k + B_k
        #print(N_k, T_k, B_k, lam)
        return N_k * np.log(lamtot) - lamtot * np.log(T_k)
...
def bayesian_blocks(t, x=None, sigma=None,
    ...
    for R in range(N):
        ...
        # N_k: number of background counts expected in each block
        if 'B_k' in fitfunc.args:
            kwds['B_k'] = np.cumsum(B[:R + 1][::-1])[::-1]
...
def bbhist(edges, t, x):
	los = edges[:-1]
	his = edges[1:]
	values = np.array([x[np.logical_and(t >= lo, t < hi)].sum() / (hi - lo) for lo, hi in zip(edges[:-1], edges[1:])])
	return edges, values
...
if __name__ == '__main__':
    ...
    #src = np.arange(1, 11)
    src = np.random.poisson(np.zeros(N) + 10)
    print(bayesian_blocks(x=src, t=t, fitness=Events()))
    bkgmod = (np.sin(t)**2 * 20 * dT)
    bkgbkg = np.random.poisson(bkgmod * 100)
    srcbkg = np.random.poisson(bkgmod + 10)
    import matplotlib.pyplot as plt
    plt.plot(bkgmod)
    plt.plot(bkgbkg, 'o ')
    plt.plot(srcbkg, 'o ')
    plt.savefig('bbb.pdf', bbox_inches='tight')
    plt.close()
    print("bkg:", bkgbkg)
    print("src:", srcbkg)
    print("not accounting for background:")
    r = bayesian_blocks(x=srcbkg, t=t, fitness=Events())
    print(r)
    print("accounting for background:")
    #print(bayesian_blocks(x=np.arange(1, 11), B=np.zeros(10, dtype=int) + 1, t=np.arange(1000, 1200, 20), fitness=EventsWithBackground()))
    r2 = bayesian_blocks(x=srcbkg, B=bkgbkg, t=t, fitness=EventsWithBackground())
    print(r2)
    plt.plot(bkgmod)
    plt.plot(bkgbkg / 100, 'o ')
    plt.plot(t, srcbkg, 'o ')
    edges, values = bbhist(r2, t, srcbkg)
    plt.bar(x=(edges[1:] + edges[:-1]) / 2, width=edges[1:] - edges[:-1], height=values * dT, fill=False, edgecolor='b')
    edges, values = bbhist(r, t, srcbkg)
    plt.bar(x=(edges[1:] + edges[:-1]) / 2, width=edges[1:] - edges[:-1], height=values * dT, fill=False, edgecolor='r', lw=0.5)
    plt.savefig('bbb.pdf', bbox_inches='tight')
    plt.close()
