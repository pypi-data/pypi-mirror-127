...
class CPGBN(Basic_Model):
    ...
    def train(self, iter_all: int, *args, **kwargs):
        ...
        for iter in range(self._model_setting.Iteration):
            ...
            # update local_params
            self.local_params.c_n = np.random.gamma(self._hyper_params.c_n_e0 + np.sum(self._hyper_params.W_nk_gamma))
            self.local_params.c_n = self.local_params.c_n / (self._hyper_params.c_n_f0 + np.sum(np.sum(np.sum(W_nk_aug, axis=3), axis=2), axis=1))
            self.local_params.p_n = 1 / (self.local_params.c_n + 1)

            for t_phi in range(self._model_setting.T - 1):
                if t_phi == self._model_setting.T - 2:
                    self.local_params.c_j[t_phi] = np.random.gamma(self._hyper_params.c_j_e0 + np.sum(self._hyper_params.Theta_r_k, axis=0))
                else:
                    self.local_params.c_j[t_phi] = np.random.gamma(self._hyper_params.c_j_e0 + np.sum(self.local_params.Theta[t_phi+1], axis=0))
                self.local_params.c_j[t_phi] = self.local_params.c_j[t_phi] / (self._hyper_params.c_j_f0 + np.sum(self.local_params.Theta[t_phi], axis=0))
            ...
            for t_phi in range(self._model_setting.T - 2, -1, -1):
                ...
                Theta = np.random.gamma(Xt_to_t1[t_phi] + shape)
            ...
            self.local_params.W_nk = np.random.gamma(W_nk_aug + self._hyper_params.W_nk_gamma) / (1 + self.local_params.c_n)[:, np.newaxis, np.newaxis, np.newaxis]
