...
class WEDTM(Basic_Model):
    ...
    def train(self, embeddings: np.ndarray, S: int, iter_all: int, data: np.ndarray, is_train: bool = True):
        ...
        for Tcurrent in range(self._model_setting.T):
            ...
            for iter in range(1, self._model_setting.Iteration[Tcurrent]):
                ...
                if iter > 10:
                    if Tcurrent > 0:
                        p_j[1] = np.random.beta(np.sum(Xt_to_t1[0], 0)+a0, np.sum(self.Theta[1], 0)+b0)
                    else:
                        p_j[1] = np.random.beta(np.sum(Xt_to_t1[0], 0)+a0, np.sum(r_k)+b0)
    ...
    def sample_beta(self, n_topic_word, F, beta1):
        ...
        ## Eq. (3)
        log_inv_q = -np.log(np.random.beta(np.sum(beta1, 1), np.maximum(n_sum, realmin)))
