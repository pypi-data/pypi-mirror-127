    ...
    def train(self, iter_all: int, data: np.ndarray, is_train: bool = True):
        ...
        for iter in range(iter_all):
            ...
            for t in range(self._model_setting.T - 1, 0, -1):  # T-1 : 1
                ...
                L_kdott[:, t:t + 1] = self.Crt_Matrix(tmp1, self._hyper_params.tao0 * np.dot(self.global_params.Pi, self.local_params.Theta[:, t - 1:t]))
            ...
            if is_train:
                ...
                b[b == 0] = 1e-10  #
                self.global_params.q = np.maximum(np.random.beta(b, a), 0.00001)

                # Sample h checked
                for k1 in range(self._model_setting.K):
                    for k2 in range(self._model_setting.K):
                        self.global_params.h[k1:k1 + 1, k2:k2 + 1] = self.Crt_Matrix(L_KK[k1:k1 + 1, k2:k2 + 1], Piprior[k1:k1 + 1, k2:k2 + 1])
                ...
                for k in range(self._model_setting.K):
                    L_kdott[k:k + 1, 0:1] = self.Crt_Matrix(A_KT[k:k + 1, 0:1] + L_dotkt[k:k + 1, 1:2],
