    ...
    def train(self, iter_all: int, data: np.ndarray, is_train: bool = True):
        ...
        for iter in range(iter_all):
            ...
            for l in range(self._model_setting.L):
                ...
                if l == 0:
                    A_KT[l], A_VK[l] = self.Multrnd_Matrix(data, self.global_params.Phi[l], self.local_params.Theta[l])
                else:
                    A_KT[l], A_VK[l] = self.Multrnd_Matrix(Xt_to_t1[l], self.global_params.Phi[l], self.local_params.Theta[l])

                if l == self._model_setting.L - 1:
                    for t in range(self._model_setting.T - 1, 0, -1):  # T-1 : 1
                        tmp1 = A_KT[l][:, t:t + 1] + L_dotkt[l][:, t + 1:t + 2]
                        L_kdott[l][:, t:t + 1] = self.Crt_Matrix(tmp1, self._hyper_params.tao0 * np.dot(self.global_params.Pi[l], self.local_params.Theta[l][:, t - 1:t]))
                        L_dotkt[l][:, t:t + 1], tmp = self.Multrnd_Matrix_CPU(L_kdott[l][:, t:t + 1], self.global_params.Pi[l], self.local_params.Theta[l][:, t - 1:t])
                        L_KK[l] = L_KK[l] + tmp
                    ...
                    for t in range(self._model_setting.T - 1, 0, -1):
                        L_kdott[l][:, t:t + 1] = self.Crt_Matrix(A_KT[l][:, t:t + 1] + L_dotkt[l][:, t + 1:t + 2],
                                                                 self._hyper_params.tao0 * (np.dot(self.global_params.Phi[l + 1], self.local_params.Theta[l + 1][:, t:t + 1]) + np.dot(self.global_params.Pi[l], self.local_params.Theta[l][:, t - 1:t])))

                        # split layer2 count
                        tmp_input = np.array(L_kdott[l][:, t:t + 1], dtype=np.float64, order='C')
                        tmp1 = prob1[l][:, t - 1:t]
                        tmp2 = prob2[l][:, t:t + 1]
                        [tmp, X_layer[l][:, t, :]] = self.Multrnd_Matrix_CPU(tmp_input, np.concatenate((tmp1, tmp2), axis=1), np.ones((2, 1)))
                        ...
                        L_dotkt[l][:, t:t + 1], tmp = self.Multrnd_Matrix_CPU(tmp_input, self.global_params.Pi[l], np.array(self.local_params.Theta[l][:, t - 1:t], order='C'))
                        L_KK[l] = L_KK[l] + tmp

                    L_kdott[l][:, 0:1] = self.Crt_Matrix(A_KT[l][:, 0:1] + L_dotkt[l][:, 1:2],
                                                         self._hyper_params.tao0 * np.dot(self.global_params.Phi[l + 1], self.local_params.Theta[l + 1][:, 0:1]))
            ...
            for l in range(self._model_setting.L - 1, -1, -1):  # L-1 : 0
                if l == self._model_setting.L - 1:
                    for t in range(self._model_setting.T):
                        ...
                        self.local_params.Theta[l][:, t:t + 1] = np.random.gamma(shape) / scale
                else:
                    for t in range(self._model_setting.T):
                        if t == 0:
            ...
            if is_train:
                ...
                for l in range(self._model_setting.L):
                    ...
                    self.global_params.beta[l] = np.random.gamma(shape) / scale
                    ...
                    b[b == 0] = 1e-10  #
                    self.global_params.q[l] = np.maximum(np.random.beta(b, a), 0.00001)
                    # Sample h checked
                    for k1 in range(self._model_setting.K[l]):
                        for k2 in range(self._model_setting.K[l]):
                            self.global_params.h[l][k1:k1 + 1, k2:k2 + 1] = self.Crt_Matrix(L_KK[l][k1:k1 + 1, k2:k2 + 1], Piprior[l][k1:k1 + 1, k2:k2 + 1])
                    # Sample Xi checked
                    ...
                    self.global_params.Xi[l] = np.random.gamma(shape) / scale

                # Sample V # check
                for k in range(self._model_setting.K[self._model_setting.L - 1]):
                    L_kdott[self._model_setting.L - 1][k:k + 1, 0:1] = self.Crt_Matrix(A_KT[self._model_setting.L - 1][k:k + 1, 0:1] + L_dotkt[self._model_setting.L - 1][k:k + 1, 1:2],
                ...
                self.global_params.V[self._model_setting.L - 1] = np.random.gamma(shape_top) / scale_top
                ...
                if (self._model_setting.L > 1):
                    for l in range(self._model_setting.L - 1):
                        ...
                        self.global_params.V[l] = np.random.gamma(shape) / scale
