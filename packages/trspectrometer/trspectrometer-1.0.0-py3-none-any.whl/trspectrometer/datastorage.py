...
from ufsfile import UFSData                                 #pylint: disable=import-error 
...
class SignalStorage(QtCore.QObject):
    ...
    #: If the parameter is ``True```, it indicates that a complete data set was loaded from a file,
    #: and thus user interface controls should be updated to match. If ``False``, it indicates that
    #: the new data is from an acquisition process, and UI updates should not be performed.
    data_changed = Signal(bool)
    
    #: Signal that data has been updated.
    #: The parameters are lists of scan, time, wavelength indices which may have changed.
    #: None indicates that all elements have potentially changed.
