...
- :data:`~trspectrometer.plugins.acquisition.ta_dummy.TA_DummySteppedAcquisition`, which simulates a
  ...
  require :data:`~trspectrometer.plugins.interface`, :data:`~trspectrometer.plugins.chopper`, or
  :data:`~trspectrometer.plugins.detector` hardware, but does depend on a
  :data:`~trspectrometer.plugins.delay` being present (which may be a
  :data:`~trspectrometer.plugins.delay.dummydelay.DummyDelay`).
...
if not "sphinx-build" in os.path.basename(sys.argv[0]):
