...
class Agent:
    def __init__(self, app_name: str, key_pair: key_pair_format, **kwargs) -> None:
        self.__aese = lambda v, _={}: self.__check() and (
            ...
            (EasyRSA(public_key=key_pair["public_key"]).encrypt(_[0]), AESCipherCBCnoHASH(_[0]).encrypt(v))
        )
        ...
        self.__setk = lambda k, v: self.__check() and machd(sha3_512hd(k), v)
...
class AgentV1(Agent):
    def __init__(self, app_name: str, key_pair: key_pair_format) -> None:
        super().__init__(app_name, key_pair)
        self.__aese = lambda v, _={}: self.__check() and (
            _.update({0: Random.new().read(256)}) or
            (EasyRSA(public_key=key_pair["public_key"]).encrypt(_[0]), AESCipherCBC(_[0]).encrypt(v))
        )
