...
import re
from typing import TYPE_CHECKING, Iterable, List, Optional, Set, Text
...
class Generator:
	...
	def cleanup_old_docs(self):
		...
		for orphaned_dir in orphaned_dirs:
			self._logger.info(
				f"Deleting orphaned directory {self._root_path_finder.relative(orphaned_dir)}"
			)
			orphaned_dir.rmdir()
	...
	def _generate_doc(self, module_record, md_document):
		...
		self._logger.debug(
			f"Generating doc {self._root_path_finder.relative(md_document.path)} for {self._root_path_finder.relative(module_record.source_path)}"
		)
		try:
	...
	def generate_docs(self):
		...
		"""
		self._logger.debug(
			f"Generating docs for {self._project_name} to {self._root_path_finder.relative(self._output_path)}"
		)
	...
	def generate_index(self):
		...
		with self.md_index as md_index:
			...
			modules_section = f"Full {self._project_name} project documentation can be found in {md_index.render_md_doc_link(self.md_modules, title=self.MODULES_TITLE)}"
	...
	def generate_modules(self):
		...
		with self.md_modules as md_modules:
			...
			subtitle_parts.append(
				f"Full list of {md_modules.render_md_doc_link(self.md_index, title=self._project_name)} project modules."
			)
			md_modules.subtitle = "\n\n".join(subtitle_parts)
	...
	def _build_modules_toc_lines(self, import_string, max_depth, md_document, start_level):
