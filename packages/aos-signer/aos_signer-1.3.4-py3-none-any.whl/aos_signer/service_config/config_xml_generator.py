...
class ConfigXMLGenerator:
    ...
    def generate(self, file_details) -> Element:
        certificate = self._configuration.user_credentials.sign_cert_path
        private_key = self._configuration.user_credentials.sign_key_path

        with open(certificate, 'rb') as c:
            cert = c.read()

        with open(private_key, 'rb') as c:
            key = c.read()

        signed_root = XMLSigner().sign(self._root_element(file_details=file_details), key=key, cert=cert)
        return signed_root
