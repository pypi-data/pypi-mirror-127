...
class TempCredentials(object):
    ...
    def __enter__(self):
        self._key_file_name = TempCredentials.__create_temp_file(self._key)
        self._cert_file_name = TempCredentials.__create_temp_file(self._certificate)
        return self
    ...
    @staticmethod
    def __create_temp_file(data: bytes):
...
class UserCredentials(object):
    ...
    def find_sign_key_and_cert(self):
        if self._config.build.sign_pkcs12 is not None:
            ...
            with(self._sign_key_path, 'rb') as key_file:
                self._sign_key = key_file.read()

            with(self._sign_cert_path, 'rb') as cert_file:
