...
def download_file_kernel(fname,url):
    ...
    try:
        ...
        if global_dict['aria2']:
            os.system(f'aria2c {url} -o "{fname}"')
        else:
...
def main(args,parallel,url,include,exclude,ftypes,decode_name,convert_links,one,sort,include_html,wget,include_html_re,aria2,recursive):
    if not url:
        url = args[0]
