...
def convert(source:str, destination:str, orientation="P", delimiter=",",
    ...
    ***Possible Values** ➜ **'J' ➣ Justify** --- **'C' ➣ Center** --- **'L ➣ Left** --- **'R' ➣ Right***
    ...
    if font == None:
        PDF.add_font("normal-font", "", os.path.join(BASE_DIR, 'src', 'fonts', 'rs_normal.ttf'), uni=True)
    else:
        PDF.add_font("normal-font", "", font, uni=True)        

    if headerfont == None:
        PDF.add_font("header-font", "", os.path.join(BASE_DIR, 'src', 'fonts', 'rs_bold.ttf'), uni=True)
    else:
