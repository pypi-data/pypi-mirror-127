...
class StatefulAuthenticator(Authenticator):
    ...
    def __init__(self, app) -> None:
        super().__init__(app)
        self.redis = self.create_blocking_redis_client()

    def create_blocking_redis_client(self):
        return redis.StrictRedis(
            host=self.app.config.auth.redis.host,
            port=self.app.config.auth.redis.port,
        )
