...
class ModuleRepository:
    ...
    def register_runfiles(
        ...
        You can also test the globbing with exa.module_repository.glob.
        """
        metadata = module_repository_pb2.Metadata()
    ...
    def register_function_wrapper_module(
        ...
        **kwargs,
        ...
        if dependent_shared_object_paths is not None:
            dependent_so_ids = [self.register_shared_object(x) for x in dependent_shared_object_paths]
    ...
    def _parse_tag(self, tag: str) -> Tuple[str, str]:
        for c in tag:
            ...
            if oc < 0x20 or oc > 0x7E:
                raise ValueError("Module tag contains non-printable or non-ASCII characters")
    ...
    def _register_object(self, metadata: module_repository_pb2.Metadata, tag: Optional[str]) -> str:
