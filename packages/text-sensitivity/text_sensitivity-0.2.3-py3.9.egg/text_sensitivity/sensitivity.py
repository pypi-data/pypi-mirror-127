...
def invariance(pattern: str,
               ...
               **kwargs,
               ) -> Tuple[float, List[MemoryTextInstance], List[MemoryTextInstance], MemoryLabelProvider]:
    ...
    Returns:
        Tuple[float, List[MemoryTextInstance], List[MemoryTextInstance], MemoryLabelProvider]: Failure rate
            (percentage), list of correctly invariant instances, list of wrong instances (variant), and perturbation 
            labels for subset selection.
    """
    ...
    return len(wrong) / len(predictions), correct, wrong
