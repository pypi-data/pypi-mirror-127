...
def find_peaks(correlation, box_size_x, box_size_y, window_x, window_y):
    ...
    if maxcor2/(maxcor1+1e-10) > Parameters.peak_ratio or not len(fit_peak):
...
def load_images(name_img_1, name_img_2):
    ...
    Img1 = 0.1 + np.asarray(cv2.cvtColor(cv2.imread(name_img_1),
        cv2.COLOR_BGR2GRAY)).astype(np.float32)

    Img2 = 0.1 + np.asarray(cv2.cvtColor(cv2.imread(name_img_2),
        cv2.COLOR_BGR2GRAY)).astype(np.float32)
