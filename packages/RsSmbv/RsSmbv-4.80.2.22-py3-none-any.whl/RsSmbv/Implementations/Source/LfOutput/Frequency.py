...
class Frequency:
	...
	def set(self, frequency: float, lfOutput=repcap.LfOutput.Default) -> None:
		...
		Sets the frequency of the LF signal in LFOutput:FREQuency:MODE CW|FIXed mode.
			...
			- In sweep mode (LFOutput:FREQuency:MODE SWE) , the frequency is coupled with the sweep frequency. \n
	...
	def get_manual(self) -> float:
		...
		Sets the frequency of the subsequent sweep step if LFO:SWE:MODE MAN. Use a separate command for each sweep step. \n
			:return: manual: float You can select any value within the setting range, where: STARt is set with LFOutput:FREQuency:STARt STOP is set with LFOutput:FREQuency:STOP Range: STARt to STOP
		"""
	...
	def set_manual(self, manual: float) -> None:
		...
		Sets the frequency of the subsequent sweep step if LFO:SWE:MODE MAN. Use a separate command for each sweep step. \n
			:param manual: float You can select any value within the setting range, where: STARt is set with LFOutput:FREQuency:STARt STOP is set with LFOutput:FREQuency:STOP Range: STARt to STOP
		"""
	...
	def get_mode(self) -> enums.LfFreqMode:
		...
		Sets the mode for the output of the LF generator frequency, and determines the commands to be used for frequency settings. \n
			:return: mode: CW| FIXed| SWEep CW|FIXed Sets the fixed-frequency mode. CW and FIXed are synonyms. To set the output frequency, use command LFOutputch:FREQuency SWEep Sets sweep mode. To set the frequency, use the commands: LFOutput:FREQuency:STARt and LFOutput:FREQuency:STOP Or LFOutput:FREQuency:MANual
		"""
	...
	def set_mode(self, mode: enums.LfFreqMode) -> None:
		...
		Sets the mode for the output of the LF generator frequency, and determines the commands to be used for frequency settings. \n
			:param mode: CW| FIXed| SWEep CW|FIXed Sets the fixed-frequency mode. CW and FIXed are synonyms. To set the output frequency, use command LFOutputch:FREQuency SWEep Sets sweep mode. To set the frequency, use the commands: LFOutput:FREQuency:STARt and LFOutput:FREQuency:STOP Or LFOutput:FREQuency:MANual
		"""
