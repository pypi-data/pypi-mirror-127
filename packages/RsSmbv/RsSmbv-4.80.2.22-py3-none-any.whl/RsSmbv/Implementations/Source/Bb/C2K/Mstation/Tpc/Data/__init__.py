...
class Data:
	...
	def set(self, data: enums.TpcDataSour, mobileStation=repcap.MobileStation.Default) -> None:
		...
		Sets the data source for the power control bits of the traffic channels. \n
			:param data: ZERO| ONE| PATTern| DLISt DLISt A data list is used. The data list is selected with the command BB:C2K:MSTationst:TPC:DATA:DSELect. ZERO | ONE Internal 0 and 1 data is used. PATTern Internal data is used. The bit pattern for the data is defined by the command BB:C2K:MSTationst:TPC:DATA:PATTern. The maximum length is 64 bits.
