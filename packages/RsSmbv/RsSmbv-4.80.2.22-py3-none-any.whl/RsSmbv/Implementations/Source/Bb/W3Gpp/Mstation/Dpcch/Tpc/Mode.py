...
class Mode:
	...
	def set(self, mode: enums.TpcMode, mobileStation=repcap.MobileStation.Default) -> None:
		...
		Selects the TPC (Transmit Power Control) mode. The command sets the slot format (BB:W3GPp:MSTation<st>:DPCCh:SFORmat) in
		conjunction with the set TFCI status (BB:W3GPp:MSTation<st>:DPCCh:TFCI:STATe) and the FBI Mode
		(BB:W3GPp:MSTation<st>:DPCCh:FBI:MODE) to the associated values. \n
