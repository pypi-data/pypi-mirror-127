...
class Misuse:
	...
	def set(self, misuse: bool, baseStation=repcap.BaseStation.Default, channelNull=repcap.ChannelNull.Default) -> None:
		...
		The bit pattern (see command BB:W3GPp:BSTation<st>:CHANnel<ch0>:FDPCh:DPCCh:TPC:DATA:PATTern) of the TPC field of each
		channel is used to control the channel power. A '1' leads to an increase of channel powers, a '0' to a reduction of
		channel powers. Channel power is limited to the range 0 dB to -60 dB. The step width of the change is defined with the
		command BB:W3GPp:BSTation<st>:CHANnel<ch0>:FDPCh:DPCCh:TPC:PSTep. \n
			:param misuse: ON| OFF
