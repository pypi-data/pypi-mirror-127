...
class Sformat:
	...
	def set(self, sformat: int, channelNull=repcap.ChannelNull.Default) -> None:
		...
		standard is selected (BB:W3GPp:BSTation:ENHanced:CHANnel<ch0>:DPCH:CCODing:TYPE) and channel coding is activated, the
		slot format is (BB:W3GPp:BSTation:ENHanced:CHANnel<ch0>:DPCH:CCODing:STATe) automatically set to the associated value.
		Changing the slot format automatically activates User coding (W3GP:BST:ENH:CHAN<11...13>:DPCH:CCOD:TYPE USER) .
		The command sets the symbol rate (W3GP:BST:ENH:CHAN:DPCH:CCOD:SRAT) , the bits per frame
