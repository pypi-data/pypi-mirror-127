...
class Tchannel:
	...
	def get_flength(self) -> enums.Cdma2KpredFramLen:
		...
		The setting takes effect only after execution of command BB:C2K:PPARameter:EXECute. It is specific for the selected radio
		configuration. The frame length affects the data rates that are possible within a channel. Changing the frame length can
		lead to a change of data rate and this in turn can bring about a change of Walsh code. \n
