...
class State:
	...
	def set(self, state: bool, channelNull=repcap.ChannelNull.Default) -> None:
		...
		physical or the transport layer, BB:W3GPp:BSTation:ENHanced:CHANnel<ch0>:DPCH:DERRor:BIT:LAYer) . When the data source is
		read out, individual bits are deliberately inverted at random points in the data bit stream at the specified error rate
		in order to simulate an invalid signal. \n
			:param state: ON| OFF
