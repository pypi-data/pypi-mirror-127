...
class Mode:
	...
	def set(self, mode: enums.FbiMode, mobileStation=repcap.MobileStation.Default) -> None:
		...
		format (BB:W3GPp:MSTation<st>:DPCCh:SFORmat) in conjunction with the set TFCI status
		(BB:W3GPp:MSTation<st>:DPCCh:TFCI:STATe) and the TPC Mode (BB:W3GPp:MSTation<st>:DPCCh:TPC:MODE) to the associated values. \n
			:param mode: OFF| D1B
