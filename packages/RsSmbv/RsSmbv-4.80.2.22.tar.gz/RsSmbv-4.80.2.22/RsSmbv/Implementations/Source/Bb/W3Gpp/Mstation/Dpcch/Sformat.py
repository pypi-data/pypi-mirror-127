...
class Sformat:
	...
	def set(self, sformat: int, mobileStation=repcap.MobileStation.Default) -> None:
		...
		command sets the FBI mode (BB:W3GPp:MSTation<st>:DPCCh:FBI:MODE) ,
		the TFCI status (BB:W3GPp:MSTation<st>:DPCCh:TFCI:STATe) and the TPC Mode (BB:W3GPp:MSTation<st>:DPCCh:TPC:MODE) to the
		associated values. \n
