...
class Dme:
	...
	def ppst(self):
		...
		if not hasattr(self, '_ppst'):
			from .Ppst import Ppst
			self._ppst = Ppst(self._core, self._cmd_group)
		return self._ppst
	...
	def pinput(self):
		...
		if not hasattr(self, '_pinput'):
			...
			self._pinput = Pinput(self._core, self._cmd_group)
		return self._pinput
	...
	def rdistance(self):
		...
		if not hasattr(self, '_rdistance'):
			from .Rdistance import Rdistance
			self._rdistance = Rdistance(self._core, self._cmd_group)
	...
	def get_mode(self) -> enums.AvionicDmeMode:
		...
		signal for each mode is determined by the channel selected withDME:CSUFfix. \n
			:return: mode: INTerrogation| REPLy INTerrogation The interrogation signal of the airborne transmitter is simulated. REPLy The reply signal of the ground based transponder is simulated. The trigger is automatically set to external and the default trigger delay either to 50 us (channel X) or 56 us (channel Y) depending on the selected channel (DME:CSUFfix) . The interval between the pulse pairs can be set to a fixed value (repetition rate, DME:RATE) or to random generation (pulse squitter, DME:SQUitter) . The trigger signal is input via the Pulse Ext connector.
		"""
	...
	def get_pps(self) -> float:
		...
		of each pulse) . Available only for DME:SINGleOFF \n
	...
	def get_rate(self) -> int:
		...
		If DME:MODE is set to INTerrogation, sets the pulse repetition rate. If DME:MODE is set to REPLy, indicates the mean
		pulse repetition rate in squitter mode. \n
