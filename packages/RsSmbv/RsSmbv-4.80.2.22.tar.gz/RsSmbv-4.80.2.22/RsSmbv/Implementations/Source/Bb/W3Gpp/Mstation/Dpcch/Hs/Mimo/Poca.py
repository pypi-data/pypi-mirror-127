...
class Poca:
	...
	def set(self, poca: float, mobileStation=repcap.MobileStation.Default) -> None:
		...
		to the CQI Power PCQI (BB:W3GPp:MSTation<st>:DPCCh:HS:POWer) . The power PCQI Type A used during the PCI/CQI slots is
		calculated as: PCQI Type A = PCQI + Poff_CQI Type A Since the CQI Type B reports are used in a single stream transmission,
		the power PCQI Type B= PCQI. \n
