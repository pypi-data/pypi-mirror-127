...
class Mode:
	...
	def get_value(self) -> enums.AutoManStep:
		...
		Sets the cycle mode for the level sweep. \n
			:return: mode: AUTO| MANual| STEP AUTO Each trigger triggers exactly one complete sweep. MANual The trigger system is not active. You can trigger every step individually with the command POWer:MANual. The level value increases at each step by the value that you define with POWer. Values directly entered with the command POWer:MANual are not taken into account. STEP Each trigger triggers one sweep step only. The level increases by the value entered with POWer.
		"""
