...
exports.escapeFilterValue = exports.MutableSearch = exports.TokenType = void 0;
...
class MutableSearch {
            ...
            switch (token.type) {
                case TokenType.FILTER:
                    if (token.value === '' || token.value === null) {
                        ...
                        formattedTokens.push(`${token.key}:"${(0, utils_1.escapeDoubleQuotes)(token.value)}"`);
                    }
                    else {
                ...
                case TokenType.FREE_TEXT:
                    if (/[\s\(\)\\"]/g.test(token.value)) {
                        formattedTokens.push(`"${(0, utils_1.escapeDoubleQuotes)(token.value)}"`);
                    }
                    else {
    ...
    addStringFilter(filter, shouldEscape = true) {
        const [key, value] = parseFilter(filter);
        this.addFilterValues(key, [value], shouldEscape);
        return this;
    }
    addFilterValues(key, values, shouldEscape = true) {
        for (const value of values) {
            ...
            const token = { type: TokenType.FILTER, key, value: escaped };
            this.tokens.push(token);
        }
        return this;
    }
    setFilterValues(key, values, shouldEscape = true) {
    ...
    get filters() {
        const reducer = (acc, token) => {
            var _a;
            return (Object.assign(Object.assign({}, acc), { [token.key]: [...((_a = acc[token.key]) !== null && _a !== void 0 ? _a : []), token.value] }));
        };
        return this.tokens
    ...
    addFreeText(value) {
        const token = { type: TokenType.FREE_TEXT, value: formatQuery(value) };
        this.tokens.push(token);
        return this;
    }
    addOp(value) {
        const token = { type: TokenType.OPERATOR, value };
        this.tokens.push(token);
        return this;
    }
    get freeText() {
...
exports.MutableSearch = MutableSearch;
/**
