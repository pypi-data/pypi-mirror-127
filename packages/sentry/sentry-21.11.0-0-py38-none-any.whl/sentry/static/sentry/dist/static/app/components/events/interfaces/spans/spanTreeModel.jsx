...
class SpanTreeModel {
    constructor(parentSpan, childSpans, api, isRoot = false) {
        var _a;
        this.children = [];
        ...
        this.isSpanFilteredOut = (props) => {
            ...
            return !filterSpans.spanIDs.has((0, utils_1.getSpanID)(this.span));
        };
        this.getSpansList = (props) => {
            const { operationNameFilters, generateBounds, isLastSibling, hiddenSpanGroups, 
            // The set of ancestor span IDs whose sub-tree that the span belongs to
            spanGroups, filterSpans, previousSiblingEndTimestamp, event, isOnlySibling, spanGrouping, toggleSpanGroup, showSpanGroup, addTraceBounds, removeTraceBounds, } = props;
            let { treeDepth, continuingTreeDepths } = props;
            const parentSpanID = (0, utils_1.getSpanID)(this.span);
            ...
            const isNotLastSpanOfGroup = isOnlySibling && !this.isRoot && descendantsSource.length === 1;
            const shouldGroup = isNotLastSpanOfGroup;
            const hideSpanTree = hiddenSpanGroups.has(parentSpanID);
            const isLastSpanOfGroup = isOnlySibling && !this.isRoot && (descendantsSource.length !== 1 || hideSpanTree);
            const isFirstSpanOfGroup = shouldGroup &&
            ...
            if (isLastSpanOfGroup &&
                ...
                if (spanGrouping.length === 1) {
                    const treeDepthEntryFoo = (0, utils_1.isOrphanSpan)(spanGrouping[0].span)
            ...
            const spanGroupingCriteria = isLastSpanOfGroup && Array.isArray(spanGrouping) && spanGrouping.length > 1;
            const wrappedSpan = {
            ...
            const treeDepthEntry = (0, utils_1.isOrphanSpan)(this.span)
                ? { type: 'orphan', depth: treeDepth }
                : treeDepth;
            const shouldHideSpanOfGroup = shouldGroup &&
                !isLastSpanOfGroup &&
                ((toggleSpanGroup === undefined && !this.showSpanGroup) ||
                    (toggleSpanGroup !== undefined && !showSpanGroup));
            const descendantContinuingTreeDepths = isLastSibling || shouldHideSpanOfGroup
                ? continuingTreeDepths
            ...
            const { descendants } = (hideSpanTree ? [] : descendantsSource).reduce((acc, span, index) => {
                acc.descendants.push(...span.getSpansList({
                    ...
                    spanGrouping: shouldGroup
                        ? [...(spanGrouping !== null && spanGrouping !== void 0 ? spanGrouping : []), wrappedSpan]
                        : undefined,
            ...
            if (isLastSpanOfGroup &&
                ...
                return [
                    spanGroupChain,
                    Object.assign(Object.assign({}, wrappedSpan), { toggleSpanGroup: undefined }),
                    ...descendants,
                ];
            }
            ...
            const gapSpan = this.generateSpanGap(event, previousSiblingEndTimestamp, treeDepth, continuingTreeDepths);
            if (gapSpan) {
                return [gapSpan, wrappedSpan, ...descendants];
            }
            return [wrappedSpan, ...descendants];
        };
        this.toggleEmbeddedChildren = ({ addTraceBounds, removeTraceBounds, }) => (props) => {
            ...
            if (this.showEmbeddedChildren) {
                if (this.embeddedChildren.length === 0) {
                    return this.fetchEmbeddedTransactions(Object.assign(Object.assign({}, props), { addTraceBounds }));
                }
                this.embeddedChildren.forEach(child => {
        ...
        this.isRoot = isRoot;
        const spanID = (0, utils_1.getSpanID)(parentSpan);
        const spanChildren = (_a = childSpans === null || childSpans === void 0 ? void 0 : childSpans[spanID]) !== null && _a !== void 0 ? _a : [];
    ...
    get operationNameCounts() {
        ...
        for (const directChild of this.children) {
            ...
            for (const [key, count] of operationNameCounts) {
                result.set(key, ((_a = result.get(key)) !== null && _a !== void 0 ? _a : 0) + count);
            }
        }
        // sort alphabetically using case insensitive comparison
        return new Map([...result].sort((a, b) => String(a[0]).localeCompare(b[0], undefined, { sensitivity: 'base' })));
    }
    generateSpanGap(event, previousSiblingEndTimestamp, treeDepth, continuingTreeDepths) {
        // hide gap spans (i.e. "missing instrumentation" spans) for browser js transactions,
        // since they're not useful to indicate
        const shouldIncludeGap = !(0, utils_1.isEventFromBrowserJavaScriptSDK)(event);
        const isValidGap = shouldIncludeGap &&
        ...
        const gapSpan = {
            ...
            span: {
                ...
                description: (0, locale_1.t)('Missing instrumentation'),
                isOrphan: (0, utils_1.isOrphanSpan)(this.span),
            },
            numOfSpanChildren: 0,
            treeDepth,
            isLastSibling: false,
    ...
    fetchEmbeddedTransactions({ orgSlug, eventSlug, addTraceBounds, }) {
        ...
        return this.api
            ...
            .then((0, mobx_1.action)('fetchEmbeddedTransactionsSuccess', (event) => {
            if (!event) {
                return;
            }
            const parsedTrace = (0, utils_1.parseTrace)(event);
            const rootSpan = (0, utils_1.generateRootSpan)(parsedTrace);
            const parsedRootSpan = new SpanTreeModel(rootSpan, parsedTrace.childSpans, this.api, false);
            this.embeddedChildren = [parsedRootSpan];
            this.fetchEmbeddedChildrenState = 'idle';
            addTraceBounds(parsedRootSpan.generateTraceBounds());
        }))
            .catch((0, mobx_1.action)('fetchEmbeddedTransactionsError', () => {
            this.embeddedChildren = [];
