...
const wink_jaro_distance_1 = (0, tslib_1.__importDefault)(require("wink-jaro-distance"));
...
function diffTransactions({ baselineEvent, regressionEvent, }) {
    var _a, _b;
    ...
    const spansToBeCompared = [
        {
            type: 'root',
            baselineSpan: (0, utils_1.generateRootSpan)(baselineTrace),
            regressionSpan: (0, utils_1.generateRootSpan)(regressionTrace),
        },
    ];
    while (spansToBeCompared.length > 0) {
        ...
        const { comparablePairs, children } = createChildPairs({
            ...
            baseChildren: (_a = baselineTrace.childSpans[baselineSpan.span_id]) !== null && _a !== void 0 ? _a : [],
            regressionChildren: (_b = regressionTrace.childSpans[regressionSpan.span_id]) !== null && _b !== void 0 ? _b : [],
        });
...
function createChildPairs({ parent_span_id, baseChildren, regressionChildren, }) {
    ...
    for (const baselineSpan of baseChildren) {
        ...
        const candidates = remainingRegressionChildren.reduce((acc, regressionSpan, index) => {
        ...
        const baselineSpanDuration = Math.abs(baselineSpan.timestamp - baselineSpan.start_timestamp);
        const { regressionSpan, index } = candidates.reduce((bestCandidate, nextCandidate) => {
            ...
            // start timestamp
            const deltaStartTimestampThisSpan = Math.abs(thisSpan.start_timestamp - baselineSpan.start_timestamp);
            const deltaStartTimestampOtherSpan = Math.abs(otherSpan.start_timestamp - baselineSpan.start_timestamp);
            ...
            const thisSpanScore = deltaDurationThisSpan + deltaStartTimestampThisSpan + (1 - thisSpanMatchScore);
            const otherSpanScore = deltaDurationOtherSpan + deltaStartTimestampOtherSpan + (1 - otherSpanMatchScore);
...
function jaroSimilarity(thisString, otherString) {
    ...
    let jaroDistance = (0, wink_jaro_distance_1.default)(thisString, otherString).distance;
...
function matchableSpans({ baselineSpan, regressionSpan, }) {
...
function generateMergedSpanId({ baselineSpan, regressionSpan, }) {
...
function getMatchedSpanDurationDeltas({ baselineSpan, regressionSpan, }) {
...
function isOrphanDiffSpan(diffSpan) {
    switch (diffSpan.comparisonResult) {
        case 'matched': {
            return (0, utils_1.isOrphanSpan)(diffSpan.baselineSpan) || (0, utils_1.isOrphanSpan)(diffSpan.regressionSpan);
        }
        case 'baseline': {
            return (0, utils_1.isOrphanSpan)(diffSpan.baselineSpan);
        }
        case 'regression': {
            return (0, utils_1.isOrphanSpan)(diffSpan.regressionSpan);
        }
        default: {
            throw Error('Unknown comparisonResult');
        }
...
function generateWidth({ duration, largestDuration, }) {
    if (duration <= 0) {
        return {
            type: 'WIDTH_PIXEL',
            width: 1,
...
function boundsGenerator(rootSpans) {
    ...
    //            comparison page.
    const largestDuration = Math.max(...rootSpans.map(rootSpan => {
...
function generateCSSWidth(width) {
    ...
    switch (width.type) {
        ...
        case 'WIDTH_PERCENTAGE': {
            return (0, utils_2.toPercent)(width.width);
        }
        default: {
