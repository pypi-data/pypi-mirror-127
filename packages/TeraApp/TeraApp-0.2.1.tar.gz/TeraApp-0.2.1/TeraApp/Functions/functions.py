...
class TeraAppWIcon:
  ...
  def txtcf(self,txt, x, y, size, color, font):
    self.txtcf_name = QtWidgets.QLabel(self.win)
    self.txtcf_name.setSizePolicy(QtWidgets.QSizePolicy.Ignored, QtWidgets.QSizePolicy.Ignored)
    self.txtcf_name.resize(20000,70)
    self.txtcf_name.setText(txt)
    self.txtcf_name.move(x, y)
    self.txtcf_name.setFont(QFont(f'{font}', size))
    self.txtcf_name.setStyleSheet(f"color: {color}; background-color: none")
    self.txtcf_name.show()
